...
void Cabin_ReloadStartFade()
{
	...
	// удаляем отражения и фонари кораблей
	aref ship;
	if (FindEntity(&ship, "ship")) 
	{
		LayerDelObject(SEA_REFLECTION2, &ship);
		while (FindEntityNext(&ship)) 
		{
			LayerDelObject(SEA_REFLECTION2, &ship);
		}
	}
	LayerDelObject(SEA_SUNROAD, &ShipLights);
	// удаляем отражения островов
	LayerDelObject(SEA_REFLECTION2, &IslandReflModel);
	LayerDelObject(SUN_TRACE, &Island);
...
void Return2SeaAfterCabin()
{
	...
	if (!QuestCheckReturn2SeaAfterCabin()) {
		return;
	}
...
{
	Log_TestInfo("Cabin_ReloadEndFadeAfter");
	...
	Whr_UpdateWeather();
	// восстанавливаем отражения и фонари кораблей
	aref ship;
	if (FindEntity(&ship, "ship")) 
	{
		LayerAddObject(SEA_REFLECTION2, &ship, 100);
		while (FindEntityNext(&ship)) 
		{
		  LayerAddObject(SEA_REFLECTION2, &ship, 100);
		}
	...
	// восстанавливаем отражения островов
	LayerAddObject(SEA_REFLECTION2, &IslandReflModel, -1);
	LayerAddObject(SUN_TRACE, &Island, 1);
	Sea_UpdateIslandGrass(AISea.Island);
	if(isEntity(&Island) && !IsEntity(&CoastFoam)) CreateCoastFoamEnvironment(AISea.Island, SEA_EXECUTE, SEA_REALIZE);
}
...
void Sea_DeckBoatStartNow(ref _iShipsCharacter)
{
	...
	if (!bAbordageStarted)
	{
	    ...
	    //PlaySound("interface\" + LanguageGetLanguage() + "\_Abandon0.wav");// шлюпки на воду!
