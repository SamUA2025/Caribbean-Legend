...
float Cannon_GetFireHeight()
{
	...
	// введем логику и учёт от расстояния. также учёт что, бомбы тяжелее ядер
...
float Cannon_DamageEvent()
{
	...
	if (aCharacter.index == GetMainCharacterIndex() && GetCharacterIndex("Knippel") != -1 && sti(aCharacter.Fellows.Passengers.cannoner) != -1)
	{
		ref rKnippel = CharacterFromID("Knippel");
		if (IsCharacterPerkOn(rKnippel, "Bombardier") && sti(aCharacter.Fellows.Passengers.cannoner) == sti(rKnippel.index)) fCurDamage = fCurDamage/2.0;
	}
	...
	if (fCurDamage >= 1.0)
	{
		...
		if (sti(aCharacter.index) == GetMainCharacterIndex())
		{
		    ...
		    notification(XI_ConvertString("Cannon_DamageEvent"), "Cannons");
		}
