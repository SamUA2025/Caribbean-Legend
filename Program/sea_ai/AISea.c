...
{
	float seaUpdateTime = 0.25;	//Mett: для запуска апдейта моря
	DelEventHandler("SeaAI_SailToEndFadeEvent", "SeaAI_SailToEndFade");
	SendMessage(&SailToFader, "lfl", FADER_IN, 0.5 + seaUpdateTime, true);
	...
	//Mett: обновление моря, обновление текста в интерфейсе --->
...
void SeaAI_SailToCreateFader()
{
	...
	SendMessage(&SailToFader, "lfl", FADER_IN, 0.5, false);
