...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Quel genre de questions ?","Que voulez-vous, "+GetAddress_Form(NPChar)+"?"),"Des questions encore, "+GetAddress_Form(NPChar)+"... ","Questions, questions... Que diriez-vous d'échanger, "+GetAddress_Form(NPChar)+"hein ?","Écoute, combien de temps comptes-tu parler ?","block",1,npchar,Dialog.CurrentNode);
