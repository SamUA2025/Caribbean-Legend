...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Quel genre de questions ?","Que désirez-vous, "+GetAddress_Form(NPChar)+"?"),"Encore des questions, "+GetAddress_Form(NPChar)+"...","Des questions, des questions... Et si nous faisions des affaires, "+GetAddress_Form(NPChar)+"hein ?","Ecoutez, combien de temps comptez-vous parler?","block",1,npchar,Dialog.CurrentNode);
