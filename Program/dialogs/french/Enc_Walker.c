...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Mon mari ne me permet pas de parler avec des étrangers!","Il n'y a rien dont nous pourrions parler.");
				Link.l1 = "Ce n'est pas grave.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Dites-moi, quels sont les potins populaires à la taverne locale?","Préféreriez-vous me dire ce qui se passe par ici?","Hey, camarade, pourrais-tu me dire comment va la vie sur la terre ferme?");
		...
		case "sovet":
			...
			link.l10 = LinkRandPhrase("Dites-moi, quels sont les ragots populaires à la taverne locale?","Préféreriez-vous me dire ce qui se passe ici?","Hey, l'ami, pourrais-tu me dire comment est la vie sur la terre ferme?");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("Oh oui, je suis content"+NPCharSexPhrase(NPChar,"","")+" te voir est presque comme une coupe de tafia. Demande ce que tu veux.","Vous aimez parler, capitaine? Eh bien, moi aussi... Surtout autour d'une tasse de rhum.","Oui, "+PChar.name+"?"),LinkRandPhrase("De quoi avez-vous besoin?","Je vois, capitaine "+PChar.name+", tu es vraiment un bavard ?","Des questions fatigantes, Capitaine, ce serait mieux si vous me traitiez avec du rhum.")),PCharRepPhrase(LinkRandPhrase("Qu'est-ce que tu voulais savoir "+GetAddress_Form(NPChar)+"?","Je vous écoute, capitaine.","Je dois partir, donc veuillez faire vite, capitaine."),LinkRandPhrase("Toujours heureux d'avoir une bonne compagnie, "+GetAddress_Form(NPChar)+" "+PChar.lastname+". Parle ton esprit.","Oui, "+GetAddress_Form(NPChar)+"?","Vous aimez bavarder, capitaine? Eh bien, moi aussi...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("Quels types de rumeurs sont populaires dans la taverne locale?","Qu'est-ce qui se passe dans ces terres?","Quoi de neuf sur la terre par ici?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("Non, tu entends des choses. Je m'en vais maintenant.","Non, rien. Je suis juste sorti pour une promenade."),RandPhraseSimple("Non, rien. Bonne chance à toi!","Juste en train de faire une promenade. Au revoir."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Écoutez, en tant que citoyen de cette ville, je dois vous demander de vous abstenir de vous promener avec une lame non gainée.");
				link.l1 = LinkRandPhrase("D'accord.","D'accord.","Comme vous dites...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Retiens tes chevaux, "+GetSexPhrase("camarade","jeune fille")+", quand tu te promènes avec une arme à la main. Cela peut me rendre nerveux...","Je n'aime pas quand "+GetSexPhrase("hommes","gens")+" se promener avec des armes prêtes. Ça me fait peur...");
				link.l1 = RandPhraseSimple("Je l'ai eu.","Je l'enlèverai.");
			}
			link.l1.go = "exit";
