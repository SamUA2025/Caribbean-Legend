...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		{
			dialog.text = "What do you want, captain? To discuss my ransom? I'd like you to know that I care about the girl's fate...";
		...
		if (CheckAttribute(pchar, "questTemp.FMQG") && pchar.questTemp.FMQG != "fail" && CheckAttribute(pchar, "questTemp.FMQG.Prisoner") && GetFullName(npchar) == pchar.questTemp.FMQG.Prisoner)
		{
			...
			link.l4.go = "Slave_1";
			link.l5 = "You have ignored an order to lower your sails and resisted with force. You will be hanged for it. Boatswain has already received my orders.";
		...
		case "second time":
			if(CheckAttribute(NPChar,"Hold_GenQuest") && !CheckAttribute(pchar,"GenQuest.Hold_GenQuest"))
			{
				...
				{				
					...
					case 1:
						...
						if (attrLoc != "none") 
						{
							link.l2 = "You have ignored an order to lower your sails and resisted with force. You will be hanged for it. Boatswain has already received my orders.";
						...
						link.l3 = LinkRandPhrase("Don't test my patience. You are already a lucky man to stay alive.",
							"I have got my own plans for you.","I suppose that you can still be useful for me in future.");
						link.l3.go = "exit";	
					break;
					
					case 2: // "ransom"
						dialog.text = "Captain, this uncertainty is killing me. May I know what do you plan to do with me?";
			...
			else
			{
				if (sti(NPChar.nation) == PIRATE)
				{
					dialog.text = "Captain, why won't you just let me go? You don't need me. Choosing to be a pirate was the only option for me because I have no valuable skills or education. My wish was to make some coin and retire then.";
					if (FindFreeRandomOfficer() > 0)
					{
				...
				else
				{
					...
					{
						dialog.text = RandPhraseSimple("Captain, I have lost a fight and I beg you for your mercy. ",
						...
						link.l2 = LinkRandPhrase("Don't test my patience. You are already a lucky man to stay alive.", 
							RandPhraseSimple("I have got my own plans for you.","I suppose that you can still be useful for me in future."),
							...
							"You are... well you were a captain of war ship and you were aware of risks. Don't try to represent yourself as a saint here."));
						link.l2.go = "exit";	
		...
		case "free_to_officer":
			...
			{
				dialog.text = "Serving under your command? I'd better feed the sharks!";
		...
		case "free_to_officer_trader":
			if (GetSummonSkillFromName(pchar, "Leadership") <= GetSummonSkillFromName(npchar, "Leadership"))
			{
				dialog.text = LinkRandPhrase("It is a tempting offer... but I have to reject it. On principle.",
					...
					"No, captain. After all of this terror I can't do this job anymore. I made a vow that if the Blessed Virgin will hear my prayers and grant me a freedom I won't sail anymore.");
		...
		case "Slave_2":
			...
			else
			{
                if (rand(1) == 1)
                {
                    if (GetPrisonerQty() > 1)
                    {
                        dialog.text = "Butcher! Guys join me, we have got nothing to lose!";
		...
		case "free_withoutFee":
			dialog.text = "Oh, " + GetAddress_Form(NPChar) + ", are you really that kind, and you are releasing me without a ransom?";
		...
		case "free_withoutFee_4":	
			switch(NPChar.EncType)
			{
				case "pirate":
					...
					link.l1 = "If you are being honest then I wish you luck, and if not then pray to never meet me again...";	
				break;
				case "trade":
		...
		case "PunishmentAction1":
			...
			if (rand(1) == 1)
            {
                if (GetPrisonerQty() > 1)
                {
                    dialog.text = "Butcher! Guys, we have nothing to lose!";
                    link.l1 = "Wrong choice, ladies. I deal with mutinies by myself and in a very fast way...";
					link.l1.go = "free_in_sea_battle_all";
                }
				else
		...
		case "free_tip_off":
			...
			link.l1 = "How can I give you my guarantees if I don't even know your proposal? I doubt that your information will even give me anything.";
		...
		case "free_tip_off_1":
			...
			dialog.text = "Fine. It's enough for me. Now straight to the business, in " + XI_ConvertString("Colony" + NPChar.Hold_GenQuest.City + "Voc") + " lives a wealthy merchant " + NPChar.Hold_GenQuest.Name + ". " + 
				...
				"When he has too much cargo he pays for an escort." + sTmp + " I am sure that this information more valuable for you than a life of a single man.";
		...
		case "free_tip_off_2":
			...
			link.l1 = RandPhraseSimple(RandPhraseSimple("I wish I could say the same about you","Sorry but I can't say the same about you."),
				RandPhraseSimple("Go and remember that next time I won't be that generous.","Go and stay out of my way. Next time I won't believe you so easily."));
		...
		case "free_buyout":
			dialog.text = "And I have a proposal for you. In " + XI_ConvertString("Colony" + NPChar.Hold_GenQuest.City + "Voc") + " lives my friend, " + NPChar.Hold_GenQuest.Name + " is his name. We had a business together once." +
		...
		case "free_by_hoard":
			...
			link.l2 = "Spare me of your stories. You'd better worry about your soul.";
			link.l2.go = "free_tip_off_0";
		break;
		...
		case "free_by_hoard1":
			...
			link.l1 = "I will decide when and what to do with you by myself. Your 'stories' won't make a difference.";
			link.l1.go = "free_tip_off_0";
		...
		case "free_by_hoard2":
			...
			link.l1 = "Tell me your story or I will let you go right now... overboard and headless.";
			link.l1.go = "free_by_hoard4";
		break;
		
		case "free_by_hoard4":
			dialog.text = "Ha-ha, nice joke... Now listen. There is one pal in " + XI_ConvertString("Colony" + NPChar.Hold_GenQuest.City + "Voc") + 
				" named " + NPChar.Hold_GenQuest.Name + ", he was a really reckless guy in his youth. He had a lot of 'deals' those days..." + 
				"So once in the tavern he told me that he was a member of " + GetName( NAMETYPE_VIP, NPChar.Hold_GenQuest.PirateName, NAME_ACC) + " gang and he saw how those pirates were burying a treasure." +
				" He had five of his buddies killed over that treasure... There were gossips that he got caught and hanged, but truth is that he has withdrawn from business, and so he started a family. He remembered the location of the treasure, even had drew a map. He offered me to accompany him in finding the treasure. A superstitious man he is, the treasure was covered in blood, so he fears to go alone there. I admit, I didn't venture either because of the same reason. "+ 
		...
		case "free_by_hoard5":
			dialog.text = "My thanks, captain! Now I see the price of the word of one "+ GetSexPhrase("gentleman!","lady!");
			link.l1 = "Go and stay out of my way. Next time I won't let you go so easily.");
		...
		case "free_in_sea":
			dialog.text = "But, " + GetAddress_Form(NPChar) + ", we are in the open sea! It's the same as if you executed me!";
		...
		case "FalseTrace_Prisoner":
			...
			link.l1 = "Let's stay away from poetry captain, and talk about more serious things. I'm going to tell you about the current situation and I am expecting an answer from you. Your answer will determine your and Katerine's fate.";
		...
		case "FalseTrace_Prisoner_3":
			dialog.text = "Adam made a promise? Ha-ha! And you have trusted him? Captain, do you even know what kind of person he is? He...";
			link.l1 = "Katerine has already told me a lot about him and put a great effort in that. But his proposal make sense and though I have no reason to trust this bastard but maybe this time he is telling the truth. So let's get straight to the point...";
			link.l1.go = "FalseTrace_Prisoner_4";
		break;
		
		case "FalseTrace_Prisoner_4":
			dialog.text = "";
			link.l1 = "I was going to ransom you to the authorities and to deliver Adam with Katerine to "+XI_ConvertString("Colony"+pchar.questTemp.FalseTrace.QuestCity+"Acc")+", then get from Adam the information about gold of Squint-Eyed Solly and sail to it. But Katerine has persuaded me to talk with you first, she said that you might offer me something better and more reliable than Adam did, in exchange for your lives and freedom.";
			link.l1.go = "FalseTrace_Prisoner_5";
		break;
		
		case "FalseTrace_Prisoner_5":
			dialog.text = "";
			link.l1 = "That's why I am standing here and talking to you. I have finished. Now it is your turn. Think about my words and words of Katerine, and give me your ideas. You don't have much time.";
			link.l1.go = "FalseTrace_Prisoner_6";
		break;
		
		case "FalseTrace_Prisoner_6":
			dialog.text = "Captain! Though I don't think that you are a good man, but you did listen to pleading of a poor girl and you came here. It shows me that you can be honorable and feel pity. I don't need time, I am ready to give you my proposal.";
		...
		case "FMQG_2":
			...
			link.l1 = "Go tell me then. I will land you in Mosquito's bay, same as I did the last time. But lie to me and I will return and set that plantation you were gifted by don Carlos on fire, I will also inform Basse-Terre of your whereabouts, so I won't be the only one hunting you.";
			link.l1.go = "FMQG_3";
		break;
		
		case "FMQG_3":
			dialog.text = "First, land me there. That will make me feel safer and I swear I'll tell you everything.";
			link.l1 = "Fine. It's not like you are going anywhere.";
			link.l1.go = "FMQG_4";
		break;
