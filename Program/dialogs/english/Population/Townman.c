...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (sti(NPChar.nation) == PIRATE)
			{
				...
				{ // пираты, не наши
					...
					{
						dialog.text = RandPhraseSimple("Alarm! An armed lunatic is here! To arms!", "To arms! An armed lunatic is here!");
            ...
            if(NPChar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Good day, "+GetAddress_Form(NPChar)+". I see that you are a captain of your own ship. I want to ask you about a favor...";
			...
			else
			{
				...
				dialog.Text = NPCharRepPhrase(npchar,
                        ...
                        + Pchar.name, "Oh, that's the "+ GetSexPhrase("captain himself","captain herself") +" "
                         +GetFullName(Pchar))
						+RandPhraseSimple(". I thought that you were dead!", ". And perhaps "+ GetSexPhrase("drunk as always.","a bit drunk.") )
		...
		case "ShipLetters_7":
			...
			link.l1 = "Does the name on these papers mean anything to you?";
			link.l1.go = "ShipLetters_8";
		break;
		
		case "ShipLetters_8":
			dialog.text = LinkRandPhrase("I don't give a damn about some random paper and plus I don't know how to read. Go to the usurer, they say that he knows how to do shit with letters, ha!",
			"I don't know what you are talking about, ask someone else, captain" + GetFullName(Pchar) + ".", 
			"I don't know how to help you, captain " + GetFullName(Pchar) + ". Try to talk with the usurer, he might know.");
			link.l1 = "Thanks.";
			link.l1.go = "exit";
		break;
		...
		case "Meeting":
            ...
            link.l1 = PCharRepPhrase("I just wanna ask something about this shithole you call 'town'.", "I want to ask you something about this town.");
			link.l1.go = "new question";
            Link.l2 = RandPhraseSimple("I've got a question for you.", "I need some information.");
			if (CheckCharacterItem(pchar, "CaptainBook"))				
			{
		...
		case "new question":
			...
			link.l5 = PCharRepPhrase(RandPhraseSimple("Your ears deceive you. I have to go.", "No, it's nothing, I'm on my way to the tavern."),
		...
		case "Device_Citizen_1":
			if (sti(pchar.GenQuest.Device.Shipyarder.Chance1) > 3 && sti(pchar.GenQuest.Device.Shipyarder.Chance1) < 6 && npchar.sex != "woman")
			{
				...
				link.l1 = "How did he look like and where was he heading to? I really need something like that.";
		...
		case "passenger":
			...
			{
				dialog.text = "Captain, I need to get to the colony " + XI_ConvertString("Colony"+pchar.GenQuest.Townpassenger.City)+", it's on a "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Townpassenger.City)+"Voc")+", in a "+FindRussianDaysString(sti(pchar.GenQuest.Townpassenger.DaysQty))+". I know this sounds extremely risky, but maybe " + FindRussianDublonString(sti(pchar.GenQuest.Townpassenger.Money)) + " would be a nice boost for you?";
			}
		...
		case "passenger_1":
			dialog.text = "Great, because I'm tired of waiting. You will get your payment when we will be there.";
...
// <--
		...
		case "passenger_3":
			...
			dialog.text = "Here we are, excellent! This travel on your ship was quite satisfying. My thanks. Take your money, sir.";
...
//<-- горожанин-пассажир
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar, "Be careful while running with a weapon. I can get nervous...", "I don't like it when some stranger is walking in front of me with a naked sword...");
