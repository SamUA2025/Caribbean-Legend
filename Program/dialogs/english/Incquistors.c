void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
					dialog.text = RandPhraseSimple("Brethren, there's an enemy in the ranks of the Inquisition!", "Raise the alarm, brethren!");
			...
			if (CheckAttribute(npchar, "protector.CheckAlways")) //гарды на камерах
			{
				if (GetNationRelation2MainCharacter(sti(NPChar.nation)) == RELATION_ENEMY)
				{					
					dialog.text = "To arms! It's a spy! Seize him!";
					link.l1 = "Caramba!";
					link.l1.go = "fight";
				}
				else
			...
			if (findsubstr(npchar.id, "Prisoner", 0) != -1) //заключенные
			{
				...
				{
					dialog.text = LinkRandPhrase("Those hypocrites are totally inhuman. They will burn in the hell for everything they've done to us...", "They were torturing and tormenting me every day! But I am not guilty in our Lord's face!", "My cellmate died yesterday... They had been torturing him on the rack for two days... And who knows how long will I endure ...");
