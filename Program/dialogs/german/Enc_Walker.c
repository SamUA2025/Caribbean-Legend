// диалог из ПКМ
#include "DIALOGS\%language%\Enc_Walker.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Mein Mann erlaubt mir nicht, mit Fremden zu sprechen!","Es gibt nichts, worüber wir reden könnten.");
				Link.l1 = "Keine große Sache.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Sag mir, was sind die beliebten Klatschgeschichten in der örtlichen Taverne?","Möchten Sie mir vielleicht erzählen, was hier vor sich geht?","Hallo, Kamerad, könntest du mir erzählen, wie das Leben an Land ist?");
		...
		case "sovet":
			...
			{
                dialog.text = RandPhraseSimple("Mein Mann erlaubt mir nicht, mit Fremden zu sprechen!","Es gibt nichts, worüber wir sprechen könnten.");
				Link.l1 = "Keine große Sache.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Sag mir, was sind die beliebten Klatsch und Tratsch in der örtlichen Taverne?","Könntest du mir eher sagen, was hier vor sich geht?","Hallo, Kamerad, könntest du mir sagen, wie das Leben an Land ist?");
			link.l10.go = "rumours_citizen"; //повесил на горожанина, если нужны отедельные слухи, то пишем соответствующую реализацию
		...
		case "new question":
            dialog.text = NPCharRepPhrase(npchar,PCharRepPhrase(LinkRandPhrase("Oh ja, ich bin froh"+NPCharSexPhrase(NPChar,"","")+" Dich zu sehen ist fast wie ein Schluck Schnaps. Frag, was du willst.","Redest du gerne, Kapitän? Nun, ich auch... Besonders bei einem Becher Rum.","Ja, "+PChar.name+"?"),LinkRandPhrase("Was brauchst du?","Ich verstehe, Kapitän "+PChar.name+", du bist ziemlich ein Schwätzer?","Mühsame Fragen, Kapitän, es wäre besser, wenn Sie mich mit Rum behandeln würden.")),PCharRepPhrase(LinkRandPhrase("Was wolltest du wissen "+GetAddress_Form(NPChar)+"?","Ich höre Ihnen zu, Kapitän.","Ich muss los, also bitte beeilen Sie sich, Kapitän."),LinkRandPhrase("Immer froh, nette Gesellschaft zu haben, "+GetAddress_Form(NPChar)+" "+PChar.lastname+". Sprich Deinen Verstand aus.","Ja, "+GetAddress_Form(NPChar)+"?","Du plauderst gerne, Kapitän? Nun, ich auch...")));

            // homo 25/06/06
			link.l1 = LinkRandPhrase("Welche Art von Klatsch ist in der örtlichen Taverne beliebt?","Was geht in diesen Ländern vor?","Was gibt's Neues an Land hier unten?");
			link.l1.go = "rumours_citizen";
			link.l5 = PCharRepPhrase(RandPhraseSimple("Nein, du hörst Dinge. Ich gehe jetzt.","Nein, nichts. Bin nur spazieren gegangen."),RandPhraseSimple("Nein, nichts. Viel Glück dir!","Nur einen Spaziergang machen. Auf Wiedersehen."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Hör zu, als Bürger dieser Stadt, muss ich dich bitten, nicht mit einer gezogenen Klinge herumzulaufen.");
				link.l1 = LinkRandPhrase("Gut.","Gut.","Wie du sagst...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Halt die Pferde, "+GetSexPhrase("Kumpel","lass")+", wenn man mit einer Waffe in der Hand herumläuft. Das kann mich nervös machen...","Ich mag es nicht, wenn "+GetSexPhrase("Männer","Leute")+" lauf mit gezogenen Waffen herum. Es macht mir Angst...");
				link.l1 = RandPhraseSimple("Ich habe es.","Ich werde es entfernen.");
			}
			link.l1.go = "exit";
