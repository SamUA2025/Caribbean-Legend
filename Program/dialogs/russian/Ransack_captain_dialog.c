...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		{
			dialog.text = "Что вам угодно, капитан? Вы пришли обсудить вопрос о моём выкупе? Я бы хотел вас уведомить, что мне крайне небезразлична судьба девушки...";
			link.l1 = "Катерины Рейнер? Я уже в курсе, "+pchar.questTemp.FalseTrace.CapName+". Именно по её просьбе я здесь.";
		...
		case "free_buyout":
			dialog.text = "Верно, предложение имеется. В " + XI_ConvertString("Colony" + NPChar.Hold_GenQuest.City + "Voc") + " живёт мой хороший товарищ, " + NPChar.Hold_GenQuest.Name + " зовут, мы дело вместе начинали. " + // belamour gen
		...
		case "free_by_hoard4":
			dialog.text = "Хе-хе, удачная шутка, хе-хе... А история следующая. Живёт в " + XI_ConvertString("Colony" + NPChar.Hold_GenQuest.City + "Voc") + " человек "+  // belamour gen
		...
		case "FalseTrace_Prisoner":
			dialog.text = "Катерина! По её просьбе вы здесь? Удивительная девушка...";
			link.l1 = "Оставим лирику, капитан, и перейдём к вопросам более приземлённым. Я сейчас обрисую сложившуюся ситуацию и буду ждать вашего ответа. От того, что вы сможете мне предложить, будет зависеть и ваша судьба, и судьба Катерины.";
			link.l1.go = "FalseTrace_Prisoner_1";
		break;
		
		case "FalseTrace_Prisoner_1":
			dialog.text = "Я слушаю вас внимательно, капитан.";
			link.l1 = "Ваш галеон я захватил по наводке Адама Рейнера, мужа Катерины. Он-то и сказал, что на нём полно серебра. Этот обман позволил ему вернуть назад свою жену, сбежавшую от него, как я понимаю, с вами.";
			link.l1.go = "FalseTrace_Prisoner_2";
		break;
		
		case "FalseTrace_Prisoner_2":
			dialog.text = "Адам? Эта скотина? Так вот кто всё организовал! Теперь всё становится на свои места... Да, это я помог три месяца назад Катерине бежать из дома. Несчастная девушка, вы даже не представляете, как он с ней обращался...";
		...
		case "FalseTrace_Prisoner_4":
			dialog.text = "";
			link.l1 = "В мои планы входило следующее: вас сдать за выкуп властям, Адама и Катерину доставить в "+XI_ConvertString("Colony"+pchar.questTemp.FalseTrace.QuestCity+"Acc")+", получить у Адама информацию о золоте некоего Косого Солли и отправиться за ним. Но Катерина уговорила меня поговорить с вами, сказав, что вы сможете предложить мне нечто более существенное, и менее сомнительное, нежели Адам, за ваши жизни и свободу.";
			link.l1.go = "FalseTrace_Prisoner_5";
		break;
		
		case "FalseTrace_Prisoner_5":
			dialog.text = "";
			link.l1 = "Вот поэтому я здесь стою и распинаюсь перед вами. Я сказал"+GetSexPhrase("","а")+" всё. Теперь ваша очередь. Подумайте над моими словами и словами Катерины и дайте свой ответ. Времени на размышления у вас немного, так что не тяните.";
		...
		case "FalseTrace_Prisoner_7":
			dialog.text = "Вы арестуете Адама Рейнера как преступника, наденете на него кандалы и посадите в трюм. Затем отвезёте меня и Катерину в "+XI_ConvertString("Colony"+pchar.questTemp.FalseTrace.TargetCity)+", куда мы и направлялись. В сам город не причаливайте, чтобы не возникло ненужных эксцессов - ведь это всё-таки вы, а не кто-то другой захватил мой галеон - и высадите нас у " + XI_ConvertString(pchar.questTemp.FalseTrace.TargetShore+"Gen") + ", всех троих: меня, Катерину и Адама\nТакже я попрошу у вас оружие для себя. Мы доберёмся в город по суше, а там я уже представлю дело так, что вся вина ляжет на этого негодяя. Заслуженно, между прочим - ведь это он ввёл вас в заблуждение и организовал нападение\nВаше имя никаким образом упомянуто не будет и преследования можете не опасаться - даю слово. Адама вздёрнут на виселицу, так что он тоже не сможет вам вредить. А за это я дам вам истинные сведения о корабле враждебной державы, скрытно перевозящем золотую руду\nВы сможете без труда захватить его - добыча намного превзойдёт и выкуп за меня, и мифические сокровища Косого Солли, существуй они даже на самом деле.";
			link.l1 = "Что-то не внушает доверия мне ваш план... Сдаётся мне, что это ловушка, и не успею я отойти от " + XI_ConvertString(pchar.questTemp.FalseTrace.TargetShore+"Gen") + " на полсотни миль, как за мной будет отряжена погоня. Нет, капитан, я не намерен"+GetSexPhrase("","а")+" рисковать своей головой. На этом разговор окончен.";
		...
		// Addon-2016 Jason ФМК-Гваделупа
		case "FMQG":
			dialog.text = "Ты мне пришёл морали читать, или для более важных дел?";
