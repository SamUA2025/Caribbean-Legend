void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.text = "Ой, "+GetSexPhrase("молодой человек","милое дитя")+", мы сегодня закрыты.";
				link.l1 = "Закрыты?";
			...
			if (CheckAttribute(pchar, "questTemp.ChickenGod.HostessDialog")) {
				dialog.text = "О, боже! Как вы сюда попали? "+GetSexPhrase("Дорогой капитан","Дорогая")+", я умоляю вас: пожалуйста, немедленно уходите! Наше заведение закрыто!";
				link.l1 = "Рад"+GetSexPhrase("","а")+" вас видеть, дорогая " + npchar.name + ". Пожалуйста, успокойтесь! Если вы так переживаете из-за того странного индейца, Агуэбаны, то я здесь как раз по его просьбе. Чёрт, не смейтесь, пожалуйста, но нам нужна комната.";
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie.Gigolo") && pchar.questTemp.Sharlie.Gigolo == "start" && npchar.location == "FortFrance_SecBrRoom")
			{
				...
				link.l1 = "Послушай, Аврора, мне нужна девочка на эту ночь. Причём не наверху в твоём заведении, а у меня дома. Ты можешь мне помочь?";
                link.l1.go = "Gigolo";
				break;
			}
			//<-- туториал Мартиника
			...
			if (CheckAttribute(pchar, "GenQuest.Badboy.Complete") || CheckAttribute(pchar, "GenQuest.Badboy.Continue"))
			{
				if (npchar.City == pchar.GenQuest.Badboy.Brothel.City)
				{
					...
					link.l5 = "Да. Более он сюда не придёт.";
			...
			if (CheckAttribute(pchar, "questTemp.HWIC.Detector") || SandBoxMode)
			{
				...
				if (!CheckAttribute(pchar, "questTemp.Portugal") && bOk && !CheckAttribute(npchar, "quest.Portugal") && npchar.location == "Marigo_SecBrRoom" && makeint(environment.time) > 6.0 && makeint(environment.time) < 22.0)
				{
					...
					link.l5 = ""+GetSexPhrase("Для вас, мадам, всё что угодно... я к вашим услугам!","Я к вашим услугам!")+"";
					link.l5.go = "Portugal";
					link.l8 = "Извини, но я сейчас занят"+GetSexPhrase("","а")+".";
					link.l8.go = "exit";
			...
			{
				DelLandQuestMark(npchar);
				dialog.text = "Вы говорили с Хьюго, "+GetSexPhrase("минхер капитан","мефрау")+"?";
				...
				{
					link.l1 = "Да, я передал"+GetSexPhrase("","а")+" вашу просьбу.";
			...
			if (CheckAttribute(pchar, "questTemp.Mtraxx.Retribution") && pchar.questTemp.Mtraxx.Retribution == "brothel_repeat" && npchar.location == "Tortuga_SecBrRoom")
			{
				...
				{
					link.l1 = "Конечно! Чарли Принц слов на ветер не бросает, в отличие от двух-трёх десятков тысяч песо, ха-ха-ха!";
			...
			{
				link.l8 = "Послушай, я хотел бы снять лучшую девушку, которая у тебя есть.";
				link.l8.go = "NA_Girl";
			}
			// <-- приключенец
        ...
        case "Hostess_NotChoice":
			...
			else
			{
				...
				{
					dialog.text = ""+ GetSexPhrase("Ну что, жеребец, превосходно!","У меня они все умелые, можешь не сомневаться.") +" Могу предложить прекрасную девушку по имени " + GetFullName(sld) + ", она сейчас свободна.\nОй, а что это ты там так скромно прячешь?! Неужели эта та самая высшая награда Франции? Ходят слухи, что носители этого знака отличия легендарны не только на поле брани... Ну, ты понимаешь о чём я, мой флибустьер... К тому же, любая сочтёт за честь прикоснуться к твоему 'Ордену', поэтому сегодня отдых за счёт заведения. Отказы не принимаются, красавчик.";
					Link.l1 = "Конечно, соглас"+ GetSexPhrase("ен","на") +", какие могут быть вопросы?!";
        ...
        case "Hostess_NotChoice_agree":
			...
			if (sti(pchar.money) >= (sti(sld.quest.price) + charWhoreCost))
			{
				...
				Link.l1.go = "exit";
				// belamour legendary edition деньги только без ордена 
				if(IsEquipTalisman9())
				{
		...
		case "Hostess_inSexRoom":
			...
			if (CheckAttribute(pchar, "questTemp.RomNaTroih_Pogovoril"))
			{
				...
				dialog.text = "" + sld.name + "! Мой герой, вы не теряли времени зря!";
				link.l1 = "Надеюсь, вы простите мне эту слабость, моя королева?";
        ...
        case "Horse_2":
			if (rand(1))
			{
				...
				{
					link.l1 = "Дорогая, именно это я и собирался сделать! Как удачно, что твоя хозяйка как раз назначила мне приватную встречу наверху. Пойдём вместе? Обсудим все детали втроём - за бутылочкой хорошего вина.";
			...
			{
				dialog.text = LinkRandPhrase(""+ GetSexPhrase("Знаешь что я тебе скажу, милый мой? Не нужно мне пустых разговоров. Займись делом!","Ну вот только этого мне не хватало - комплиментов от женщин!") +"", ""+ GetSexPhrase("Ты тоже воображаешь, что все женщины любят ушами? Хм, это очень зря... Если хочешь меня - плати хозяйке заведения, а трепотня мне не нужна.","Девушка, не растрачивайтесь на пустые слова. Хотите дел - платите...") +"", ""+ GetSexPhrase("Ещё один любитель чувственности... Плати - и я буду твоей, но только без этого нежного бреда!","Ты чего, дорогуша? Хочешь удовольствия - плати, а не занимайся болтовней!") +"");
        ...
        case "Horse_4": 
			...
			link.l1 = HeroStringReactionRepeat("Да, я знаю.", "Я помню.",
                      "Можешь не повторять, о ней я помню.", "Хм, о чём это ты?..", npchar, Dialog.CurrentNode);
			link.l1.go = "exit";
        ...
        case "HorseChoice_1": 
			...
			else
			{
				...
				Link.l1.go = "exit";
				Link.l2 = "Мне очень приятно, что ты меня запомнила, но сейчас провести с тобой ночь я, к сожалению, не могу.";
		...
		case "Badboy_1":
			dialog.text = "В такое время он обычно ошивается в таверне. Накачивается вином, а потом уже идёт сюда.";
		...
		case "Portugal":
			...
			link.l1 = "Не нужно слов! Я найду его и вытряхну всё до последнего песо"+GetSexPhrase(", за один лишь взгляд ваших бездонных глаз","")+"! Назовите его имя и только!";
			link.l1.go = "Portugal_1";
		break;
		
		case "Portugal_1":
			dialog.text = "Нет-нет, вы меня неправильно поняли, капитан! Этот человек... не делайте ему ничего дурного. Просто напомните ему о долге. Его зовут Хьюго Авендел, и, я полагаю, вы найдёте его там, где подают самую дешёвую выпивку. Только Бога ради, не применяйте к нему силу!\nПросто скажите... напомните, что он обещал заплатить. Я не хочу обращаться к страже, но у меня есть знакомые офицеры, так что передайте ему - я жду, и при всём моём к нему отношении, не позднее завтрашнего дня, я вынуждена буду принять меры. Просто напомните ему о его обещании.";
			link.l1 = ""+GetSexPhrase("Ваше слово - закон для меня, мадам. Я с удовольствием выполню вашу просьбу.","Я с удовольствием выполню вашу просьбу.")+"";
		...
		case "Portugal_exit":
			...
			link.l1 = "Я уверен"+GetSexPhrase("","а")+" в этом, мадам. На этом позвольте откланяться.";
			link.l1.go = "exit";
		...
		case "Portugal_4":
			...
			link.l1 = "Хорошо-хорошо, я понял вас, мадам. Не нужно возвращать деньги - они ваши, и я не сделаю ничего вашему другу. А сейчас, разрешите откланяться - меня ждут дела. Рад"+GetSexPhrase("","а")+" был"+GetSexPhrase("","а")+" помочь столь прекрасной даме.";
		...
		case "mtraxx_7":
			...
			link.l2 = "В другой раз, дорогая. Считай, дарю тебе обеденный перерыв. В церковь там сходи, или ещё чего.";
		...
		// belamour ночной приключенец
		case "NA_Girl":
            dialog.text = "О, так у нас тут ценитель, хи-хи... Что ж, хорошо, есть у меня такая девушка. Но стоит она недёшево: "+makeint(pchar.GenQuest.NightAdventure_money)+" песо."; 
		...
		case "NA_Girl_1":
            ...
            dialog.text = "Вот и отлично, дорогой. Жрица любви будет ждать тебя наверху. Поверь, ты не пожалеешь."; 
			link.l1 = "Уже не жалею.";
		...
		case "chicken_god":
			...
			if (sti(pchar.money) >= 10000) {
				...
				link.l2 = "Хм, прямо сейчас у меня нет такой суммы с собой. Может быть мы рассмотрим возможность кре...";
				link.l2.go = "chicken_god_notpay";
			}
		break;
		
		case "chicken_god_pay":
			dialog.text = "Приятного отдыха, капитан. Если вам захочется разнообразить процесс ласками женских рук - не стесняйтесь, мои девочки не станут задавать лишних вопросов.";
			link.l1 = "...";
		...
		case "chicken_god_pay_1":
			dialog.text = "";
			link.l1 = "Мадам " + npchar.name + ", может хоть вы объясните, что тут происходит? Губернатор вообще в курсе, что у вас тут всем уже месяц командует абориген? Ни один город не переживёт закрытия борделя хотя бы на день, я это точно могу сказать!";
			link.l1.go = "chicken_god_pay_2";
		break;
		
		case "chicken_god_pay_2":
			dialog.text = "Комната ваша, "+GetSexPhrase("молодой человек","девушка")+"! А теперь, прошу меня извинить...";
			link.l1 = "Очаровательно. До свидания, мадам.";
			link.l1.go = "exit";
		...
		case "chicken_god_notpay":
			dialog.text = "Вон отсюда! Уходите, уходите, я так больше не могу! "+GetSexPhrase("Мерзавец","Мерзавка")+"!";
			link.l1 = "Конфуз, однако.";
			link.l1.go = "exit";
		...
		case "RomNaTroih_1":
			dialog.text = "М-м-м... не совсем моё это, но с тобой я готова рискнуть. Сегодня твой счастливый день, капитан.";
			link.l1 = "Идём!";
			link.l1.go = "exit";
		...
		case "RomNaTroih_2":
			dialog.text = "Вы сильно разозлили меня, капитан... Но вам повезло, и сегодня я склонна быть милостивой... Если вы постараетесь...";
			link.l1 = "Я буду очень стараться. Откроем вино?";
			link.l1.go = "exit";
		...
		case "SantaMisericordia_1":
			dialog.text = "Пока этот проклятый де Аламида не уберётся. Чёртов святоша...";
			link.l1 = "А может...";
			link.l1.go = "SantaMisericordia_2";
		break;
		
		case "SantaMisericordia_2":
			dialog.text = "Никаких 'может'. Что, если он зайдёт сюда прямо сейчас? Все вон.";
			link.l1 = "...";
			link.l1.go = "exit";
