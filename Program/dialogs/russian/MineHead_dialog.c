	...
	switch(Dialog.CurrentNode)
	{
		...
		case "slaves":
			...
			if (sti(location.quest.slaves.qty) < 5)
			{
				...
				link.l1 = "Хорошо, сударь. Я всё понимаю: не нужны сейчас - понадобятся потом.";
				link.l1.go = "exit";
			}
			else
			{
		...
		case "slaves_1":
			...
			link.l1.go = "slaves_g";
			link.l2 = "Серебряные самородки.";
			link.l2.go = "slaves_s";
			link.l3 = "Извините, сеньор, но что-то я передумал"+GetSexPhrase("","а")+". В другой раз.";
