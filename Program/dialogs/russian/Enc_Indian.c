...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				dialog.text = "Здравствуй, "+GetSexPhrase("белый брат","белая сестра")+". Ты хотеть говорить индеец?";
				link.l1 = "Приветствую, сын джунглей. Я рад"+GetSexPhrase("","а")+" тебя видеть, но мне пора продолжить свой путь.";
				link.l1.go = "exit";
				link.l2 = "Да. Я слышал"+GetSexPhrase("","а")+", у вас можно купить интересные вещички. У тебя нет чего-нибудь на продажу?";
		...
		case "select_trade": //индейская торговля
			switch (drand(7))
			{
				case 0: // торговля через интерфейс
					dialog.text = "Тебе повезти, "+GetSexPhrase("белый брат","белая сестра")+". У "+npchar.name+" есть мала-мала вещей на продажа. Смотри сюда.";
					...
					if (CheckCharacterItem(pchar, "cannabis7") && CheckAttribute(pchar, "questTemp.Mangarosa") && !CheckAttribute(npchar, "quest.mangarosa"))
					{
						...
						{
							link.l9 = "Погоди. Сегодня у меня есть кое-что на продажу. Это растение, называется мангароса. Я слыхал"+GetSexPhrase("","а")+", вы таким интересуетесь...";
				...
				case 1: // не торгует
					dialog.text = "Нет, "+GetSexPhrase("белый брат","белая сестра")+". "+npchar.name+" ничем не торгует. "+npchar.name+" - воин.";
		...
		// большой жемчуг
		case "big_pearl":
			dialog.text = "Сколько слёз ты хотеть купить?";
			link.l1 = "";
			Link.l1.edit = 4;
		...
		case "big_pearl_qty":
			...
			if (iQty < 1)
			{
				dialog.text = "Ты пошутить, "+GetSexPhrase("бледнолицый","белая скво")+"? Тебе не нужны слёзы богов? Тогда прощай.";
				link.l1 = "Гм...";
				link.l1.go = "exit";
				break;
			}
		...
		case "small_pearl_qty":
			...
			if (iQty < 1)
			{
				dialog.text = "Ты пошутить, бледнолиый? Тебе не нужны слёзы богов? Тогда прощай.";
				link.l1 = "Гм...";
				link.l1.go = "exit";
				break;
			}
		...
		// мангароса
		case "mangarosa":
			dialog.text = "Мангароса? Я хотеть на неё глянуть.";
		...
		case "mangarosa_1":
			...
			link.l1 = "Послушай, краснокожий брат, я бы хотел"+GetSexPhrase("","а")+" побольше узнать об этом растении. Зачем оно так вам нужно, не расскажешь?";
		...
		case "CitizenNotBlade":
			dialog.text = NPCharSexPhrase(NPChar, "Не искушать судьбу, "+GetSexPhrase("бледнолицый","белая скво")+"! Убери свой сабля!", "Знаете, я, как гражданка этого города, прошу вас не ходить у нас с обнажённым клинком.");
