...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First_officer":
			...
			{
				dialog.text = "Мисс МакАртур? Удивлён вас увидеть. Мы вроде никого из вашей команды или команды капитана Свенсона не арестовывали.";
				link.l1 = "Я просто так, сэр. Нельзя? Вроде с заключёнными не шушукалась без вашего ведома.";
			...
			{
				dialog.text = "А-а, Шарль де Мор... Вас следовало бы арестовать и отправить вслед за вашим офицером, но... случившееся в банке решено признать досадным недоразумением.";
				link.l1 = "Такое начало не может не радовать, мистер "+GetFullName(NPChar)+". Я мог бы всё объяснить."; 
			...
			{
				dialog.text = "Приветствую вас, капитан. Что вас привело сюда?";
			}
			// вице-адмирал
			...
			{
				dialog.text = "Приветствую вас, господин вице-адмирал! Чем я могу вам помочь?";
			}
			// генерал-губернатор
			if(CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
			{
				dialog.text = "Ваше превосходительство, генерал-губернатор! Есть какие-либо приказания?";
			}
			// <-- legendary edition
			...
			{
				link.l13 = "Хотел"+GetSexPhrase("","а")+" поговорить с вами об одном человеке по имени " + PChar.GenQuest.JusticeOnSale.SmugglerName + ". Если не ошибаюсь - он ваш заключённый."; // belamour gen
			...
			{
				link.l14 = "Здравствуйте, офицер. Насколько я знаю, некоторое время назад на улицах задержали одного пьяного гражданина, пытавшегося подраться со стражником."; 
				link.l14.go = "NightAdventure_CitizenHomie";
			}
			
			if(CheckAttribute(pchar,"GenQuest.NightAdventureToPrisoner"))
			{
				link.l14 = "Офицер, я поговорил с часовым, и он отозвал свои претензии. Вот расписка от него."; 
		...
		case "Marginpassenger_2":
			...
			link.l1 = "Именно так. За выкупом нужно обратиться в "+XI_ConvertString("Colony"+pchar.GenQuest.Marginpassenger.Targetcity)+", к некому человеку по имени "+pchar.GenQuest.Marginpassenger.q2Name+". Я заплатил"+GetSexPhrase("","а")+" ему за эту информацию, но, конечно, даже и не собирал"+GetSexPhrase("ся","ась")+" осуществлять похищение.";
		...
		case "Marginpassenger_4":
			...
			link.l1 = "Хм... Да, собственно, не за что... Рад"+GetSexPhrase("","а")+" был"+GetSexPhrase("","а")+" помочь. До свидания!";
        ...
        case "ForGoodMove":
			...
			if(CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
			{
				dialog.text = "Конечно, вы можете всё осмотреть - у меня здесь полный порядок!";
				link.l1 = "Ну, вот и проверим, лейтенант.";
				link.l1.go = "exit";
				pchar.questTemp.jailCanMove = true;
				break;
			}
			...
			if(isMainCharacterPatented() && sti(Items[sti(pchar.EquipedPatentId)].TitulCur) > 4 && npchar.nation == sti(Items[sti(pchar.EquipedPatentId)].Nation))
			{
				dialog.text = "Разумеется, Ваше Превосходительство. Можете проходить.";
				link.l1 = "Благодарю, офицер.";
				link.l1.go = "exit";
				pchar.questTemp.jailCanMove = true;
				break;
			}
			...
			{
				dialog.text = "Ну что же, не вижу причин вам отказывать - вы всё-таки капитан корабля на службе " + NationNameGenitive(sti(npchar.nation)) + ". Проходите."; 
        ...
        case "KnowAboutPrisoner":
			...
			if(CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
			{
				link.l1 = "Понятно... Отдайте его мне, офицер, а дальше я уже сам решу судьбу этого заключённого.";
		...
		case "KnowAboutPrisoner_gengov":
			dialog.text = "Конечно, Ваше Превосходительство, если вы так желаете, то забирайте этого несчастного. Только будьте осторожны: он всё-таки преступник...";
			link.l1 = "Вот и отлично. Не переживай, лейтенант, я знаю, что делаю.";
			link.l1.go = "exit";
			pchar.questTemp.jailCanMove = true;
			DeleteAttribute(pchar, "questTemp.jailCanMove.ownerPrison");
			sld = characterFromId(pchar.questTemp.jailCanMove.prisonerId)
		...
		case "KnowAboutPrisoner_admiral":
			dialog.text = "Ну... Не знаю даже... Вообще я не могу просто так отпускать заключённых. Но вы всё-таки вице-адмирал, так что, наверное, имеете право забрать этого преступника.";
			link.l1 = "Вот и отлично. Не переживай, лейтенант, я знаю, что делаю.";
			link.l1.go = "exit";
			pchar.questTemp.jailCanMove = true;
			DeleteAttribute(pchar, "questTemp.jailCanMove.ownerPrison");
        ...
        case "First_protector":
			...
			if(CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
			{
				dialog.text = "Генерал-губернатор, ваше сиятельство! Проходите, но будьте осторожны: от этих преступников можно ждать чего угодно...";
				link.l1 = "Не переживай, солдат, справлюсь в случае чего.";
				link.l1.go = "exit";
			...
			if(isMainCharacterPatented() && sti(Items[sti(pchar.EquipedPatentId)].TitulCur) > 4 && npchar.nation == sti(Items[sti(pchar.EquipedPatentId)].Nation))
			{
				dialog.text = "Приветствую, господин вице-адмирал! Я здесь несу службу: слежу, чтобы никто без разрешения не входил и не выходил. Но вы, разумеется, имеете право свободно здесь проходить.";
        ...
        case "Prisoner_4":
			...
			{			
				...
				if(!bAdmiral && !bGenGov)
				{
				link.l2 = "Пожалуй, стоит рискнуть... Предлагаю следующее: я могу перебить стражу в тюрьме и забрать тебя на свой корабль. Если всё получится, я хочу, чтобы ты не отходил от меня до тех пор, пока мы не доберёмся до грота на " + XI_ConvertString(pchar.questTemp.jailCanMove.islandId + "Voc") + ". Идёт?"; // belamour gen
			...
			{
				link.l2 = "Ну, хорошо, поверю тебе и открою камеру. Я хочу, чтобы ты не отходил от меня до тех пор, пока мы не доберёмся до грота на " + XI_ConvertString(pchar.questTemp.jailCanMove.islandId + "Voc") + ". Идёт?"; // belamour gen
        ...
        case "ToPrisonHead_canMove_1":
			...
			link.l1 = "Подведём итоги. Я хочу, чтобы всё время, пока мы не доберёмся до грота на " + XI_ConvertString(pchar.questTemp.jailCanMove.islandId + "Voc") + ", ты не отходил от меня ни на шаг. На всякий случай. Ты согласен?"; // belamour gen
		...
		case "JusticeOnSale_3":
			...
			link.l1 = "Я слышал"+GetSexPhrase("","а")+", что обвинение бездоказательно и улик у вас нет, разве не так? Нельзя держать за решёткой несправедливо обвиненного человека.";
		...
		case "NightAdventure_CitizenHomie":
			dialog.text = "Да, был один такой буйный. Нахамил часовому, а затем вообще драться полез. Ничего: посидит несколько дней за решёткой - протрезвеет. А вам-то что за дело до него? Вы вроде бы капитан, а этот пьяница на матроса похож не был.";
			link.l1 = "Да, так и есть, я капитан. Дело в том, что этот пьяница - мой давний знакомый. Только не говорите об этом никому, пожалуйста. Если я правильно понял"+GetSexPhrase("","а")+", он даже за оружие не хватался. Может быть вы выпустите его на этот раз по моей просьбе?";
		...
		case "NightAdventure_CitizenHomie_1":
			...
			{
				dialog.text = "Да уж, хорош знакомый. Не переживайте, никому не скажу. На самом деле он даже ударить-то толком не смог, просто попытка была - за это и посадили. Раз вы просите, из уважения к вам я его выпущу. Только предупредите его, чтобы в следующий раз держал себя в руках. А лучше вообще пусть пьёт поменьше.";
				link.l1 = "Благодарю вас, офицер.";
			...
			{
				dialog.text = "Да уж, хорош знакомый. Не переживайте, никому не скажу. Только вот дело в том, что он успел дать в морду часовому, прежде чем его повязали. Так что выпустить не могу. Если только, конечно, часовой не согласится отозвать свои претензии. Можете поговорить с ним сами, он сегодня в патруле по городу.";
				link.l1 = "Нет, боюсь, если он ударил стражника, то тут я уже ничем помочь не смогу. Придётся ему посидеть пару деньков. Прошу прощения за беспокойство, офицер.";
				link.l1.go = "CitizenHomieEnd";
				link.l2 = "Хорошо, пойду попробую договориться с часовым.";
			...
			{
				dialog.text = "Да уж, хорош знакомый. Не переживайте, никому не скажу. Только вот дело в том, что нападение на военнослужащего, находящегося при несении службы - это преступление. Я могу пойти вам навстречу и выпустить его, но только под залог.";
				link.l1 = "И сколько мне нужно за него внести?";
		...
		case "NightAdventure_HomieToPrisoner":
			...
			pchar.quest.NightAdventure_JungleExit.over = "yes";
			SetFunctionLocationCondition("NightAdventure_CitizenHomieOK", npchar.City+"_town", false);
			dialog.text = "*читает* Так... так... Ну, раз у часового претензий нет, то и у меня тоже. Можете забирать вашего знакомого. Только предупредите его, чтобы в следующий раз держал себя в руках. А лучше вообще пусть пьёт поменьше.";
			link.l1 = "Благодарю вас, офицер.";
			link.l1.go = "exit";
		break;
		
		case "NightAdventure_CitizenHomieOfficer":
			dialog.text = "Ну, с учётом того, что он за оружие не хватался, то всего 1500 песо.";
			link.l1 = "Боюсь, столько я отдать не готов. Ну, учитывая, что он тут всего на несколько дней, думаю, ему это не повредит. Может быть будет меньше пить.";
			link.l1.go = "NightAdventure_CitizenHomieEnd";
			if(sti(pchar.money) >= 1500)
			{
				link.l2 = "Хорошо, я готов внести залог.";
		...
		case "NightAdventure_CitizenHomieOfficer_1":
			...
			pchar.quest.NightAdventure_JungleExit.over = "yes";
			SetFunctionLocationCondition("NightAdventure_CitizenHomieOK", npchar.City+"_town", false);
			dialog.text = "Можете забирать вашего знакомого. Только предупредите его, чтобы в следующий раз держал себя в руках. А лучше вообще пусть пьёт поменьше.";
			link.l1 = "Благодарю вас, офицер.";
			link.l1.go = "exit";
		break;
		// <-- ночной приключенец
		...
		case "PZ_BasTer_ComendantDialog":
			...
			{
				dialog.text = "Спасибо, что согласились прийти, а не стали устраивать бойню. Хоть кто-то в последнее время ведёт себя разумно.";
				link.l1 = "Так чем обязан такому вниманию, месье офицер? Это как-то связано с тем, что я разыскиваю одного китайца?";
				link.l1.go = "PZ_BasTer_ComendantDialog_Opozdal_1";
			}
			else
			{
				dialog.text = "Хорошо, что пришли к нам по своей воле - в противном случае никто бы не стал пытаться именно арестовать и допрашивать вас. Хватает нам буйных в последнее время.";
				link.l1 = "Ну, как видите, я не из буйных. Однако... зачем я здесь, офицер? Я уже спрашивал ваших людей, но не получил ответа - это из-за того, что я ищу некоего китайца в городе?";
		...
		// <-- Путеводная звезда
		case "Helen_meeting":
			dialog.text = "Ну что вы! Проходите, пожалуйста. Вам всегда рады в наших застенках. Э-э, в хорошем смысле, само собой. Так что же вам понадобилось у нас?";
			link.l1 = "Да ничего особенного, осматриваю город, знаете ли. Вот и забр"+ GetSexPhrase("ёл","ела") +" сюда по случаю.";
			link.l1.go = "exit";
			link.l2 = "Хочу поговорить по делу.";
			link.l2.go = "quests";
