...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		if(CheckAttribute(pchar, "questTemp.HWIC.CanTake") && !CheckAttribute(pchar, "questTemp.HWIC.CanTake.Eng") && !CheckAttribute(npchar, "quest.HWICTake") && !CheckAttribute(pchar, "questTemp.HWIC.Holl") && !CheckAttribute(pchar, "questTemp.HWIC.Self"))
		{
			...
			link.l1 = "Здравствуйте. Я без конкретного дела зашёл, просто поприветствовать и поговорить...";
			link.l1.go = "exit";
			link.l2 = "Послушайте, Чарли, ходят слухи, что вы каким-то образом связаны с английским военным флотом. У меня отличный корабль и обученная команда. Я бы хотел предложить себя и своё судно для работы на благо Англии. Причём настоящей работы, достойной военного моряка. Не могли бы вы что-нибудь посоветовать, или кому-нибудь рекомендовать, или ещё как-то помочь в этом вопросе?";
		...
		break;
			...
			dialog.text = "Доброго дня, "+GetSexPhrase("мистер","мисс")+", книппель мне в корму! Каким попутным ветром вас сюда занесло?";
			link.l1 = "Здравствуйте. Я без конкретного дела заш"+GetSexPhrase("ёл","ла")+", просто поприветствовать и поговорить...";
			link.l1.go = "exit";
...
//-------------------------------------------за Голландию--------------------------------------------------	
		...
		case "Dominica_1":
			...
			link.l1 = "Меня спасло только то, что моё судно способно развивать приличную скорость в бейдевинд. В последний раз, чтобы убежать, мне пришлось выбросить за борт весь товар... вы представляете, какие это убытки?";
		...
		case "Dominica_5":
			dialog.text = "Вы хорошо сделали, мистер, что пришли ко мне. Я сообщу обо всём услышанном куда следует, книппель мне в печень! Скоро вы снова сможете торговать с Мартиникой.";
			link.l1 = "Ваши бы слова да Богу в уши! Спасибо, что выслушали меня, Чарли. Надеюсь на лучшее. До свидания, книппель вам в корму!";
...
//----------------------------------------------за Англию--------------------------------------------------	
		case "Knippel_check"://начинаем проверять нашего ГГ по всем статьям
			...
			{
				// belamour legendary edition покажем критерии  -->
				notification("Недостаточно развит навык (30)", SKILL_SAILING);
				//<--
			...
			dialog.text = "Хм... Вы очень вовремя обратились, капитан, книппель мне в корму! А то Ричард даже и не знал, кому поручить это дело, думал, что придётся отказаться... Попробую озадачить данным вопросом вас. Если хорошо себя покажете - я порекомендую вас одному очень хорошему человеку. Теперь слушайте, что надо сделать.";
		...
		case "HollConvoy_Check"://начинаем проверять
			...
			if (iHal > 0)
			{
				...
				if (amount <= 0)
				{
					...
					link.l1 = "Спасибо, книппель мне... то есть... приятно слышать лестную оценку своих действий.";
					link.l1.go = "HollConvoy_complete";
				}
				else
				{
					dialog.text = "Ну, отчёта не нужно - после досмотра вашего судна в порту мне всё уже рассказали... Капитан, вы привезли не весь груз, книппель мне в корму! Я достоверно знал, сколько серебра везли на этом галеоне. Это задание было не только проверкой ваших боевых качеств, но и вашей честности. Испытания на жадность вы не прошли. Я не желаю иметь с вами дел - покиньте мой дом!";
					link.l1 = "Ну и чёрт с тобой, чистоплюй!";
		...
		case "HollConvoy_complete":
			dialog.text = "Зайдите ко мне завтра - я познакомлю вас с моим патроном. Думаю, он сможет предложить работу, достойную вас. А за выполненное задание предусмотрена награда - вы её завтра же и получите от моего командира. До встречи!";
		...
		case "InFleetwoodHouse":
			dialog.text = "Ричард у себя в комнате, наверху. Поднимайтесь к нему, он вас ждёт, а я побуду здесь.";
			link.l1 = "Я понял, Чарли. Уже иду.";
			link.l1.go = "exit";	
		break;
		
		case "Knippel_ToOfficer":
			dialog.text = "Капитан, тут такое дело... Ричард под хорошим присмотром и в безопасности. А я хочу помочь вам отомстить за его ранение. Я предлагаю вам свои услуги в качестве офицера. Махать шашкой я не горазд - годы, понимаете ли, но наводить орудия ещё не разучился.";
		...
		case "MC_GoAway":
			...
			{
				dialog.text = "Капитан, вы полный олух, книппель мне в корму! Вам же ясно было сказано - нужно сыграть роль купца на 'Валькирии', а вы здесь что устроили? Вы спугнули пирата своими кораблями! Надо было идти в одиночку, и только на 'Валькирии'! Он теперь в жизни на эту уловку не попадётся\nНаша команда не желает больше вам ни подчиняться, ни вообще видеть вас. Мы уверены - сэр Флитвуд одобрит наше решение.";
				link.l1 = "Да и чёрт с тобой и твоим хозяином!";
				link.l1.go = "exit";
			}
		...
		case "OnCuracao":
			...
			link.l1 = "Приветствую, Чарли. Деньги у тебя? Давай их мне, и нам нужно выдвигаться в Виллемстад за Абигайль. Ты пойдёшь со мной?";
			link.l1.go = "OnCuracao_1";	
		break;
		
		case "OnCuracao_1":
			dialog.text = "Капитан, тут такое дело... Уже никуда выдвигаться не нужно.";
			link.l1 = "Ты это о чём? Что за вздор?!";
		...
		case "OnCuracao_2":
			...
			link.l1 = "Да что же это за чертовщина такая в последнее время творится? Опять неприятный сюрприз? Давай сюда своё письмо!";
		...
		case "OnCuracao_4":
			dialog.text = "Прочли, капитан? Мне очень стыдно за Ричарда и за его поступок, книппель ему в... ах, ладно. Роденбург пришёл в ярость, когда узнал, с кем сбежала Абигайль. На Кюрасао усилены патрули, в море крейсируют военные корабли. Вы не встретили их? Нам надо убираться отсюда, и побыстрее\nИ, капитан: вам не нужен толковый канонир? Мне много не надо: кусок сухаря, ломоть солонины и кружка воды - что ещё нужно старому артиллеристу?";
...
//------------------------------------------------против всех-----------------------------------------------
		case "Knippel_abordage":
			...
			link.l1 = "Голландское? Ха-ха! Не смеши меня, 'голландец'. Я захватил твою бригантину, чтобы взять тебя в плен, дорогой Чарли. Добро пожаловать в трюм. Побеседуем, куда ты шёл, и зачем...";
		...
		case "Knippel_prisoner_1":
			...
			link.l1 = "Это уже не твоё дело, Чарли. Ты мне больше не нужен. Встань.";
		...
		case "Knippel_officer":
			...
			{
				Link.l4 = "Чарли, я собираюсь отправиться в старый индейский город Тайясаль. Не буду скрывать: это крайне опасное путешествие, и более того - необычное: через телепортационный истукан. Ты... пойдёшь со мной?";
				Link.l4.go = "tieyasal";
			}
			...
			{
			    Link.l11 = "Чарли, дай мне полный отчёт о корабле.";
		...
		/////////////////////////// ответы для казначея ///////////////////////////////////
		case "QMASTER_1":
			dialog.Text = "Ну... Орудия вроде в порядке, те, что взрываются, меняем, порох сухой. А какой отчёт я ещё дам? Я ведь просто старый канонир, а не казначей, капитан. Но зато лучший канонир, книппель мне в рею!";
			Link.l1 = "Да, тут не поспоришь, Чарли, канонир ты отменный. А казначея я найду тогда, не переживай.";
			Link.l1.go = "exit";
		break;	

		case "QMASTER_2":
			dialog.text = "Чтобы старина Чарли сменил орудийную палубу на споры с купцами в магазинах? Капитан, я привык корабли на дно отправлять, а не за каждый песо торговаться. Да и не умею я это, надуют меня в первой же колонии... Прости, капитан.";
			link.l1 = "Ладно, Чарли, ты прав. Нужно найти казначея, а не на тебя перекладывать эти обязанности.";
			link.l1.go = "exit";
		break;
	...
	// на Тайясаль
		case "tieyasal":
			dialog.text = "Я чертовски привязался к вам за время службы, "+pchar.name+", да и не в моих правилах бросать своего капитана. Я с вами!";
