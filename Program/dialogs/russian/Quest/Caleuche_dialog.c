void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Tuttuat_1":
			...
			link.l1 = "Я слышал, что ты прибыл сюда из далёких мест на юге. У меня есть индейский амулет, и один знающий человек сказал, что ты сможешь помочь мне выяснить, как и для чего можно его применить.";
		...
		case "Tuttuat_4":
			dialog.text = "Белый человек удивлять Туттуатхапак свой знание. Да, моя пожрать разум тот шелудивый пёс. Он и его гнусный люди получить по заслуги.";
		...
		case "Tuttuat_10":
			if (CheckAttribute(pchar, "questTemp.Caleuche") && pchar.questTemp.Caleuche == "hunting" && Caleuche_CheckAmulet())
			{
				...
				link.l1 = "Уже нашёл, великий шаман. Вот, смотри.";
		...
		case "Tuttuat_11":
			...
			link.l1 = "Хуже, великий шаман. Едва я вышел в море, как на меня напал корабль, битком набитый живыми скелетами! Их капитан неуязвим. Он отнял у меня амулет, чуть не убив при этом. А затем они едва не утопили моё судно.";
		...
		case "Tuttuat_20":
			...
			if (CheckAttribute(pchar, "questTemp.Caleuche.Skul"))
			{
				...
				{
					link.l1 = "Нефритовый череп с зелёными камнями в глазницах? Он был у меня в руках, ты можешь представить себе? Я сам передал его Соломону. Но, он уехал в Европу с дочерью, и череп, похоже, забрал с собой.";
				}
				else
				{
		...
		case "Tuttuat_21_1":
			...
			link.l1 = "Так, я правильно понимаю, что мне снова предстоит найти этот череп? И что мне с ним потом делать?";
		...
		case "Tuttuat_22":
			dialog.text = "Сначала твоя должен найти два амулет, про который мы говорить. Без них ничего не суметь сделать. Амулет дать тебе сила противостоять люди-ягуары.";
		...
		case "Tuttuat_24":
			dialog.text = "Чему твоя удивляться, белый капитан? Юм Симиль - бог смерти, нефритовый череп Юм Симиль давать власть над живые и мёртвые. С амулет твоя суметь пройти храм, оставить там череп, потом найти Калеуче, победить его капитан и забрать амулет, который он у тебя отобрать.";
		...
		case "Tuttuat_36":
			...
			Log_Info("Вы отдали амулет");
			PlaySound("interface\important_item.wav");
			dialog.text = "(смотрит) Да, это он. Третий амулет чавинцы. Твоя молодец. Твоя найти все нужный амулет, чтобы пройти через чавинави, слуги Юм Симиль, мёртвый воины-ягуары.";
		...
		case "Tuttuat_39":
			...
			if (CheckAttribute(pchar, "questTemp.Caleuche.Skul"))
			{
				if (CheckAttribute(pchar, "questTemp.Caleuche.SolomonSkul")) sTemp = "Начну с Кюрасао, где я передал его в руки Соломона. Возможно, мне удастся найти хоть какую-то зацепку, чтобы узнать о судьбе черепа.";
				else sTemp = "Начну с посещения Кюрасао - именно там я продал нефритовый череп Жоакиму Мерриману.";
			}
		...
		// капитан Калеуче
		case "CaleucheCap":
			PlaySound("VOICE\Russian\LE\Baltazar_de_Kortes\Baltazar_01.wav");
		...
		case "CaleucheCap_3":
			PlaySound("VOICE\Russian\LE\Baltazar_de_Kortes\Baltazar_02.wav");
		...
		case "on_coast_1":
			dialog.text = "Когда мы зашли в капитанскую каюту, вы лежали на полу без движения. Ребята подхватили вас и потащили на наш корабль, и вовремя, потому что мертвецы, что валялись на палубе, начали подниматься и снова взялись за сабли\nПресвятая Дева и все святые угодники! Мы быстро обрубили лини и отчалили от их корабля, но далеко уйти не успели: костлявые всадили в нас залп бортовой батареи, и наше судно в один миг превратилось в дырявый и неуправляемый кусок дерева. А затем они подняли паруса и в миг скрылись за горизонтом\nКорабль наш сел на мель, а все, кто выжил, добрались на лодке до берега. Никто не знал, переживёт ли корабль эту бурю, так что мы прихватили ваш капитанский рундук из каюты на всякий случай. Там ведь много чего важного, да?";
		...
		case "on_coast_4":
			dialog.text = "Не сомневаюсь... Это был 'Летучий Голландец', кэп? Почему они на нас напали? И почему сразу не сделали ни одного выстрела, а после абордажа чуть не потопили одним залпом?";
		...
		case "Beliz_forestbandos_10":
			...
			link.l1 = "Что ты меня стращаешь, я всё понимаю, не маленький. Буду нем, как рыба. А через три дня жди меня - я обожаю лёгкие деньги!";
			link.l1.go = "Beliz_forestbandos_11";
		break;
		
		case "Beliz_forestbandos_11":
			dialog.text = "Хорошо, фраерок. А теперь проваливай, пока стража не явилась. Да и мы канаем отсюда.";
		...
		case "monk_caleuche_5":
			sld = characterFromId("Villemstad_Priest");
			dialog.text = "Да, минхер. Днём там всё тихо. Очень вас прошу. Отец "+sld.name+" отблагодарит и вознаградит вас за службу. Как только побываете ночью в этом вертепе - расскажите ему, что вы там обнаружили. Уверяю вас, наши опасения имеют под собой почву.";
		...
		case "monk_caleuche_7":
			...
			if (CheckAttribute(pchar, "questTemp.Caleuche.Skul"))
			{
				if (CheckAttribute(pchar, "questTemp.Caleuche.SolomonSkul")) sTemp = "посетить дом Жоакима Мерримана, в котором, по его словам, по ночам творятся странные вещи. Вспоминая тот особенный блеск в глазах Мерримана, когда тот смотрел на нефритовый череп, мне становится жутковато. Похоже, старый еврей не устоял перед суммой в полмиллиона песо. Как бы там ни было, но как-то мне совсем не улыбается идти ночью в его бывшее обиталище.";
				else sTemp = "посетить дом моего старого знакомца Жоакима Мерримана, в котором, по его словам, по ночам творятся странные вещи. Вспоминая тот особенный блеск в глазах Мерримана, когда тот смотрел на нефритовый череп, мне становится жутковато. Как-то мне совсем не улыбается идти ночью в его бывшее обиталище.";
			}
			else sTemp = "посетить дом c обратной стороны от банка местного ростовщика, в котором снимал комнату некий Жоаким Мерриман. Судя по всему, это и есть тот самый белый колдун, о котором сообщил Туттуатхапак. По словам монаха, по ночам в доме творятся странные вещи. Как-то мне совсем не улыбается идти ночью в бывшее обиталище колдуна.";
			AddQuestUserData("Caleuche", "sText", sTemp);
		...
		case "cryptguard_2":
			...
			link.l1 = "Хорошо, я понял"+GetSexPhrase("","а")+". Всего доброго.";
		...
		// чавинави-вождь
		case "Chavinavi":
			PlaySound("VOICE\Russian\LE\Chavinavi\Chavinavi_01.wav");
		...
		case "question5_r":
			...
			{
				PlaySound("VOICE\Russian\LE\Chavinavi\Chavinavi_02.wav");
		...
		case "Chavinavi_5":
			PlaySound("VOICE\Russian\LE\Chavinavi\Chavinavi_03.wav");
