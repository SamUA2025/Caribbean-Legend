...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "AxelHelper_1":
			dialog.text = "Аксель занят. Он сейчас у Дональда Гринспи, и пробудет у него до ночи. Так что если ты хотел что-то купить - отложи своё желание до завтрашнего утра. А я здесь присматриваю за магазином, чтобы никто ничего не украл, так что смотри...";
		...
		case "agnes_3":
			dialog.text = "Ох... видать, Господь услышал мои молитвы и послал мне утешение на старости лет! Вы не представляете, как я переживала, когда он исчез! Ведь это я, я была во всём виновата! Где вы его нашли? Он ведь не простой ребёнок, он... ну, вы, наверное, понимаете, о чём я.";
		...
		case "agnes_5":
			...
			link.l1 = "Оле все эти годы собирал жемчужины. Он называет их белыми бусинками. Собирал их для вас, чтобы вернуть. И, наконец, собрал, и попросил отвезти его домой. Никаких примет, кроме того, что его мама, то есть вы, живёте в доме с красной крышей - не было. Но всё-таки мне удалось найти вас.";
			link.l1.go = "agnes_6";
		break;
		...
		case "agnes_8":
			DialogExit();
			SetLaunchFrameFormParam("Прошёл час...", "", 0, 5);
			LaunchFrameForm();
