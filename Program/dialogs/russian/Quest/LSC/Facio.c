	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				link.l1 = TimeGreeting()+". Моё имя - "+GetFullName(pchar)+". Вы правы, я действительно здесь недавно. Простите за любопытство, а какие такие проблемы вы решаете?";
				link.l1.go = "meeting";
				npchar.quest.meeting = "1";
			}
			else
			{
				dialog.text = "А, это снова вы, дорогой "+pchar.name+"! Что привело вас в моё скромное жилище?";
				...
				{
					link.l10 = "Фацио, у меня возник серьёзный конфликт с одним из кланов. Мне нужно как-то решить эту проблему.";
				...
				link.l3 = "Ничего особенного, Джузеппе. Я просто зашёл поздороваться с вами.";
				link.l3.go = "exit";
			}
		...
		case "parol_rvd":
			dialog.text = "Отправляетесь в гости к Чёрному Эдди, я угадал?";
		...
		case "parol":
			...
			{
				dialog.text = "А как же! Но сначала позвольте внести краткие пояснения. Пароли кланов представляют собой одно определённое слово и меняются четыре раза в месяц, то есть практически еженедельно. Смена паролей происходит каждого седьмого, четырнадцатого, двадцать первого и двадцать восьмого числа\nТак что следите за текущей датой, чтобы не попасть впросак. Ну и, как вы понимаете, пароль я вам предоставлю не бесплатно. Стоимость одной услуги - пятьдесят дублонов. За пароли я беру только дублоны, имейте это в виду.";
		...
		case "parol_1":
			dialog.text = "Ну-ну, мой мальчик, не стоит так горячиться. Не жадничайте, поверьте: я отлично знаю цену своей информации. Ведь вы на корабли "+pchar.questTemp.LSC.parol+" не на прогулку собрались, верно? Верно. Значит, должны были быть готовы к тому, что вам придётся расстаться с некоторой суммой денег... Так вы готовы заплатить пятьдесят дублонов?";
			if (GetCharacterItem(pchar, "gold_dublon") >= 50)
			{
				link.l1 = "Да. Вот, держите ваши монеты.";
				link.l1.go = "parol_pay";
		...
		case "adolf_1":
			dialog.text = "Всегда пожалуйста, мой дорогой. Но не ищите Адольфо у себя в каюте раньше вечера - он очень, очень занятой, что вы, всё бегает и бегает по делам... Да, а вы заходите ко мне почаще, ещё поболтаем!";
		...
		case "whiskey":
			switch (sti(npchar.quest.poisonnode))
			{
				case 1: // у себя
					...
					link.l1 = "Сейчас я позволю себе ещё большее... отравитель!";
					link.l1.go = "whiskey_1";
				break;
				
				case 2: // на Эве
					dialog.text = "Пощади! Клянусь, они заставили меня! Они приставили мне пистолет к...";
				...
				case 3: // у Акулы
					dialog.text = "Клянусь, они заставили меня! Я уже всё рассказал адмиралу! Я не мог им сопротивляться, они бы...";
					link.l1 = "Замолчи. Я это уже понял. Где последний ублюдок из их компании? Где Марчелло Циклоп?!";
					link.l1.go = "whiskey_3";
					SetFunctionTimerCondition("LSC_FacioReturn", 0, 0, 1, false); // вертаем на место Фацио через сутки
				break;
		...
		case "whiskey_1":
			...
			link.l1 = "Я всё знаю, негодяй. Ты действовал заодно с Чадом Каппером. Ты принёс бочонок с отравленным ромом адмиралу. Теперь молись, ничтожество...";
			link.l1.go = "whiskey_2";
		break;
		
		case "whiskey_2":
			dialog.text = "Стой! Остановись! Пощади! Клянусь, они заставили меня! Они приставили мне пистолет к...";
		...
		case "whiskey_3":
			...
			link.l1 = "Лжёшь, поганый мешок навоза! Я это вижу. Где Циклоп?";
			if (pchar.questTemp.LSC.Mary == "die") link.l1.go = "whiskey_10"; // ушёл на Флерон
			else link.l1.go = "whiskey_4";
		break;
		
		case "whiskey_4":
			dialog.text = "Он... он получил от Каппера какой-то приказ и ушёл. Я не знаю, где он!";
			link.l1 = "Опять лжёшь. Ты всё знаешь. Не трусь, малодушная тварь, Каппер мёртв и ничего не сделает тебе. Но если ты не скажешь, где Марчелло - я проткну тебя, как жука, прямо сейчас! Что приказал Циклопу Чад?";
			link.l1.go = "whiskey_5";
		break;
		
		case "whiskey_5":
			dialog.text = "Он отправил его убить... Красную Мэри.";
			link.l1 = "Что?! Говори немедленно, каким образом он планирует это сделать? К ней в каюту на 'Церес Смити' не попасть, у неё заперты все двери! Как он собирается убить её?!";
			link.l1.go = "whiskey_6";
		break;
		
		case "whiskey_6":
			dialog.text = "Он пройдёт через корму 'Церес Смити', в каюту Юргена, оружейника, когда тот отлучится. Потом спустится на самое дно трюма по верёвке - все трапы, ведущие туда, сломаны. Затем тихо пересечёт подтопленный трюм и выйдет прямо к каюте Мэри\nДальше ему нужно будет только дождаться, пока девушка повернется к нему спиной, или заснёт - и всё, дело сделано. Это всё сказал Чад. Клянусь, я больше ничего не знаю!";
			link.l1 = "Этого достаточно. Когда он ушёл? Когда, я спрашиваю, Циклоп ушёл на 'Церес Смити'?";
			link.l1.go = "whiskey_7";
		break;
		
		case "whiskey_7":
			dialog.text = "Он уже наверняка у Юргена... или спускается в трюм...";
			link.l1 = "Проклятие!! Вплавь вокруг острова я не успею! Говори пароль нарвалов для прохода на их корабли! Говори, а если солжёшь - я всё равно останусь в живых, но тогда тебе точно конец!";
		...
		case "whiskey_repeat":
			dialog.text = "Я уже всё сказал, что знал! Клянусь!";
			link.l1 = "...";
			link.l1.go = "exit";
		...
		case "conflict":
			...
			link.l4 = "Ни с каким. Я пошутил, Джузеппе, у меня всё в порядке.";
			link.l4.go = "exit";
		break;
		...
		case "narval":
			...
			link.l2 = "К сожалению, у меня нет сейчас такого количества дублонов. Но я обязательно принесу, и мы вернёмся к нашему разговору.";
			link.l2.go = "exit";
		break;
		...
		case "drink_1":
			...
			link.l1 = "Да, чирикали об этом пташки... Только я бы её назвал 'мордой в стол', эту вашу игру.";
			link.l1.go = "drink_2";
		break;
		...
		case "drink_3":
			dialog.text = "Да, и проигравший оплачивает всё выпитое. Согласны?";
		...
		case "drink_4":
			...
			PlaySound("interface\important_item.wav");
			dialog.text = "Отлично, мой драгоценнейший! Тогда встречаемся в девять вечера в таверне у милейшего Санчо и сыграем! Да, если вы вдруг передумаете, или паче чаяния вас отвлекут неотложные дела - то ваша ставка останется у меня, это одно из правил. Но вы ведь придёте, так, мой мальчик?";
		...
		case "drink_8":
			dialog.text = "Ну что, любезнейший, готовы к встрече со столом, хе-хе? Ром наш драгоценный Санчо уже принёс. Правила не надо оглашать? Они очень просты: пьём из одной бутылки кружку за кружкой до дна, пока кто-то из нас не отключится\nДа, разговаривать нам с вами, милейший, не запрещается, наоборот - приветствуется! Это же ведь так замечательно - поболтать с хорошим человеком! Ну, а если вам удастся выиграть - то свой приз получите у Санчо. Начинаем?";
