void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "MarchCap":
			...
			dialog.text = "Вот и вы, капитан. Теперь приступим к обсуждению подробностей моего предложения. Вы сразу поймёте, почему я не захотел об этом говорить в людном месте.";
		...
		case "MarchCap_1":
			...
			switch (rand(2))
			{
				...
				case 2: //одиночный пиратский кулсейлор у бухты, установка параметров
					...
					dialog.text = "Я получил достоверные сведения об одном пирате по кличке "+GetName( NAMETYPE_ORIG, pchar.GenQuest.MarchCap.PirateName, NAME_NOM)+". Так вот, этот головорез недавно хорошенько пощипал "+RandPhraseSimple(RandPhraseSimple("англичан","французов"), RandPhraseSimple("испанцев","голландцев"))+" и загрузил себе полный трюм "+GetGoodsNameAlt(sti(pchar.GenQuest.MarchCap.Goods))+". Но вот ведь незадача: в бою его "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.MarchCap.ShipType), "Name")+"Acc"))+" так сильно потрепали, что он со всем этим грузом был вынужден встать на ремонт\nСейчас он находится у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island.Shore+"Gen")+", это у "+XI_ConvertString(pchar.GenQuest.MarchCap.Island+"Gen")+", устанавливает новые мачты и латает пробоины в корпусе. По моим расчётам, у него на это уйдёт не больше, чем "+FindRussianDaysString(sti(pchar.GenQuest.MarchCap.DaysQty))+". Если постараемся - то сможем успеть к нему на свидание, и тогда весь груз "+GetGoodsNameAlt(sti(pchar.GenQuest.MarchCap.Goods))+" станет нашим\nСправиться в одиночку с этим пиратом мне не под силу - он весьма искушен в ведении морского боя, но вдвоём мы сумеем разделаться с ним, несмотря на весь его опыт. Ну как, вы согласны отправиться со мной в этот поход?";
					link.l1 = "Звучит заманчиво. Пожалуй, я соглас"+GetSexPhrase("ен","на")+"!";
					link.l1.go = "MarchCap_3_1";
					link.l2 = "Хм. Сдаётся мне, что эта добыча не только вам в одиночку, но и нам вдвоём не по зубам. Нет, я на такое не подпишусь. Прощайте, сударь, и даже не пытайтесь меня задерживать!";
					link.l2.go = "MarchCap_exit";
				break;
			}
		break;
		...
		case "MarchCap_1_2":
			...
			AddQuestUserData("MarchCap", "sSex", GetSexPhrase("","а"));
			AddQuestUserData("MarchCap", "sSex1", GetSexPhrase("ся","ась"));
		...
		case "MarchCap_TimeOver":
			dialog.text = "И зачем я только с вами связался, не подскажете, а, капитан? Из-за вас мы упустили такую богатую добычу! Лучше бы я пошёл один...";
			link.l1 = "Из-за меня? Да вы на себя и свою лоханку посмотрите! Что вы сделали для общего дела? Плелись, как краб по отмели? Я только и делал"+GetSexPhrase("","а")+", что ждал"+GetSexPhrase("","а")+", пока вы подтянетесь! Прощайте!";
		...
		case "MarchCap_AfterBattle":
			...
			if (iTemp < 50)
			{
				dialog.text = "Какой же вы неумёха, капитан! Мы ничего не захватили, вообще, ни-че-го!! А всё из-за вас! Это вы во всём виноваты, разрази вас гром!";
			...
			if (iTemp > 500 && iTemp < 1500)
			{
				...
				link.l1 = "Вы всё верно подсчитали, "+npchar.name+". У меня нет возражений. Шлюпки уже грузятся. Возможно, ещё когда-нибудь встретимся с вами...";
				link.l1.go = "MarchCap_Talk_exit";
			...
			link.l1 = "Вы всё верно подсчитали, "+npchar.name+". У меня нет возражений. Шлюпки уже грузятся. Может, ещё как-нибудь сходим в совместный рейд, а? Вы мне кажетесь вполне подходящим компаньоном.";
		...
		case "MarchCap_Deck_continue":
			dialog.text = "Всё возможно, капитан. Если подвернётся денежное дельце, с которым я не смогу справиться сам - я постараюсь найти вас. Вы мне тоже кажетесь "+GetSexPhrase("надёжным боевым товарищем","надёжной боевой подругой")+".";
		...
		case "MarchCap_DieHard":
			dialog.text = "Вы струсили, чёрт бы вас побрал! Вы попросту сбежали с поля боя!";
			link.l1 = "А что, по-вашему, я долж"+GetSexPhrase("ен","на")+" был"+GetSexPhrase("","а")+" дать этому пирату потопить моё судно? Нет уж, лучше быть "+GetSexPhrase("живым трусом","живой трусихой")+", чем "+GetSexPhrase("мёртвым идиотом","мёртвой идиоткой")+"!";
		...
		case "MarchCap_DieHard_1":
			...
			link.l1 = "Идите читать нотации в другом месте, сударь! Мы чуть было не погибли, а вы тут устраиваете разглагольствования! Вас погубит жадность, попомните моё слово! А теперь попрошу вас покинуть мой корабль - я отправляюсь на ремонт.";
			link.l1.go = "MarchCap_Talk_exit";
		break;
		...
		case "MarchCap_AfterBattleGold":
			...
			{
				dialog.text = "Неплохо сработали, капитан! Как я вам и говорил - вдвоём мы справились с этим пиратом! Добыча, как я вижу, составила "+FindRussianQtyString(iQty)+" товара. Моя доля - "+FindRussianQtyString(sti(pchar.GenQuest.MarchCap.CapPart))+".";
				link.l1 = "Вы всё верно подсчитали, "+npchar.name+". У меня нет возражений. Шлюпки уже грузятся. Возможно, ещё когда-нибудь встретимся с вами...";
				link.l1.go = "MarchCap_Talk_exit";
				pchar.GenQuest.MarchCap = "good";
				break;
