...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (CheckAttribute(pchar, "questTemp.HelenDrinking.Dialogs.Svenson")) {
				if (pchar.questTemp.HelenDrinking.Result == "won") {
					dialog.text = "А, Шарль. Спасибо, что зашёл. Просто хотел поблагодарить тебя за то, что с Элен время провёл. Девочке надо было залечить раны, да и ты тоже, я слышал, недурно развеялся, ха-ха! Чёрт побери, даже завидно!";
					link.l1 = "Ты не в обиде? За бочонок и... за прочее?";
					link.l1.go = "after_drinking";
				} else {
					dialog.text = "А, Шарль. Спасибо, что зашёл. Просто хотел поблагодарить тебя за то, что с Элен время провёл. Девочке надо было залечить раны, да и ты тоже, я слышал, недурно развеялся, ха-ха! Удачи тебе в Картахене!";
					link.l1 = "Я тоже рад, что так вчера все обернулось. До встречи, Ян.";
					link.l1.go = "exit";
				}
			...
			if(CheckAttribute(pchar, "questTemp.Saga"))
			{
				...
				if(pchar.questTemp.Saga == "baronsquests") // старт трёх квестов баронов
				{
				...
				if(pchar.questTemp.Saga == "calendar") // квесты баронов завершены
				{
					...
					if (CheckAttribute(pchar, "questTemp.Saga.Late"))
					{
						...
						link.l1 = "У меня просто нет слов... Столько сил было потрачено - и всё впустую. Неужели ничего нельзя сделать?";
						link.l1.go = "saga_l2";
					}
					else
					{
				...
				else sTemp = "Что-то ещё, "+pchar.name+"?";
				dialog.text = sTemp;
			...
			else
			{
				...
				if(startHeroType == 4)
				{
					dialog.text = "Элен, здравствуй, дорогая. Какие у тебя новости? Ты что-то хотела?";
					link.l1 = "Здравствуй, Ян! Всё по-прежнему, просто зашла проведать.";
					link.l1.go = "exit";
					NextDiag.TempNode = "First time";
					break;
				}
		...
		case "saga_3":
			...
			link.l1 = "Джекман разыскивает некую Глэдис Чандлер и её дочь. После общения с Хиггинсом я начинаю думать, что разыскивает он Румбу, и что Элен не родная дочь Глэдис, и что до замужества с Шоном МакАртуром Глэдис носила фамилию Чандлер.";
			link.l1.go = "saga_4";
		break;
		...
		case "saga_13":
			dialog.text = "";
			link.l1 = "Элен они выдали за свою дочь и воспитывали, как родную. Генри потерял их след, лёг на дно, опасаясь мести Джекмана, и прожил в страхе все двадцать лет. По всему выходит, Джекман ищет дочь своего капитана, а вот зачем, пока непонятно.";
			link.l1.go = "saga_14";
		break;
		
		case "saga_14":
			dialog.text = "Возможно он выполняет чей-то приказ...";
			link.l1 = "Не думаю. Кто может ему приказывать? Бучер мёртв. Его казнь подтверждена официальными бумагами, её видели десятки, если не сотни свидетелей.";
			link.l1.go = "saga_15";
		break;
		
		case "saga_15":
			dialog.text = "Возможно, дело тут не в отце Элен, а в её настоящей матери. Я уже догадываюсь, кто она, но боюсь в это поверить до конца. Но какова Глэдис! Столько лет хранить тайну и не открыться даже мужу! Шон МакАртур обязательно рассказал бы мне про это. У нас не было тайн друг от друга.";
		...
		case "saga_17":
			...
			link.l1 = "Мне отдала его Глэдис, сказав, что это 'наследство матери Элен'. Её имени она не знает. Сдаётся мне, что это не простой лоскут бумаги.";
			link.l1.go = "saga_18";
		break;
		...
		case "shark_hunt":
			dialog.text = "Один чёрт знает, что у него на уме. Иногда он производит впечатление законченного придурка, но тем не менее, сдаётся мне, что это всего лишь хитрая игра. К тому же его люди готовы за своего обожаемого Пастора в огонь и в воду\nУ них там на Кубе что-то вроде протестантской секты супротив папистов под боком у испанской инквизиции. Так что не стоит там вступать в теософские диспуты, никто не оценит.";
		...
		case "baron_return_3":
			dialog.text = "У Натана есть жена. Зовут её Данни Хоук. Та ещё штучка! Вот она вряд ли смирилась с потерей своего капитана, тем более что с самого начала что-то подозревала и отговаривала Хоука от десанта. Помню, они тогда крупно разругались на этот счёт, но это у них было делом обычным.";
		...
		case "return_LSC_1":
			...
			link.l1 = "(пьёт) Эх, вот так намного лучше... В общем, Ян, что ты, что Чёрный Пастор, были правы. Остров Справедливости существует. Я был на нём.";
		...
		case "return_LSC_8":
			...
			link.l1 = "На Карибах есть как минимум три истукана, называемых статуей Кукулькана: у деревни мискито в Западном Мэйне, на дне мелководья Острова Справедливости и у деревни карибов на Доминике. Эти истуканы способны при помощи индейской магии мгновенно перемещать человека на расстояние.";
		...
		case "barbazon_6":
			dialog.text = "Да, доходили слухи. Этот парень работал на Маркуса Тиракса, совсем недавно. Недолго, но успел прославиться на весь архипелаг своими походами под началом Хранителя Кодекса. Взял испанский город в глубине Южного Мэйна, проплыв на лодках по реке через джунгли, участвовал в знаменитом налёте Тиракса на Картахену. А потом исчез куда-то: то ли на вольные хлеба подался, то ли испанцы его сцапали, не знаю. А он-то тут каким боком?";
		...
		case "saga_34":
			dialog.text = "Рудник был действительно заброшен испанцами, однако Лоуренс Белтроп из Порт-Ройала выкупил его за бесценок и тайно продолжил разработки. Видимо, испанцы рано покинули шахту, так как хотя золота мы и не нашли, но обнаружили нескольких рабов, которые подтвердили, что оно там было ещё до недавнего времени.";
			link.l1 = "Лоуренс Белтроп? Я никогда о таком не слышал. Кто это?";
			link.l1.go = "saga_35";
		break;
		
		case "saga_35":
			dialog.text = "Это местный уроженец, сын Томаса Белтропа, в далёком прошлом побратима Николаса Шарпа и капера Компании острова Провиденс. Этого Томаса я хорошо знал лично. Лоуренс год назад вновь появился на Карибах. Выходит, что Джекман работал на него. Учти это и держи ухо востро!";
		...
		case "saga_39":
			dialog.text = ""+pchar.name+", я тебя уверяю: острова никто просто так не раздаёт, и даже по завещанию, и даже при условии, что всё сказанное в завещании - выполнено. На этот остров точат зуб английские власти, мне-то это хорошо известно\nНе думаю, что полковник д'Ойли будет спокойно смотреть, как остров, который он уже считает английским, уплывает из его рук. Да и нам с тобой не стоит светиться в этом деле - нежелательно это.";
			link.l1 = "И что же ты предлагаешь?";
			link.l1.go = "saga_40";
		break;
		
		case "saga_40":
			dialog.text = "Отправляйся в Порт-Ройал и найди там человека по имени Альберт Локсли. Это лучший адвокат на всех Карибах. Он изрядно поднаторел во всякого рода тёмных делишках, так что возьмётся за любую работу, сулящую крупный гонорар. Аппетиты у него большие, но он того стоит.";
		...
		case "saga_41":
			...
			link.l1 = "Хорошо. Отправляюсь в Порт-Ройал. Сколько времени потребуется для собрания Совета и проведения выборов?";
			link.l1.go = "saga_42";
		break;
		...
		case "saga_44":
			...
			dialog.text = ""+sTemp+" передаёт тебе горячий привет. Он всегда будет рад видеть тебя в резиденции на Исла Тесоро. Заходи к нему в гости при случае.";
		...
		case "saga_45":
			...
			link.l1 = "Именно так. Он берётся за наше дело, но потребовал за услуги кучу золота. Аванс - "+sTemp+" дублонов. Аванс! И ещё придётся доплачивать в зависимости от того, как пойдёт доказательство прав Элен. Сумма вырастет вдвое или втрое, как он сказал.";
			link.l1.go = "saga_46";
		break;
		
		case "saga_46":
			dialog.text = "Ха! Ну что же, я предполагал от старого пройдохи подобное... Он чует добычу за версту, как гиена. Конечно, если речь идёт о целом острове, смешно рассчитывать, что Альберт запросил бы пару сотен. Значит, итоговая сумма может составить примерно полторы тысячи золотых...";
		...
		case "saga_48":
			dialog.text = "Слушай. Тебе, наверное, известно, что наше поселение полулегально. Англичане держат тут гарнизон, а взамен мы поставляем им по льготным ценам красное дерево, которое добываем. Но есть одна порода особо прочной и тяжёлой древесины - бакаут\nВстречается он редко, и обрабатывается очень трудно даже закалённой сталью. Говорят, в древности воины майя-ица делали из него свои страшные палицы. На рынке бакаут не купить, а контрабандой он идёт чуть ли не на вес серебра\nЛюди д'Ойли время от времени забирают весь заготовленный нами бакаут по бросовым ценам. Но некоторую часть мне всегда удаётся отложить для собственных нужд.";
		...
		case "saga_oust_yes":
			dialog.text = "Великолепно! Я оснащу её, как она того заслуживает. А потом использую для развития дела с бакаутом и попутных грузоперевозок, как мы и договорились. Думаю, что уже месяца через два.";
		...
		case "saga_60":
			...
			link.l1 = "В общем, теперь у меня есть две тысячи золотых для Локсли, да и сверх того осталось "+sti(npchar.quest.bakaut_sum)*2+" дублонов.";
			link.l1.go = "saga_61";
		break;
		...
		case "bakaut_pay_wait":
			...
			link.l1 = "Я готов"+GetSexPhrase("","а")+" отдать тебе ещё золота.";
		...
		case "saga_78":
			...
			link.l1 = "Лоуренс Белтроп из Порт-Ройала. Хозяин рудника и та самая таинственная тень, стоявшая за Джекманом. Это тоже мне поведал Бейкер, а узнал он это от Маски.";
			link.l1.go = "saga_79";
		break;
		
		case "saga_79":
			dialog.text = "Каррамба! Так вот, выходит, кто организовал всё это! Ну хорошо... Мы ещё доберёмся до Бучера. Я подумаю, как прищучить этого негодяя. Хотя это будет непросто - он под надёжной защитой английских властей.";
		...
		case "saga_82":
			...
			link.l1 = "Да, путь к победе был нелёгким. Если бы я заранее знал, что мне придётся столько всего пережить...";
			link.l1.go = "saga_83";
		break;
		...
		case "saga_87":
			dialog.text = ""+pchar.name+", погоди ещё минутку... У меня для тебя два подарка. За твою помощь Братству и Элен. Первый - вот этот заговорённый оберег сарацин с далёкого Востока. Его чары укрепляют дисциплину на корабле, проверено мною лично. Держи.";
			link.l1 = "Спасибо!";
			link.l1.go = "saga_88";
		break;
	...
	//----------------------------------- штурм рудника --------------------------------------------
		...
		case "mine_attack_14":
			dialog.text = "Да, "+pchar.name+", опять тебе. Я очень хотел бы пойти сам, но меня наверняка узнают и вся операция сорвётся. Там будет опаснее - противников больше, да и про орудия забывать не стоит. Кого ты возьмёшь себе в помощь - своих офицеров, или как в прошлый раз - Грегори и мушкетёра?";
		...
		case "mine_attack_21":
			...
			link.l1 = "Гм... Ха! Идея! Пушки! Гляди, тут есть не только картечь, но и бомбы! Ну, теперь бандитам крышка! Как я раньше не додумался?!";
		...
		case "mine_attack_27":
			...
			link.l1 = "Ян, чёрт возьми! Почему так долго? В моём отряде просто огромные потери!";
		...
		case "mine_attack_28":
			...
			link.l1 = "Рандольф постарался... Видел бы ты, сколько негодяев мы положили на тропе орудийными залпами, прежде чем вступить в рукопашную!";
		...
		case "mine_attack_40":
			...
			link.l1 = "Нет. В этих халупах нет даже завалящего дублона. Зато я нашёл кое-какие бумаги. В них написано, что владелец этого рудника - какой-то Лоуренс Белтроп из Порт-Ройала.";
		...
		case "helensleep":
			dialog.text = "";
			link.l1 = "Ян, не надо орать на меня. Давай обсудим, решим все вопросы - нам работать вместе!";
			link.l1.go = "helensleep_1";
		break;
		
		case "helensleep_1":
			dialog.text = "Я на тебя собак сейчас спущу, лягушатник! Забудь о Левассере, и чтобы ноги твоей тут не было!";
			link.l1 = "Ян! Послушай меня...";
			link.l1.go = "exit";
		...
		case "after_drinking":
			dialog.text = "Какой бочонок? Элен-то я конечно дам ремня... Шучу, Шарль, шучу! Я-то знаю, как оно бывает, а Джоанне вся ситуация вообще на пользу пойдёт, а то раскудахталась последнее время! Ладно, дело житейское, удачи тебе в Картахене!";
			link.l1 = "До встречи, Ян.";
			link.l1.go = "exit";
		break;
