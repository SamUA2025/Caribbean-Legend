...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "rozencraft":
			...
			else
			{
				...
				link.l1 = "Я полагаю, это вы ждёте партию бакаута для покупки?";
				link.l1.go = "rozencraft_no";
			}
		break;
		
		case "rozencraft_no":
			dialog.text = "Кто вам сказал подобную чушь, сударь? Я ничего и никого не жду. Моё судно выполняет задание губернатора Виллемстада по патрулированию здешних вод - мы разыскиваем одного пирата. Так что извольте покинуть палубу моего судна, вы, вероятно, с кем-то меня спутали.";
		...
		case "rozencraft_no_repeat":
			dialog.text = "Мы всё обсудили, сударь, разве нет?";
		...
		case "rozencraft_1":
			...
			link.l1 = "Какая тебе разница, с кем иметь дело? Я привёз бакаут на продажу. Сколько ты за него заплатишь?";
		...
		case "rozencraft_4":
			...
			if(iTemp >= 90)
			{
				dialog.text = "Отлично. Это даже больше, чем я ожидал. Вот, держи золото. Если разживёшься ещё бакаутом - приходи в таверну Синт-Маартена и поговори... сам понимаешь с кем. Он сведёт тебя со мной. Вижу, с тобой можно вести дела, и получше, чем с этим недоноском Моллиганом.";
			...
			{
				dialog.text = "Хорошо. Вот, держи золото. Если разживёшься ещё бакаутом - приходи в таверну Филипсбурга и поговори... сам понимаешь с кем. Он сведёт тебя со мной. Вижу, с тобой можно вести дела, и не хуже, чем с этим недоноском Моллиганом.";
				link.l1 = "Хорошо. Считай, что мы договорились.";
				link.l1.go = "rozencraft_5";
				pchar.GenQuest.Bakaut = "true"; // генератор сбыта бакаута
				break;
			...
			dialog.text = "Ты что, смеёшься? Мы же договаривались с Моллиганом, что партия должна быть не меньше пятидесяти штук! Проваливай, и не появляйся, пока у тебя в трюмах не будет нужного количества.";
		...
		case "rozencraft_5":
			...
			else 
			{
				...
				if (CheckAttribute(pchar, "GenQuest.Bakaut")) AddQuestUserData("Testament", "sText", "Кроме того, если я ещё добуду бакаут - я смогу сбыть эту ценную древесину Михаэлю Розенкрафту, договорившись через его человека в таверне Филипсбурга.");
	...
	// ------------------------------------генератор сбыта бакаута-----------------------------------------------
		case "bakaut":
			dialog.text = "А, старый знакомый! Привёз бакаут на продажу?";
			link.l1 = "Именно так, Михаэль. Займёмся торговлей?";
			link.l1.go = "bakaut_0";
		break;
		...
		case "bakaut_no":
			...
			{
				dialog.text = "Чёрт возьми, тебя что, не предупредили? Или крысы сжевали древесину? Мне нужно "+FindRussianQtyString(sti(pchar.GenQuest.Bakaut.Value))+", и ни единицей меньше. Привози в следующий раз столько, сколько тебе скажет мой агент. Надеюсь, больше ты меня не разочаруешь, а то придётся отказаться от твоих услуг.";
			...
			else
			{
				...
				{
					dialog.text = "Ты второй раз меня подводишь. Мне это очень не нравится. Имей в виду: ещё раз не выполнишь условия сделки - более я с тобой дел вести не буду.";
					link.l1 = "Проклятые крысы испортили товар!.. Больше накладок не будет, Михаэль, я обещаю.";
					link.l1.go = "bakaut_no_1";
				}
				else
				{
					dialog.text = "Вот теперь моё терпение действительно лопнуло! Я более не желаю вести с тобой никаких дел. Можешь убираться на все четыре стороны и забыть про меня.";
					link.l1 = "Но минхер...";
		...
		case "bakaut_2":
			LAi_Fade("", "");
			WaitDate("",0,0,0,1,10);
			dialog.text = "Приятно иметь с тобой дело. Решишь продать мне ещё бакаута - ты знаешь, куда и к кому обращаться.";
			link.l1 = "Ещё увидимся, сударь. Наше сотрудничество приносит мне неплохие барыши. До встречи!";
			link.l1.go = "bakaut_3";
		break;
