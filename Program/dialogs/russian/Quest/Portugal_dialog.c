void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Avendel_tavern":
			...
			dialog.text = "О, ещё "+GetSexPhrase("один бывалый моряк","одна бывалая морячка")+" ! Я вашего брата издалека примечаю, мой распроклятый глаз на вас намётан, хе! Спросите почему?";
		...
		case "Avendel_tavern_2":
			...
			link.l2 = "Заработать? И я долж"+GetSexPhrase("ен","на")+" поверить слову спившегося бродяги, ради фантастических обещаний?";
		...
		case "Avendel_tavern_4":
			dialog.text = "Выслушайте меня, капитан! Ик, чтоб мне провалится на этом месте, если вру! Я говорю правду! Судите сами - если вру, вы выпустите мне проклятые потроха этой вашей красоткой, что таскаете на бедре! Да один её эфес стоит в пять раз больше моего никчёмного долга, проглоти его селёдка! Ну же? Вы ничего не теряете!";
		...
		case "Avendel_tavern_8":
			...
			link.l1 = "О вашем героическом прошлом, я уже осведомл"+GetSexPhrase("ён","ена")+". К делу, Хьюго, не испытывайте моё терпение.";
		...
		case "Avendel_tavern_9":
			...
			link.l1 = "Барт Португалец? Конечно, кто о нём не слышал, хотел"+GetSexPhrase("","а")+" бы я знать! Пират, мясник и мерзавец, насколько мне известно, а причём он в вашей истории?";
		...
		case "Avendel_tavern_11":
			...
			link.l1 = "Хьюго, моё терпение на исходе! Я обещал"+GetSexPhrase("","а")+" даме, что не буду дырявить тебя шпагой, но сойдут и кулаки! Клянусь - завтра твоя голова будет трещать так, что никакая настойка не поможет!";
		...
		case "Avendel_tavern_13":
			dialog.text = "Нет, капитан, я побоялся следить за этими ребятами. Если бы Одноглазый Пес меня заметил... бр-р! Но с вашими-то парнями мы быстро прочешем город - собирайте людей, и вперёд! Проклятье, голландские дублоны уже нас заждались! Такой распроклятый шанс выпадает раз в жизни, укуси меня селедка!";
		...
		case "Portugal_guard":
			dialog.text = "Ты, "+GetSexPhrase("дружок","подруга")+", "+GetSexPhrase("ошибся","ошиблась")+" адресом. Давай, проваливай отсюда, живо!";
			link.l1 = "Хм. А вот мне кажется, что я как раз попал"+GetSexPhrase("","а")+" туда, куда надо...";
		...
		case "Portugal_doctor_4":
			dialog.text = "Я не вижу солдат, "+GetAddress_Form(NPChar)+". Где они? Вы сказали, что его арестуют, как только вы сообщите о том что случилось властям, так почему же нет солдат до сих пор?";
		...
		case "Portugal_doctor_5":
			...
			link.l1 = "Нет, док, вы отправляетесь с нами. Во-первых, я не "+GetSexPhrase("силён","сильна")+" в медицине, а во-вторых, неужели вы хотите остаться и объяснять головорезам Португальца, куда исчез их капитан?";
		...
		case "Portugal_doctor_10":
			dialog.text = "Уже бегу, "+GetAddress_Form(NPChar)+".";
			link.l1 = "...";
		...
		case "Avendel_Room":
			...
			link.l1 = "Хьюго, оставь при себе свои шутки. Я поставил"+GetSexPhrase("","а")+" людей на улице, приглядывать за домом, но ещё один сторож нашему гостю не помешает. Следи за ним в оба и постарайся не так сквернословить, иначе Джино выдаст тебе какую-нибудь микстуру - я уверен"+GetSexPhrase("","а")+", это лечится.";
		...
		case "Portugal_hunter_1":
			dialog.text = ""+GetSexPhrase("Какой грозный","Какая грозная")+", глядите-ка парни! Послушай меня, "+GetSexPhrase("щенок","девка")+" - не стой между Лео Кордом и его деньгами! Мы выслеживали Португальца почти полгода, и вот он у нас в руках, а ты опусти эту зубочистку, и отваливай, пока цел"+GetSexPhrase("","а")+"! Мы не жадные, нам он сойдёт и мёртвым, ха-ха!";
		...
		case "Avendel_street":
			...
			if (LAi_GetCharacterHP(sld) > 0)
			{
				...
				link.l1 = "Верно подмечено, Хьюго, и я рад"+GetSexPhrase("","а")+", что в ту щель, куда ты забился на время драки не залетела шальная пуля.";
		...
		case "Portugal_doctor_14":
			dialog.text = ""+GetSexPhrase("Минхер капитан","Мефрау")+", я присмотрю за Португальцем, ему сейчас нужен полный покой.";
			link.l1 = "Понял"+GetSexPhrase("","а")+", я вас оставляю, всё в порядке, док. Проклятье - да сколько мне ещё носится с этим пиратом, как с больной тётушкой? То ему вредно, то ему нужно! Можно подумать, я нанимал"+GetSexPhrase("ся","ась")+" в няньки, а не... хм, простите, доктор, накипело... Всего доброго.";
		...
		case "Portugal_patrol":
			dialog.text = ""+GetSexPhrase("Месье","Мадемуазель")+", это вы капитан "+GetFullName(pchar)+", чей корабль сегодня прибыл в гавань?";
			link.l1 = "Вы не ошиблись, чем могу быть полез"+GetSexPhrase("ен","на")+"?";
			link.l1.go = "Portugal_patrol_1";
		break;
		
		case "Portugal_patrol_1":
			dialog.text = "Вас срочно желает видеть Его Светлость Жак Дьель дю Парке, губернатор нашей колонии. Дело не терпит отлагательств!";
			link.l1 = "Хорошо, месье, я навещу Его Превосходительство сегодня же.";
		...
		case "Avendel_seapatrol_7":
			...
			link.l1 = "Да, смелости тебе действительно не занимать... Ладно, дам тебе ещё один шанс - мы отправляемся на поиски. Я слишком долго спасал"+GetSexPhrase("","а")+" шкуру этого негодяя и уже начал"+GetSexPhrase("","а")+" считать её почти что собственностью! У нас ещё есть время попробовать их перехватить - начнём с таверны, может кто-то видел больше, чем твои пятки. Иди, собирайся!";
		...
		case "Avendel_seapatrol_9":
			...
			link.l1 = "Успокойся, Хьюго. Я уверен"+GetSexPhrase("","а")+", Бартоломью ещё на острове, он слишком слаб, даже если ему и полегчало. И я, кажется, даже знаю, где его искать! Мы настигнем их - от своей награды я так просто не отступлюсь! Если не струсишь - следуй за мной, ещё не всё потеряно.";
		...
		case "LeFransua_pirate":
			dialog.text = "Много вопросов задаёшь, капитан... Постой-ка, а не "+GetSexPhrase("тот","та")+" ли ты вертляв"+GetSexPhrase("ый субчик","ая фифочка")+" , из Филипсбурга? Бегал"+GetSexPhrase("","а")+", бегал"+GetSexPhrase("","а")+" от нас, и гляди - сам"+GetSexPhrase("","а")+" приш"+GetSexPhrase("ёл","ла")+"...";
		...
		case "FranzGarke":
			dialog.text = "...ты, крабий потрох, всё нам расскажешь, попомни моё слово, крыса, всё! До, или после того, как с тебя сдерут шкуру и повесят на рее вместо марселя... А ты ещё кто так"+GetSexPhrase("ой","ая")+"? Тысяча акул, ни в чём на этих обезьян нельзя положиться! Придётся самому...";
			link.l1 = "Кажется, я вам помешал"+GetSexPhrase("","а")+", господа?";
		...
		case "Avendel_LeFransua":
			...
			if (LAi_GetCharacterHP(sld) > 0)
			{
				dialog.text = "Отличная работа! Вы свалили Одноглазого Пса - вот это я понимаю - "+GetSexPhrase("мастер","амазонка")+"! Скольких ещё его ребят вы отправили на сковородки?";
				link.l1 = "Я не считал"+GetSexPhrase("","а")+", я драл"+GetSexPhrase("ся","ась")+". А где был ты, Хьюго?";
		...
		case "Portugal_LeFransua":
			...
			link.l1 = "Что же, значит придётся лично его поприветствовать. Шагайте, месье Португалец, и постарайтесь поменьше болтать - мы снимаемся с якоря при первой возможности.";
		...
		case "Portugal_free":
			if (LAi_group_GetTarget(pchar) <= 0)
			{
				...
				link.l2 = "Ты не думал, что я просто соскучился? Зашёл вот в гости к тебе. А у тебя уютно!";
		...
		case "Portugal_free_6":
			dialog.text = "Ха, и это отменное качество! Но, я всё же настаиваю - вот, возьмите... это единственное, что мне удалось утаить в тюрьме... Мой оберег. Он всегда приносил мне удачу в море. Вы отличный моряк, так что он вам точно пригодится.";
