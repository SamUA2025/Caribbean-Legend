...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		case "Node_rat_1":
			dialog.text = "Нет здесь никакого Гонтьера! Если это всё, что ты хотел"+ GetSexPhrase("","а") +" узнать - убирайся!";
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//--> Jason ------------------------ Сомнительное предложение -------------------------------------------	
		...
		case "Contraoffer_patrol":
			...
			dialog.text = "Так-так... Вот вы оба и попались, голубчики. Разве вы не в курсе, что оборот " + GetGoodsNameAlt(iCGood)+ " частными лицами в нашей колонии запрещён? Как всё это понимать?";
			link.l1 = "Офицер, разве запрещён? Я впервые об этом слышу, честное слово. Это постановление приняли недавно, наверное?";
		...
		case "Contraoffer_patrol_1":
			...
			{
				dialog.text = "Вы были не в курсе? Хм... похоже, вы не лжёте. Но тем не менее, это так, и своими действиями вы нарушили закон. Я не буду арестовывать вас, но оштрафовать всё-таки придётся. Ну, а что делать с грузом - это будем решать со вторым участником сделки.";
			...
			else
			{
				...
				link.l1 = "Офицер, я не привычен к казематам. Я люблю море... и сейчас намерен в него выйти. Но, вероятно, это придётся сделать через ваш труп.";
				link.l1.go = "Contraoffer_patrol_fight";
				link.l2 = "Мне ничего не остаётся, как подчиниться. Но я правда не знал!";
		...
		case "Contraoffer_patrol_pay1":
			...
			{
				dialog.text = "Вот как... Ну что же, давайте посмотрим на ваши монеты... Тут прозвучала сумма в " + FindRussianMoneyString(sti(pchar.GenQuest.Contraoffer.Trader.Summ)) + ". Поскольку у нас теперь не двухсторонний, а трёхсторонний договор, то я бы хотел увидеть " + FindRussianMoneyString(sti(pchar.GenQuest.Contraoffer.Trader.PatrolSumm)) + ". Это с вас. Столько же и от второго участника сделки.";
				...
				link.l2 = "Ого! У вас неумеренные аппетиты, офицер. Вы никогда не слышали поговорки 'большая ложка рот дерёт'? Пожалуй, дешевле будет познакомить вас и ваших подчинённых с моей саблей.";
			...
			else
			{
				...
				link.l1 = "Ну, раз на деньги вы смотреть не хотите, тогда взгляните на острие моей сабли!";
				link.l1.go = "Contraoffer_patrol_fight";
				link.l2 = "Мне ничего не остаётся сделать, как подчиниться. Но я же не знал...";
				link.l2.go = "Contraoffer_patrol_jail";
			}
		break;
		
		case "Contraoffer_patrol_complete1":
			dialog.text = "Да, действительно, вы правы... Кроме монет, я тут и впрямь ничего не вижу. Вы свободны, капитан, ступайте, а мы потолкуем с вашим партнёром.";
		...
		case "Contraoffer_patrol_pay2":
			...
			link.l2 = "Сдаётся мне, офицер, что вы нечисты на руку. Уж больно велик назначенный вами штраф. Думаю, что эта колония только выиграет, если я избавлю её от вашего присутствия.";
			link.l2.go = "Contraoffer_patrol_fight";
		break;
		
		case "Contraoffer_patrol_complete2":
			dialog.text = "Хорошо. Теперь можете идти. И прежде чем торговать, советую ознакомиться со списком запрещённых к ввозу товаров, чтобы избежать повторения сегодняшней ситуации. Ступайте, теперь нам нужно заняться вторым участником сделки - он-то точно был в курсе, чем можно торговать, а чем нельзя.";
		...
		case "Contraoffer_patrol_jail":
			dialog.text = "Комендант разберётся, что вы знали, а чего не знали. Следуйте за мной!";
			link.l1 = "...";
	...
	//Jason --> ----------------------------------- Неудачливый вор --------------------------------------------
		case "Device_poorman":
			...
			link.l1 = "Да, приятель. Сдаётся, что ты и есть тот, кто мне нужен. Ведь это ты украл на местной верфи "+pchar.GenQuest.Device.Shipyarder.Type+"? Все улики ведут к тебе, так что не отпирайся.";
		...
		case "Device_poorman_1":
			switch (sti(pchar.GenQuest.Device.Shipyarder.Chance3))
			{
				...
				{
					dialog.text = "Помилуйте, госпо"+GetSexPhrase("дин","жа")+"! Да, я стащил на верфи эту странную железку. Но я даже не смог её никому продать - никому она оказалась не нужна. Я её и выбросил. Пощадите, сударь, я просто очень хотел кушать... иначе я никогда бы не решился на кражу!";
				...
				{
					dialog.text = "Помилуйте, госпо"+GetSexPhrase("дин","жа")+"! Да, я стащил на верфи эту странную железку. Но я даже не смог её никому продать - никому она оказалась не нужна. Я отдам её вам. Вот, берите, только не трогайте меня и не сдавайте солдатам!";
					link.l1 = "Ладно, живи, воришка. Ты мне не нужен, мне нужен этот инструмент. Живо давай его сюда, а потом можешь проваливать на все четыре стороны!";
					link.l1.go = "Device_poorman_0_2";
				}
				break;
				case 1://упирается, хитрый или храбрый
					dialog.text = "О чём вы говорите? Что за "+pchar.GenQuest.Device.Shipyarder.Type+"? Не понимаю!";
					link.l1 = "Вот сейчас обыщу тебя, тогда поймёшь, о чём я говорю! Отдай лучше подобру-поздорову!";
					link.l1.go = "Device_poorman_1_1";
				break;
				case 2://трусливый
					dialog.text = "Ой... Что за "+pchar.GenQuest.Device.Shipyarder.Type+"? Я... а что это вообще такое?";
					link.l1 = "Не прикидывайся дурачком и не пытайся дать тягу, всё равно догоню. А сейчас мы проведём небольшой обыск...";
		...
		case "Device_poorman_0_1"://идем на поиски
			dialog.text = "Да сразу за городскими воротами, у выхода в джунгли. Помилуйте, великодушн"+GetSexPhrase("ый","ая")+" госпо"+GetSexPhrase("дин","жа")+"! Если этот инструмент вам так нужен - забирайте его, он там так и валяется, я уверен.";
			link.l1 = "Надо было бы заставить тебя самого пойти разыскать и принести, да боюсь, дольше потом тебя искать придётся, чем "+pchar.GenQuest.Device.Shipyarder.Type+". Сам"+GetSexPhrase("","а")+" найду. Но смотри: если солгал - из под земли достану!";
			link.l1.go = "exit";
		...
		case "Device_poorman_0_2"://получили девайс
			dialog.text = "Вот, забирайте. Спасибо, "+GetSexPhrase("великодушный господин","великодушная госпожа")+"!";
		...
		case "Device_poorman_1_1":
			...
			dialog.text = "Послушайте, вы случайно не об этом инструменте речь ведёте? Если об этом, то это моя вещь, и я её не крал! И отдавать вам её просто так вот не намерен! Если хотите её получить - заплатите мне " + FindRussianMoneyString(sti(pchar.GenQuest.Device.Shipyarder.Poorsumm)) + ".";
			...
			link.l2 = "Вот как? Ты что же, жалкий воришка, думаешь, я тебе поверил? А ну, давай сюда "+pchar.GenQuest.Device.Shipyarder.Type+", живо, не то стражу позову - и мы все вместе сходим на верфь и разберёмся, чей это инструмент на самом деле.";
		...
		case "Device_poorman_1_2":
			...
			link.l1 = "Меньше болтай языком, воришка! Не думай, что я тебе поверил, просто шум лишний поднимать неохота. Да и всё равно тебе эти деньги во благо не пойдут. Всё, проваливай!";
			link.l1.go = "exit";
		...
		case "Device_poorman_1_3":
			if (sti(pchar.GenQuest.Device.Shipyarder.Chance2) == 0)//если мы в городе
			{
				...
				link.l1 = "Так-то лучше! А теперь можешь проваливать. Всё равно дорога тебе одна - на виселицу, это лишь вопрос времени.";
				link.l1.go = "exit_device";
			}
			else//храбрый попался
			{
				dialog.text = "Капитан, какая стража? Тут только мы с вами. Ну, а раз вы так взъерепенились, то мне не остаётся ничего иного, как немножко вас успокоить... сталью моего клинка.";
		...
		case "Device_poorman_1_4":
			...
			{
				dialog.text = "Ну, капитан, это мы ещё посмотрим, кто кого проткнёт!";
				link.l1 = "Ты смеешь мне угрожать, мразь?";
				link.l1.go = "Device_poorman_fight";
			}
		break;
		...
		case "Device_poorman_2_1":
			dialog.text = "Какой ещё такой обыск? Помилуйте, благородный господин!";
			link.l1 = "Значит так: или ты мне сейчас добровольно отдаёшь то, что украл с верфи, или я отведу тебя в комендатуру. Там ты всё равно с этой вещью расстанешься, правда, после лёгкого прижигания пяток калёным железом. А может, ещё и вздернут потом. Так что выбирай.";
		...
		case "Device_poorman_2_2":
			...
			{
				dialog.text = "Не надо в комендатуру! Я сам отдам. Вот, держите, вы, наверно, это ищете. Всё равно её никому не продать...";
		...
		case "Device_poorman_2_3"://напугали
			dialog.text = "Ай! Не трогайте меня, я всё отдам! Вот, забирайте!";
			link.l1 = "То-то же! Ступай, и если я тебя ещё хоть раз увижу в этом городе - сдам коменданту. Проваливай!";
			link.l1.go = "exit_device";
		break;
...
//-------------------------------------------почтовый генератор 2 уровня------------------------------------	
		case "PostGopHunters":
			dialog.text = "Стой, приятель! Нам нужна вещь, которая сейчас у тебя. Отдай её нам по добру, и ступай своей дорогой.";
		...
		case "PostGopHunters_2":
			dialog.text = "Полегче с выражениями, приятель... Молодец, что не стал испытывать судьбу. Исход-то всё равно был бы один, ха-ха! А так поживёшь ещё... Ну, бывай.";
			link.l1 = "...";
		...
		case "PostProfHunters_1":
			dialog.text = "Считайте, капитан, что уже познакомились. Теперь по сути. Вы сейчас получили в портовом управлении "+pchar.questTemp.WPU.Current.Add+" "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+". Не пытайтесь отрицать, нам это достоверно известно - мы уже несколько дней ведём поиск этого документа.";
			link.l1 = "Теперь ясно. И что?";
			link.l1.go = "PostProfHunters_2";
		break;
		
		case "PostProfHunters_2":
			dialog.text = "Сейчас вы отдадите нам этот пакет и наше кратковременное знакомство завершится. И не пытайтесь геройствовать, кэп: мы намерены получить эти бумаги, даже если для этого потребуется драться с городским гарнизоном\nКак вы понимаете, в случае драки вы умрёте первым. Так что отдайте документы и расстанемся друзьями.";
		...
		case "exit_profhunter":
			...
			AddQuestUserData("Postcureer", "sSex", GetSexPhrase("","а"));
			AddQuestUserData("Postcureer", "sSex1", GetSexPhrase("ёл","ла"));
		...
		case "PostAgent_1":
			dialog.text = "То, что вы спешите - мне известно. А ещё мне известно, что вы сейчас получили в портовом управлении "+pchar.questTemp.WPU.Current.Add+" "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.City+"Gen")+". Вот об этих бумагах я и хочу с вами поговорить.";
		...
		case "PostAgent_2":
			dialog.text = "Полноте, капитан! Я что, похож на банального грабителя с большой дороги? Я деловой человек, и представляю интересы " + NationNameGenitive(sti(pchar.questTemp.WPU.Postcureer.EnemyNation)) + ". Я хочу предложить вам весьма выгодную сделку - но только не здесь. Давайте пройдём в комнату таверны, и с глазу на глаз, в спокойной обстановке всё обсудим. Это не займёт много времени.";
			link.l1 = "Подозрительно всё это... Я не намерен иметь никаких дел с вами, сударь! Убирайтесь!";
			link.l1.go = "PostAgent_getout";
			link.l2 = "Хм... Ну, давайте посмотрим, что вы сможете мне предложить. Но даже и не пытайтесь напасть на меня. Пойдёмте, я после вас.";
			link.l2.go = "PostAgent_3";
		break;
		
		case "PostAgent_getout":
			dialog.text = "Вы упрямый и тупой человек, капитан! Я предложил вам заработать, не пошевелив и пальцем. А вы... вы ещё пожалеете об этом.";
			link.l1 = "Убирайтесь!!";
		...
		case "PostAgent_getout_exit":
			...
			AddQuestUserData("Postcureer", "sSex", GetSexPhrase("","а"));
			AddQuestUserData("Postcureer", "sSex1", GetSexPhrase("ся","ась"));
			DialogExit();
		break;
		...
		case "PostAgent_inTavern_1":
			dialog.text = "Как я уже сказал, речь идёт о бумагах, которые вы получили в портовом управлении. Мне нужно всего лишь снять с них копию и всех дел\nОригинал останется у вас, вы его отвезёте адресату и получите свою награду. Никто ни о чём не узнает, а за возможность снять копию документа я вам неплохо заплачу.";
		...
		case "PostAgent_inTavern_2":
			dialog.text = "Я ожидал этого вопроса. Сломать печать действительно придётся, но не беспокойтесь - я восстановлю её, после того как сниму копию. У меня есть дубликат нужного клише, и сургуч, конечно, тоже. Никто и не заметит, что пакет вскрывали.";
		...
		case "PostAgent_inTavern_3":
			...
			link.l1 = "Держите. Только делайте всё аккуратно!";
			link.l1.go = "PostAgent_inTavern_4";
			link.l2 = "Знаете что, я не буду с вами сотрудничать. Не нравится мне всё это.";
			link.l2.go = "PostAgent_inTavern_getout";
		break;
		
		case "PostAgent_inTavern_4":
			dialog.text = "Не переживайте. Комар носа не подточит. Присядьте, отдохните. Это всё займёт у меня примерно час.";
			link.l1 = "Хорошо.";
		...
		case "PostAgent_inTavern_getout":
			dialog.text = "Хм... не ожидал. Ну, раз вы такой упрямец, капитан, мне просто придётся убить вас прямо здесь и забрать пакет бесплатно. Помолитесь напоследок, ибо клинком я владею не хуже, чем поддельными клише.";
			link.l1 = "Это тебе стоит помолиться, шпион!";
			link.l1.go = "PostAgent_inTavern_fight";
		break;
		
		case "PostAgent_inTavern_6":
			dialog.text = "Ну вот и всё. Как я и говорил - всё сделано в лучшем виде. Получите ваш пакет и обещанные "+ FindRussianMoneyString(sti(pchar.questTemp.WPU.Postcureer.AgentMoney))+". Ну, а теперь нам пора проститься. Всего доброго, капитан, рад, что мы нашли общий язык.";
			link.l1 = "Прощайте...";
	...
	// --> --------------------------------- эскорт 2 уровень --------------------------------------------------
		...
		case "DisasterCap_1":
			...
			link.l1 = "А, ну вот я и нашёл вас... Позвольте представиться - " + GetFullName(pchar) + ". Я прибыл сюда по просьбе начальника портового управления "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.StartCity+"Gen")+". Он вкратце рассказал о злоключениях вашего каравана и отправил меня на розыск вашего корабля.";
		...
		case "DisasterCap_3":
			dialog.text = "Ох, простите, я не представился. Моё имя - " + GetFullName(npchar) + ". Вам уже известны некоторые обстоятельства того, как мы попали сюда? После того, как налетевший шторм разлучил наше судно с караваном, нас больше суток несло по волнам - корабль был неуправляем\nВ конце концов нас выбросило на рифы недалеко от этой бухты. Шторм не прекращался, и корабль развалился в считаные минуты. Спастись удалось не всем... лишь немногим более полусотни человек моей команды смогли достичь спасительного берега. Остальные лежат там, на дне. Мы каждый день молимся за их души...";
			link.l1 = "Примите мои соболезнования, капитан " + GetFullName(npchar) + ". Мне действительно очень жаль.";
			link.l1.go = "DisasterCap_4";
		break;
		
		case "DisasterCap_4":
			dialog.text = "Спасибо... Ну, а мы, выжившие, разбили на берегу лагерь и ждали помощи. Которая явилась в вашем лице. И вы прибыли очень вовремя, капитан! Потому что о нашем появлении пронюхали местные воинственные индейцы - мы уже дважды отбивали их атаки на наш бивуак. Но долго мы против них продержаться не смогли бы - их сотни, а нас всего лишь пять десятков\nКроме того, опасность также грозит и с моря - наш дозорный уже три дня наблюдает за пиратским кораблём, крейсирующим в акватории острова - он явно разыскивает наше судно. Вы не встретили этого пирата, капитан, по пути сюда?";
			link.l1 = "Нет, не встречал.";
			link.l1.go = "DisasterCap_5";
		break;
		
		case "DisasterCap_5":
			dialog.text = "И слава Богу. Это один из тех, ну, которые напали нас тогда, до того кошмарного шторма. Он, как гиена, решил не упускать подраненную добычу, и явился вслед за нами сюда, к этому острову.";
			link.l1 = "Ясно. Ну, теперь надо решить, что мы будем делать.";
			link.l1.go = "DisasterCap_6";
		break;
		
		case "DisasterCap_6":
			dialog.text = "А что тут решать, капитан? Возьмите нас к себе на борт, и уберёмся из этого ужасного места!";
		...
		case "DisasterCap_7":
			...
			dialog.text = "Спасибо, капитан! Нам не потребуется много времени. Постойте... Это что ещё за чёрт? А-ать, опять эти краснокожие дьяволы! Тревога! К бою!";
			link.l1 = "...";
			link.l1.go = "exit";
		...
		case "DisasterCap_8":
			dialog.text = "Ух, отбились... Видите, капитан, я вам говорил: эта бухта - сущий ад. Это уже третья атака проклятых индейцев! Нужно побыстрее уносить отсюда ноги, ещё одно такое нападение - и вам уже будет некого спасать.";
	...
	//Jason --> -------------------------- Заносчивый аристократ ------------------------------------------------
		...
		case "Badboy_1":
			...
			link.l1 = "А очень простое. Твои выходки уже утомили кое-кого в этом городе. Причём изрядно утомили. И, глядя сейчас на тебя, я понимаю, почему. В общем, или ноги твоей больше в местном борделе не будет, или считай, что ты обзавёлся кучей неприятностей. Усёк?";
			link.l1.go = "Badboy_2";
		break;
			...
			switch (sti(pchar.GenQuest.Badboy.Brothel.Type))
			{
				case 0://или напугается, или будет быковать - от авторитета
					...
					else
					{
						...
						link.l1 = "Надеюсь, ты отдаёшь отчёт своим словам и последствиям, которые наступят, если ты нарушишь данное обещание. Имей это в виду. А теперь - прощай.";
						link.l1.go = "exit";
				...
				case 1://дуэль за городом
					dialog.text = "Вы просто наглец, сударь! Я клянусь, что ещё до исхода дня заставлю вас пожалеть об этих словах. Я вызываю вас на дуэль! Думаю, лучше провести её с глазу на глаз... в тихом месте. Джунгли за городскими воротами как раз подойдут - встретимся через два часа!";
	...
	//Jason --> ------------------------------- Место под солнцем ----------------------------------------------
		...
		case "Sunplace_Store_1":
			...
			link.l1 = "И что ты сделаешь, мещанская селёдка? Ты насолил одному хорошему человеку, и он очень расстроен. Так что молись, но побыстрее, ибо медлить я не намерен.";
