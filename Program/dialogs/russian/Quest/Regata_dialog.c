void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Regata_Cureer":
			...
			link.l1 = "Да, это я. А в чём дело?";
			link.l1.go = "Regata_Cureer_1";
		break;
		
		case "Regata_Cureer_1":
			dialog.text = "У меня для вас письмо от губернатора Ямайки, сэра Эдварда д'Ойли.";
			link.l1 = "Ого! Не ожидал"+GetSexPhrase("","а")+"... И что же губернатор хочет от меня?";
			link.l1.go = "Regata_Cureer_2";
		break;
		
		case "Regata_Cureer_2":
			dialog.text = "Прочтите письмо, "+GetAddress_Form(NPChar)+". Но вкратце - вам предложено принять участие в регате, гонках на время, по архипелагу. Слухи о ваших способностях к быстрому передвижению в море не остались без внимания.";
		...
		case "Regata_Head":
			if (CheckAttribute(pchar, "questTemp.Regata.Begin"))
			{
				...
				link.l1 = "Именно так, сэр. Моё имя - " + GetFullName(pchar) + ". Я получил"+GetSexPhrase("","а")+" приглашение.";
		...
		case "Regata_Head_1":
			...
			if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				...
				{
					link.l1 = "Ещё не успел отдать необходимые распоряжения. Сейчас же сделаю это и вернусь к вам.";
					link.l1.go = "exit";
		...
		case "Regata_Head_repeat":
			if (CheckAttribute(pchar, "questTemp.Regata.Begin"))
			{
				...
				if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
				{
					...
					if ((GetCompanionQuantity(pchar) > 1) || !ok)
					{
						link.l1 = "Ещё не успел. Сейчас же сделаю это и вернусь к вам.";
						link.l1.go = "exit";
						NextDiag.TempNode = "Regata_Head_repeat";
					}
					else
				...
				{
					dialog.text = "Капитан, пока я не увижу ваше судно на рейде, нам разговаривать не о чем. Вы это поняли наконец, или мне ещё раз повторить?";
					link.l1 = "Прошу прощения, сэр. Я немедленно приведу свой корабль в порт.";
					link.l1.go = "exit";
					NextDiag.TempNode = "Regata_Head_repeat";
				}
		...
		case "Regata_Head_8":
			dialog.text = "Победитель в регате только один - капитан, чей корабль вернётся в Порт-Рояль первым. Награды весьма солидные. Победитель получает крупную денежную сумму - 250 000 песо. Также ему будет вручён комплект ценных призов.";
			link.l1 = "То есть, проигравшим не достаётся ничего?";
		...
		case "Regata_rate":
			...
			link.l2 = "Пожалуй, мне ещё надо подумать.";
			link.l2.go = "Regata_Head_10";
		break;
		...
		case "Regata_rate_1":
			...
			if (iQty < 10000)
			{
				dialog.text = ""+GetSexPhrase("Сэр","Мисс")+", не смешите. Ну что за мелочность? Я даже не буду возиться с такой ничтожной суммой.";
			...
			if (iQty > 100000)
			{
				dialog.text = ""+GetSexPhrase("Сэр","Мисс")+", правила не позволяют мне принимать такие ставки. Уменьшите её размер до разумной величины.";
				link.l1 = "Жаль... Давайте пересмотрим сумму.";
				link.l1.go = "Regata_rate";
				break;
			}
			dialog.text = ""+iQty+"? Хорошая ставка. Я готов её принять. Ваши деньги, "+GetAddress_Form(NPChar)+"?";
			if (makeint(Pchar.money) >= sti(pchar.questTemp.Regata.Rate))
			{
				link.l1 = "Пожалуйста, получите.";
				link.l1.go = "Regata_rate_2";
			}
			link.l2 = "У меня сейчас недостаточно денег. Я приду позже и сделаю ставку. И ещё подумаю, может, поменяю сумму.";
			link.l2.go = "Regata_Head_10";
		break;
		...
		case "Regata_Prepare":
			...
			link.l2 = "Да нет, ничего особенного, просто решил"+GetSexPhrase("","а")+" поприветствовать вас.";
			link.l2.go = "exit";
			NextDiag.TempNode = "Regata_Prepare";
		break;
		...
		case "Regata_CitWom":
			dialog.text = LinkRandPhrase("Капитан, мы с мужем поставили на вас десять тысяч. Не подведите нас!","Капитан! Желаю вам удачи!","Ах, вы "+GetSexPhrase("такой отважный, и такой мужественный","такая отважная, и такая мужественная")+", капитан...");
			link.l1 = "...";
			link.l1.go = "exit";
		...
		case "Regata_CitHorse":
			dialog.text = LinkRandPhrase("Хей, "+GetSexPhrase("морячок","морячка")+"! Помаши красотке на прощание! Я тебя жду, когда вернёшься!",""+GetSexPhrase("Хи-и-и, моряк, солёный морячок","Хи-и-и, морячка")+"! Мадам сказала, что отдаст меня победителю регаты на неделю бесплатно... Постарайся выиграть, ты так"+GetSexPhrase("ой","ая")+" милашка!","Когда вернёшься, красав"+GetSexPhrase("чик","ица")+", не забудь заглянуть к нам - обслужим по высшему разряду!");
		...
		case "Regata_Start":
			dialog.text = "Итак, "+GetSexPhrase("джентльмены","джентльмены... и одна леди")+", вы все собрались. Я объявляю парусную регату открытой. Правила вы знаете. Побеждает тот, чей корабль вернётся в Порт-Рояль первым. Дамы и господа, пожелаем нашим отважным капитанам удачи!\n"+GetSexPhrase("Джентльмены","Участники регаты")+"! Вы готовы? На старт! Внимание! Марш!!";
			link.l1 = "...";
			link.l1.go = "Regata_Start_1";
			locCameraFromToPos(-77.00, 3.93, 109.00, false, -74.64, 0.15, 101.24);
		break;
		...
		case "Regata_Shipyarder_1":
			dialog.text = "Что верно, то верно, капитан. У меня для вас есть предложение, связанное с регатой и вашим кораблём.";
			link.l1 = "Хех! Интересно. Слушаю вас внимательно, мистер.";
			link.l1.go = "Regata_Shipyarder_2";
		break;
		
		case "Regata_Shipyarder_2":
			dialog.text = "Регата проводится на люггерах, "+GetAddress_Form(NPChar)+". А у меня как раз есть комплект парусов на люггер из хлопка самого высокого качества. Эти паруса позволят увеличить набор скорости вашим кораблём и дадут преимущество перед вашими соперниками во времени\nЯ готов вам их продать, если вы, конечно, желаете купить.";
			link.l1 = "Сколько вы хотите за эти паруса?";
			link.l1.go = "Regata_Shipyarder_3";
		break;
		
		case "Regata_Shipyarder_3":
			dialog.text = "Всего лишь 15 000 песо. Согласитесь, это небольшая цена для такой вещи, да ещё в такой ситуации.";
			if (makeint(Pchar.money) >= 15000)
			{
				link.l1 = "Соглас"+GetSexPhrase("ен","на")+"! Я покупаю у вас новые паруса на свой люггер. Вот деньги. Будьте любезны доставить их на мой корабль, пока я иду отмечаться в портовое управление.";
		...
		case "Regata_Shipyarder_4":
			...
			dialog.text = "Будет сделано, "+GetAddress_Form(NPChar)+"! Вы не пожалеете о своём приобретении. Кроме того, эти паруса выкрашены в замечательный голубой цвет - ваш люггер будет смотреться очень нарядно.";
		...
		case "Regata_Shipyarder_exit":
			dialog.text = "Жаль, очень жаль. Ну, "+GetSexPhrase("хозяин - барин","хозяйка - барыня")+". До свидания, капитан.";
		...
		case "Regata_Contra_1":
			dialog.text = "Капитан, ну это же проще простого. Идёт регата, вы её участник - как нам не знать вас!";
		...
		case "Regata_Contra_2":
			if (sti(pchar.questTemp.Regata.Index != 1))
			{
				...
				dialog.text = "Нам известно, что перед вами находится ещё один участник регаты - "+pchar.questTemp.Regata.AdversaryName.(sTemp)+" на корабле "+pchar.questTemp.Regata.AdversaryShipName.(sTemp)+". Он вышел из порта совсем недавно, и скоро будет проходить мимо бухты Гонаив.";
			}
			else
			{
				dialog.text = "Нам известно, что за вами буквально по пятам идёт ещё один участник регаты - "+pchar.questTemp.Regata.AdversaryName.b+" на корабле "+pchar.questTemp.Regata.AdversaryShipName.b+". Он с часа на час войдёт в порт.";
			}
		...
		case "Regata_Contra_3":
			if (sti(pchar.questTemp.Regata.Index != 1)) sTemp = "а вы тем временем опередите его и займёте его место";
			else sTemp = "а вы тем временем уйдёте далеко вперёд";
			...
			link.l2 = "Нет. Я не намерен"+GetSexPhrase("","а")+" применять для победы подобные методы. Вы не за "+GetSexPhrase("того","ту")+" меня приняли. Всего доброго, джентльмены.";
			link.l2.go = "Regata_Contra_exit";
		break;
		
		case "Regata_Contra_exit":
			dialog.text = "Ну, капитан, как знаете. Прощайте.";
			link.l1 = "За информацию про мыс спасибо. Буду держаться от него подальше!";
		...
		case "Regata_Contra_4":
			...
			{
				link.l1 = "Хех! Вы говорите дело, джентльмены! Я готов"+GetSexPhrase("","а")+" заплатить вам. Но какие гарантии, что вы сделаете работу? И что мой корабль не обстреляете, когда я мыс проходить буду?";
				link.l1.go = "Regata_Contra_5";
			}
			link.l2 = "Нет у меня таких денег. Так что помощью вашей воспользоваться не придётся.";
			link.l2.go = "Regata_Contra_exit";
		break;
		
		case "Regata_Contra_5":
			dialog.text = "Капитан, мы контрабандисты, а не какие-нибудь бродяги. Мы - люди честные... в своём деле. Или вы никогда нашими услугами не пользовались и не знаете, что мы не обманываем? Даю вам слово. Можете быть уверены.";
		...
		case "Regata_Contra_6":
			...
			ChangeCharacterComplexReputation(pchar, "nobility", -3);
			dialog.text = "Вы останетесь довольны этой сделкой, капитан. Ну, а мы сейчас дадим сигнал, и ваш соперник ещё не скоро сможет развить приличную скорость. Удачи!";
		...
		case "Regata_Padre":
			...
			dialog.text = "Здравствуй, "+GetSexPhrase("сын мой","дочь моя")+". Я пришёл поприветствовать тебя, а также смиренно спросить: не желаешь ли ты, как верн"+GetSexPhrase("ый","ая")+" слуга Англии и Господа нашего, внести посильную лепту в приходскую казну нашей церкви?";
			link.l1 = "Падре, люди делают пожертвования по зову своей души, а не по принуждению. У меня складывается такое впечатление, что все на архипелаге решили, что раз капитан участвует в регате - значит, его нужно обобрать...";
		...
		case "Regata_Padre_1":
			dialog.text = "Сколько позволяет тебе твоё желание и твои возможности, "+GetSexPhrase("сын мой","дочь моя")+". Наш приход будет рад даже маленькой денежной сумме.";
			Link.l1.edit = 4;			
			link.l1 = "";
		...
		case "Regata_Padre_2":
			...
			{
				dialog.text = "Нехорошо так поступать, "+GetSexPhrase("сын мой","дочь моя")+". Все мы грешны - не забывай о душе и о том, что тебе придётся предстать перед Создателем... и, возможно, очень скоро.";
			...
			{
				dialog.text = "От лица нашего прихода благодарю тебя за этот дар. И в благодарность я хочу предупредить тебя, "+GetSexPhrase("сын мой","дочь моя")+": нехорошие люди затеяли против тебя недоброе. Будь осторож"+GetSexPhrase("ен","на")+" в море!";
			...
			{
				dialog.text = "От лица нашего прихода благодарю тебя за этот дар. И в благодарность я хочу предупредить тебя, "+GetSexPhrase("сын мой","дочь моя")+": нехорошие люди затеяли против тебя недоброе. Я слышал, как эти мерзкие богоотступники договаривались потопить твой корабль при выходе из нашего порта\nОни уже снарядили судно и готовятся тебя атаковать. Будь осторожен, "+GetSexPhrase("сын мой","дочь моя")+", эти заблудшие души не шутили - следует ожидать нападения. Ступай с моим благословением!";
				link.l1 = "Спасибо, святой отец. Я буду внимател"+GetSexPhrase("ен","ьна")+".";
				link.l1.go = "Padre_exit";
				AddMoneyToCharacter(pchar, -iTemp);
			...
			{
				dialog.text = "Ты очень щедр"+GetSexPhrase("","а")+", "+GetSexPhrase("сын мой","дочь моя")+". Скромность, подобающая слугам Господа нашего, не позволяет мне принять всю сумму, которой ты готов"+GetSexPhrase("","а")+" пожертвовать ради нашей церкви. Я возьму только 10 000 песо\nИ в благодарность я хочу предупредить тебя, "+GetSexPhrase("сын мой","дочь моя")+": нехорошие люди затеяли против тебя недоброе. Я слышал, как эти мерзкие богоотступники договаривались потопить твой корабль при выходе из нашего порта\nОни снарядили люггер, под названием '"+pchar.questTemp.Regata.BranderName+"', и намереваются атаковать тебя, под английским флагом, чтобы ты не понял"+GetSexPhrase("","а")+", что к чему. Будь осторож"+GetSexPhrase("ен","на")+", "+GetSexPhrase("сын мой","дочь моя")+", эти заблудшие души не шутили - следует ожидать нападения\nУничтожь их - я заранее отпускаю тебе этот грех. Ступай с моим благословением! Я буду молиться за тебя, "+GetSexPhrase("сын мой","дочь моя")+".";
				link.l1 = "Спасибо, святой отец. Я буду внимател"+GetSexPhrase("ен","ьна")+". Если эти мерзавцы осмелятся - я отправлю их прямиком в ад!";
				link.l1.go = "Padre_exit";
		...
		case "Regata_Padre_exit":
			dialog.text = "Не гневайся, "+GetSexPhrase("сын мой","дочь моя")+". Гнев - это грех. Если ты не желаешь пожертвовать на благо церкви - то это твоё решение и твоё право. Ступай с миром, "+GetSexPhrase("сын мой","дочь моя")+".";
		...
		case "Regata_Spyglass_1":
			dialog.text = "Ну конечно же, капитан! А чему вы удивляетесь? Вы - "+GetSexPhrase("моряк видный","морячка видная")+", участвуете в регате - конечно, все хотят вам что-нибудь продать, необходимое в морском деле.";
		...
		case "Regata_Spyglass_2":
			...
			dialog.text = "Вот, посмотрите. Она мне досталась от отца, он тоже был моряком, как и вы. Мне она не нужна, а вам может пригодиться. И прошу я за неё всего лишь "+FindRussianMoneyString(sti(pchar.questTemp.Regata.SpyPrice))+".";
			if (sti(pchar.money) >= sti(pchar.questTemp.Regata.SpyPrice))
			{
				link.l1 = "Так-так... "+sTemp+"! Очень интересно. Пожалуй, я куплю её у вас. В том числе, и за ваши красивые глаза. Вы хотите за неё "+FindRussianMoneyString(sti(pchar.questTemp.Regata.SpyPrice))+"? Пожалуйста, получите.";
		...
		case "Regata_Spyglass_exit":
			dialog.text = "Жаль, очень жаль, капитан. А я надеялась, что она вам пригодится. Ну, попробую продать её кому-нибудь ещё. До свидания!";
		...
		case "Regata_SiegeOfficer":
			...
			link.l1 = "Да. А в чём дело, офицер?";
		...
		case "Regata_SiegeOfficer_2":
			dialog.text = "Самое прямое, капитан. Давайте не будем терять времени и пройдём в резиденцию. Следуйте за мной.";
		...
		case "Regata_SiegeOfficer_4":
			dialog.text = ""+GetSexPhrase("Сэр","Мисс")+"! Я готов препроводить гражданское население вам на корабль. Мы отправляемся?";
		...
		case "Regata_SiegeOfficer_8":
			dialog.text = "Спасибо вам огромное за помощь, капитан! Если бы не вы, и не ваша отвага... Вы "+GetSexPhrase("настоящий герой","настоящая героиня")+" - невзирая на опасности, пренебрегая своими личными целями, блестяще выполнили операцию. Я тут подумал по дороге... есть возможность отблагодарить вас  - ведь вы участвуете в регате.";
		...
		case "Regata_Hovernor":
			...
			link.l1 = "Сэр, если речь идёт про напавшую на вашу колонию испанскую военную эскадру, то я не представляю, в чём будет выражаться моя помощь. Я командую люггером, а не линейным кораблём...";
			link.l1.go = "Regata_Hovernor_1";
		break;
		
		case "Regata_Hovernor_1":
			dialog.text = "Выслушайте меня, капитан, и вы всё поймёте. Мы попали в очень сложное положение. Город с моря осаждает испанская эскадра, и вот-вот начнёт решающий бой с фортом. Кроме того, испанцы высадили на маяке десантную группировку из нескольких сотен человек и блокировали выходы из города в джунгли, видимо, для того, чтобы атаковать нас с двух сторон - с моря и суши\nГарнизон форта готовится к обороне с моря, а солдат в городе недостаточно - мы не можем снять блокаду с суши... Всё мужское население, способное носить оружие, мобилизовано, и все готовы к решающей схватке, но у нас в городе не только мужчины\nВ Бриджтауне остались почти сто душ женского населения. Вы представляете, что с ними будет, если мы не сможем отстоять колонию и испанцы захватят город? Думаю, подробно рассказывать вам не нужно.";
			link.l1 = "Сэр, это всё понятно, но чем я могу вам помочь? Чего вы хотите от меня? Чтобы я вступил"+GetSexPhrase("","а")+" в бой с линейными кораблями или записал"+GetSexPhrase("ся","ась")+" со своей командой в городское ополчение?";
		...
		case "Regata_Hovernor_2":
			...
			{
				case ENGLAND: sTemp = "Вы же "+GetSexPhrase("англичанин","англичанка")+", помогите спасти английских мирных граждан, тем более - женщин!" break;
		...
		case "Regata_Hovernor_3":
			dialog.text = "Я рад, что вы согласились. Вы настоящий солдат. Лейтенант Махойни займётся доставкой пассажирок на ваш корабль. И да поможет вам Бог!";
			link.l1 = "Спасибо, сэр. Не будем терять драгоценное время!";
			link.l1.go = "Regata_Hovernor_4";
		break;
		
		case "Regata_Hovernor_wait":
			dialog.text = "Поторопитесь, капитан! Время не ждёт.";
		...
		case "Regata_Hovernor_exit":
			dialog.text = "Я просто поражён... Вы не хотите даже попытаться? Ну что же, это ваше право. Я не могу ничего от вас требовать. Будь вы простым капитаном, я бы не колеблясь конфисковал ваше судно ради спасения сотни женщин, но... вы находитесь под протекцией генерал-губернатора английских колоний как участник регаты\nЯ надеялся на вашу совесть и просчитался. Ступайте, и да смилуется Господь над вашей душой в день Страшного Суда!";
		...
		case "Regata_BridgWom_1":
			dialog.text = "Лейтенант Махойни погиб... какой кошмар. Он был отважным офицером. Капитан, вы проведёте нас в пещеру? Только доведите, а дальше мы сами о себе позаботимся.";
		...
		case "Regata_skiper":
			...
			dialog.text = "Ваш путь лежит в Порт-Рояль? Самый короткий путь - через Кюрасао. Вернитесь в Бриджтаун. Там найдите человека по имени Вудро Доггерти - он старый моряк и опытный лоцман. Он как свои пять пальцев знает воды между Барбадосом и Кюрасао и проведёт ваш корабль кратчайшим путём\nВы сэкономите на этом время, столь необходимое вам для победы. Найти Вудро вы скорее всего сможете в церкви. Скажете, что вас прислал"+ NPCharSexPhrase(NPChar, "","а") +" " + GetFullName(npchar) + " - он вам не откажет. Высадите его потом в Порт-Рояле - он всё равно туда собирался.";
		...
		case "Regata_SiegeSkiper_1":
			...
			link.l1 = "Мне известно, что вы опытный штурман и хорошо знаете воды между Барбадосом и Кюрасао. Также мне известно, что вам нужно в Порт-Рояль. Я - участник регаты, мне предстоит последний переход как раз до Порт-Ройаля, и я хочу попросить вас войти в мою команду в качестве штурмана и лоцмана на этот переход.";
		...
		case "Regata_SiegeSkiper_2":
			...
			link.l1 = "Ну, потому что я узнал"+GetSexPhrase("","а")+" это от человека по имени "+pchar.questTemp.Regata.Name+", с уверением, что вы мне не откажете. И потому что я, как-никак, оказал"+GetSexPhrase("","а")+" услугу городу Бриджтаун в этот трудный момент...";
		...
		case "Regata_SiegeSkiper_3"://а тут - как уж свезет
			if(drand(2) != 2)
			{
				dialog.text = "От "+pchar.questTemp.Regata.Name+"? Это, конечно, всё проясняет. Хорошо, капитан, я помогу проложить вам кратчайший путь от Бриджтауна до Порт-Ройаля. Вы отправляетесь прямо сейчас?";
		...
		case "Regata_SiegeSkiper_6":
			dialog.text = "Капитан, регата - это спорт, а осада испанцами моего города - это жизнь. Я никуда с вами не поплыву. И повторюсь - вы должны меня понять как никто другой - вам известны понятия 'честь' и 'долг'. Это моё окончательное решение и менять его я не намерен.";
		...
		if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.Regata.Breach") || !CheckAttribute(pchar, "questTemp.Regata.Bridgetown") || GetCompanionQuantity(pchar) > 1 || !bRegLugger || pchar.Ship.Name != "Сaнта-Kатepина")
			{
			...
			else
			{
				...
				else
				{
					...
					{
						dialog.text = "Капитан, вы заняли "+n+" место. Поздравляю с успешным прохождением маршрута, вы показали достойное время - "+sti(pchar.questTemp.Regata.FifthTransitionTime)+" часов. И хоть вы и не выиграли, я всё равно поздравляю вас!";
		...
		case "First_result":
			...
			else sTemp = "А если бы вы сделали ставку на свою победу, как я вам предлагал - получили бы ещё и выигрыш.";
			...
			link.l1 = "Я всё понял"+GetSexPhrase("","а")+", сэр. Буду завтра на приеме!";
		...
		case "First_result_1":
			...
			// слухи
			AddSimpleRumour("А, так это вы - легендарный капитан, победитель"+GetSexPhrase("","ница")+" парусной регаты! Очень приятно вас видеть!", ENGLAND, 60, 10);
			AddSimpleRumour("Ха, да это же не кто иной, как повелитель ветра и волн, покоритель морских просторов, победитель"+GetSexPhrase("","ница")+" парусной регаты! Моё вам уважение...", ENGLAND, 60, 10);
			AddSimpleRumour("О, сам"+GetSexPhrase("","а")+" велик"+GetSexPhrase("ий","ая")+" мореплаватель"+GetSexPhrase("","ница")+", корол"+GetSexPhrase("ь","ева")+" ветра и моря, победитель"+GetSexPhrase("","ница")+" парусной регаты почтил"+GetSexPhrase("","ница")+" наш город своим визитом! Мы рады вам, капитан!", ENGLAND, 60, 10);
			AddSimpleRumour("О, да мне посчастливилось беседовать с настоящей живой легендой, капитаном, способн"+GetSexPhrase("ым","ой")+" за три дня пересечь архипелаг даже при встречном ветре, победител"+GetSexPhrase("ем","ьницей")+" парусной регаты! Моё почтение, капитан!", ENGLAND, 60, 10);
			AddSimpleRumour("А, так это вы - легендарный капитан, победитель"+GetSexPhrase("","ница")+" парусной регаты! Очень приятно вас видеть!", FRANCE, 60, 10);
			AddSimpleRumour("Ха, да это же не кто иной, как повелитель ветра и волн, покоритель морских просторов, победитель"+GetSexPhrase("","ница")+" парусной регаты! Моё вам уважение...", FRANCE, 60, 10);
			AddSimpleRumour("О, сам"+GetSexPhrase("","а")+" велик"+GetSexPhrase("ий","ая")+" мореплаватель"+GetSexPhrase("","ница")+", корол"+GetSexPhrase("ь","ева")+" ветра и моря, победитель"+GetSexPhrase("","ница")+" парусной регаты почтил"+GetSexPhrase("","ница")+" наш город своим визитом! Мы рады вам, капитан!", FRANCE, 60, 10);
			AddSimpleRumour("О, да мне посчастливилось беседовать с настоящей живой легендой, капитаном, способн"+GetSexPhrase("ый","ой")+" за три дня пересечь архипелаг даже при встречном ветре, победител"+GetSexPhrase("ем","ьницей")+" парусной регаты! Моё почтение, капитан!", FRANCE, 60, 10);
			// экспа
		...
		case "Other_result_repeat":
			dialog.text = "Вы что-то ещё хотели, капитан?";
		...
		case "Give_advantage":
			dialog.text = "А-а, вот и вы, уважаем"+GetSexPhrase("ый","ая")+" "+GetFullName(pchar)+". Рад вас видеть. Вы пришли за вашим выигрышем? Он подготовлен.";
			link.l1 = "Приятно это слышать! Каков же размер моего дополнительного приза?";
			link.l1.go = "Give_advantage_1";
		break;
		
		case "Give_advantage_1":
			dialog.text = "С учётом сделанных ставок, ваш выигрыш составляет "+FindRussianMoneyString(makeint(pchar.questTemp.Regata.Advantage))+". Пожалуйста, получите. Поздравляю ещё раз с победой!";
			link.l1 = "Спасибо!";
		...
		case "Give_advantage_2":
			...
			dialog.text = "Безусловно, ваша популярность среди жителей Порт-Ройаля выросла. Думаю, вы вполне сможете это заметить... правда, память у людей короткая, увы. Так что купайтесь в лучах славы, пока они ещё яркие!";
			link.l1 = "Интересные вещи вы говорите, сэр... Посмотрим, каково это - принимать ванны из лучей славы. До свидания! Рад"+GetSexPhrase("","а")+" был с вами познакомиться!";
			link.l1.go = "exit";
			NextDiag.TempNode = "Other_result_repeat";
		break;
		...
		case "Regata_CitMan_1":
			if (CheckAttribute(pchar, "questTemp.Regata"))
			{
				if (CheckAttribute(pchar, "questTemp.Regata.Result"))
				{
					...
					{
						dialog.text = "Не расстраивайтесь, капитан. Победить в регате - сложно, и даже просто участие в ней принесло вам почёт!";
