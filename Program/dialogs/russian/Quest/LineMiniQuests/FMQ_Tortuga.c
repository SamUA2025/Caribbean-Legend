...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "girl":
			...
			link.l1 = "Приветствую, мадемуазель. Спасибо за тёплый приём. Но как так? Мне казалось, что настолько значимую колонию французской короны должны регулярно удостаивать визитом люди много достойнее меня.";
			link.l1.go = "girl_1";
		break;
		...
		case "wife":
			DelLandQuestMark(npchar);
			PlaySound("VOICE\Russian\LE\Marselina\Marselina_Levasser_01.wav");
		...
		case "wife_4":
			...
			link.l1.go = "wife_5";
			link.l2 = "Хорошо, красавица, я добуду содержимое сундука. Мы получим деньги и очень приятно проведём время. Какой мужчина от такого откажется? Где этот сундук и как его открыть?";
			link.l2.go = "wife_6";
		...
		case "wife_12":
			...
			link.l1.go = "wife_13";
			link.l2 = "Ха! Ради такой женщины, я сделаю это. Ты не пожалеешь, красотка. Мы получим деньги и очень приятно проведём время. Где этот сундук и как его открыть?";
			link.l2.go = "wife_14";
		...
		case "wife_21":
			...
			PlaySound("VOICE\Russian\LE\Marselina\Marselina_Levasser_02.wav");
		...
		case "wife_24":
			PlaySound("VOICE\Russian\LE\Marselina\Marselina_Levasser_03.wav");
		...
		case "mercen":
			PlaySound("VOICE\Russian\LE\Duran\Klod_Duran_chertovy_baby.wav");
			...
			//if(CheckCharacterPerk(pchar, "IronWill") || CheckCharacterPerk(pchar, "HT2") || GetSummonSkillFromName(pchar, SKILL_LEADERSHIP) > 35 || GetSummonSkillFromName(pchar, SKILL_FORTUNE) > 20) link.l1.go = "mercen_1";
			if(GetSummonSkillFromName(pchar, SKILL_LEADERSHIP) > 34) link.l1.go = "mercen_1";
			else link.l1.go = "mercen_4";
		break;
		
		case "mercen_1":
			//if(CheckCharacterPerk(pchar, "IronWill")) notification("Железная воля", "IronWill");
			//if(CheckCharacterPerk(pchar, "HT2")) notification("Мимикрия", "Mimicry");
			if(GetSummonSkillFromName(pchar, SKILL_LEADERSHIP) > 34) notification("Проверка пройдена", SKILL_LEADERSHIP);
			//if(GetSummonSkillFromName(pchar, SKILL_FORTUNE) > 20) notification("Проверка пройдена", SKILL_FORTUNE);
		...
		case "mercen_2":
			PlaySound("VOICE\Russian\LE\Duran\Klod_Duran_pinka.wav");
		...
		case "mercen_4":
			//if(!CheckCharacterPerk(pchar, "IronWill")) notification("Не открыта способность", "IronWill");
			if(GetSummonSkillFromName(pchar, SKILL_LEADERSHIP) < 35) notification("Недостаточно развит навык (35)", SKILL_LEADERSHIP);
			//if(GetSummonSkillFromName(pchar, SKILL_FORTUNE) < 21) notification("Недостаточно развит навык (21)", SKILL_FORTUNE);
			PlaySound("VOICE\Russian\LE\Duran\Klod_Duran_nesladko.wav");
