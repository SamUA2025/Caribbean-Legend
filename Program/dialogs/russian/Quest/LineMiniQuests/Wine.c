void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Wine_fort":
			dialog.text = "Ну что, прин"+GetSexPhrase("ёс","есла")+" бутылочку?";
			...
			link.l2 = "Нет, я ещё этим занимаюсь.";
			link.l2.go = "exit";
		...
		case "Wine_fort_1":
			...
			dialog.text = "Аргх, спасибо тебе, "+GetSexPhrase("дружище","красавица")+"! Вот, держи свою тысячу, а эта бутылочка теперь моя, ха-ха-ха! Послушай, раз уже такое дело пошло, то у меня к тебе будет ещё одна просьба\nНе мог"+GetSexPhrase("","ла")+" бы ты раздобыть в городе ещё вина, для меня и всех моих друзей, про запас? Мы тут недавно хорошо контрабандистов пощипали, так что деньги у нас имеются, хе-хе...";
	...
	case "Wine_fort_exit":
			...
			WineTraderQMDel();
			dialog.text = "Ну, как скажешь. Всё равно спасибо за помощь!";
	...
	break;
		...
		case "Wine_fort_2":
			dialog.text = "Ну, при учёте цены в тысячу за бутылку, штук шестьдесят бутылок мы сможем себе позволить, но не больше. Ну и меньше десяти тоже не приноси - несерьёзно получится.";
		...
		case "Wine_fort_3":
			...
			AddQuestUserData("Wine", "sSex1", GetSexPhrase("ся","ась"));
		...
		case "Wine_fort_check":
			...
			{
				link.l1 = "Да, прин"+GetSexPhrase("ёс","есла")+".";
		...
		case "Wine_take":
			...
			if (sti(pchar.items.potionwine) > 60)
			{
				...
				link.l1 = "Забирайте. Уверен"+GetSexPhrase("","а")+", вы с друзьями неплохо отдохнёте!";
			...
			else
			{
				...
				link.l1 = "Держите ваше вино. Уверен, вы с друзьями неплохо отдохнёте!";
				link.l1.go = "Wine_take_1";
