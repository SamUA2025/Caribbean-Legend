...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "cureer":
			dialog.text = "Месье де Мор, какая удача! Я уже не первый месяц вас ищу!";
		...
		case "cureer_1":
			dialog.text = "Ох, месье, я... нет, я не продаю барахло и не убиваю людей - оставьте шпагу, она вам не понадобится! Всё гораздо прозаичнее - я капитан курьерского судна. Почту развожу. И для вас, как раз, важная корреспонденция!";
		...
		case "cureer_4":
			...
			dialog.text = "Приятно иметь с вами дело! Аббат Бенуа передаёт свои наилучшие пожелания! Прощайте, месье де Мор!";
			link.l1 = "";
			link.l1.go = "cureer_5";
		break;
		...
		case "spa_officer_1":
			if(sti(pchar.reputation.nobility) > 80 || sti(pchar.reputation.nobility) < 20) // провести учёт чести и известности
			{
				...
				link.l1 = "Вы поступили разумно месье. А недовольство отца-инквизитора я как-нибудь переживу. Хорошего вечера!";
				link.l1.go = "spa_officer_2";
			}
			else
			{
				notification("Слишком низкий уровень чести! ("+XI_ConvertString(GetReputationName(81))+")", "None");
				notification("Слишком высокий уровень чести! ("+XI_ConvertString(GetReputationName(19))+")", "None");
		...
		case "Svenson_13":
			dialog.text = "Похоже, пора открывать то, что привёз ты. Единственное - те ребята что мне помогали поначалу - напились в задней комнате до синих акул. Ты вроде ещё на ногах держишься, женишок - сможешь дотащить ящик другой хереса?";
		...
		case "Svenson_17":
			...
			link.l1 = "Нет, Ян, мы с Хоуком дрались заодно. Там какая-то чертовщина творится. В катакомбах были головорезы, вооружённые до зубов. Мы их уложили, но это лишь разведка - следом идёт основной отряд.";
		...
		// Форже - патер Мэри
		case "forge":
			dialog.text = "Месье де Мор, мы так и не перешли на 'ты', так что прошу прощения за фамильярность, но я скажу это - Шарль, я искренне за тебя рад! Если я могу что-то сделать для тебя или твоей невесты, только скажи!";
		...
		case "Tichingitu_1":
			...
			else link.l1 = "Боюсь, до танцев у костров дойдёт задолго до конца, а без абордажа я предпочёл бы обойтись. Ладно, держись - осталось недолго, и сразу отплываем в Шарптаун.";
			link.l1.go = "Guests_block";
		break;
		...
		case "Tichingitu_14":
			...
			link.l1 = "Настоящая баталия на суше и на море. Пушки всю ночь палили так, что наверное, слышали даже в Сан Хосе. Впрочем, вам было не до того, понимаю... Идём, я всё расскажу по дороге.";
		...
		case "FortFrance_Mayor":
			...
			else
			{
				...
				link.l1 = "Месье, вам ещё далеко до старости, не прибедняйтесь. Спасибо, что пришли, Жак!";
				link.l1.go = "Guests_block";
			}
		break;
		...
		// Порт Принс
		case "PortPax_Mayor":
			dialog.text = "Просто чудесная служба, месье де Мор, просто чудесная! И невеста такая... оригинальная в этом наряде. От лица всех жителей Порт-о-Пренса, прошу - примите мои самые искренние поздравления! Совет да любовь!";
		...
		// Тортуга
		case "Tortuga_Mayor":
			dialog.text = "Очень мило - вы и ваша супруга, месье де Мор, смотрелись так необычно! Впрочем - вы и человек необычный. Желаю вам долгой и счастливой семейной жизни. Совет да любовь!";
		...
		case "PortRoyal_Mayor":
			PlaySound("Voice\Russian\LE\Doily\Edvard_Doily_03.wav");
		...
		case "Nathaniel_8":
			PlaySound("ambient\jail\jail_door2.wav");
			dialog.text = "Наверное, Ян нашёл ещё ребят и послал нам в помощь. Проклятье!";
		...
		case "Jino_5":
			...
			link.l1 = "Уверен, это были последние. Джино, в гавани идёт бой, и мне срочно нужно туда. До резиденции тебе придётся дойти самостоятельно. Большой дом тут за углом - ты его не пропустишь. Не бойся - на улицах полно наших людей, и поспеши, умоляю!";
			link.l1.go = "Jino_6";
		break;
		...
		// Ангерран
		case "Shivo":
			dialog.text = "Месье де Мор, добрый вечер, или - уже почти утро? Проходите, присаживайтесь.";
		...
		case "Shivo_2":
			dialog.text = "И вы, месье пират, вы тоже опустите оружие, вот так, да. А теперь - располагайтесь, можете налить вина - вон там на столе. Нам придётся немного подождать, пока месье Барбазон закончит разносить в щепки корабли ваших друзей - пиратских баронов. Потом решим, как мы с вами уладим наши... гм, разногласия.";
			link.l1 = "Уладим разногласия? И как же? Вы выйдете со мной один на один?";
			link.l1.go = "Shivo_3";
		break;
		
		case "Shivo_3":
			dialog.text = "Не будьте смешным, Шарль. Вы с мадам Бото доставили мне массу неприятностей, и я намерен отплатить вам сполна. Если вы и импозантный месье пират будете сидеть тихо, то даю вам слово чести - ваши жёны останутся живы. Думаю, в испанских колониях на Мэйне дорого оценят таких рабынь. Пусть и неофициально. Но это всё-таки жизнь, верно?";
		...
		case "Dussak_2":
			...
			link.l1 = "И что потом, а месье Мориц? Обнимемся и вместе отправимся в закат?";
			link.l1.go = "Dussak_3";
		break;
		...
		case "waitress_1":
			...
			{
				notification("Недостаточно развит навык (95)", SKILL_LEADERSHIP);
			}
		...
		case "waitress_3":
			...
			link.l1 = "Ну-у, возможно, мне удастся его переубедить. Но это будет непросто. Маскоги очень серьёзно относятся к таким вещам - если кто-то узнает, что он похитил скво и не женился...";
