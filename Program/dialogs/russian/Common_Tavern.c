...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				dialog.text = "Чем могу служить, капитан? Вам бы наверняка хотелось передохнуть после случившегося.";
				Link.l1 = "Это завсегда успеется. А вот как раз случившееся я и хочу обсудить.";
			...
			{
				dialog.Text = GetNatPhrase(npchar, LinkRandPhrase("С прибытием в таверну '" + XI_ConvertString(NPChar.City + "TavernName") + "', " + GetSexPhrase("сэр","мисс") + ". Я рад видеть вас.","К вашим услугам, " + GetSexPhrase("сэр","мисс") + "! В таверне '" + XI_ConvertString(NPChar.City + "TavernName") + "' рады новым гостям.","Вижу, вы впервые у меня, " + GetSexPhrase("сэр","мисс") + "? Не стесняйтесь, в таверне '" + XI_ConvertString(NPChar.City + "TavernName") + "' вы всегда почувствуете себя, как дома!"), LinkRandPhrase("Прошу вас, " + GetSexPhrase("месье","мадмуазель") + " капитан. Вы впервые посетили таверну '" + XI_ConvertString(NPChar.City + "TavernName") + "'?","Не встречал вас раньше, " + GetSexPhrase("месье","мадмуазель") + ". Будем знакомы: я "+GetFullName(npchar)+", хозяин таверны '" + XI_ConvertString(NPChar.City + "TavernName") + "'.","Я рад новому гостю, " + GetSexPhrase("месье","мадмуазель") + " капитан! В таверне '" + XI_ConvertString(NPChar.City + "TavernName") + "' первое угощение всегда за мой счёт! Я - хозяин этого заведения, "+GetFullName(npchar)+", будем знакомы."), LinkRandPhrase("" + GetSexPhrase("Сеньор","Сеньорита") + " капитан! Добро пожаловать в таверну '" + XI_ConvertString(NPChar.City + "TavernName") + "'!","О, " + GetSexPhrase("какой блестящий идальго","какая блестящая сеньорита") + " посетил"+GetSexPhrase("","а")+" сегодня таверну '" + XI_ConvertString(NPChar.City + "TavernName") + "'! Вы впервые у нас, " + GetSexPhrase("сеньор","сеньорита") + "?","Здравствуйте, " + GetSexPhrase("кабальеро","сеньорита") + "! Я "+GetFullName(npchar)+", хозяин таверны '" + XI_ConvertString(NPChar.City + "TavernName") + "', рад приветствовать вас у себя!"),LinkRandPhrase("Здравствуйте, " + GetSexPhrase("минхер","госпожа") + " капитан. Я хозяин таверны '" + XI_ConvertString(NPChar.City + "TavernName") + "'. Впервые у нас?","Добрый день, " + GetSexPhrase("минхер","госпожа") + ". Добро пожаловать в таверну '" + XI_ConvertString(NPChar.City + "TavernName") + "'.","Очень приятно, " + GetSexPhrase("минхер","госпожа") + " капитан! Таверна '" + XI_ConvertString(NPChar.City + "TavernName") + "' приветствует вас! Рому, вина, эля?"));
				Link.l1 = LinkRandPhrase("А у вас тут уютно... Позвольте представиться - "+GetFullName(pchar)+", рад" + GetSexPhrase("","а") + " знакомству.",""+GetFullName(pchar)+", рад" + GetSexPhrase("","а") + " знакомству. Что вы мне предложите?","Моё имя "+GetFullName(pchar)+" и я здесь совсем недавно. Чем угостите гост" + GetSexPhrase("я","ью") + "?");
				Link.l1.go = "meeting";
/*				
				if(!CheckAttribute(pchar, "questTemp.TavernVisit."+(NPCCity) )) 
				{
...
*/				if(startHeroType == 4 && NPChar.location == "SantaCatalina_tavern")
				{
					dialog.text = "Здравствуй, Румба! Ты из похода? Или собираешься в новый?";
					link.l1 = "Здравствуй, "+npchar.name+". Просто так решила зайти.";
			...
			case "step_node":
				...
				{
					link.l19 = "Я приш"+GetSexPhrase("ёл","ла")+" за своим выигрышем, который хранится у тебя, " + npchar.name + ".";
		...
		case "Helen_meeting":
			dialog.Text = "А, ну проходи-проходи, тебе у нас всегда рады.";
			link.l1 = "Да, спасибо. Рада была видеть. Бывай.";
			link.l1.go = "exit";
			Link.l2 = "Мне бы не помешали новые ребята. Только подбери толковых, хорошо?";
			Link.l2.go = "crew hire";
			link.l3 = "Я бы хотела передохнуть на твёрдой земле и собраться с мыслями, а не когда мама по дому суетится.";
			link.l3.go = "Helen_room";
			Link.l4 = "Можно кое что у тебя спросить?";
			Link.l4.go = "quests";
			link.l5 = "У меня есть к тебе дело, "+ NPChar.name + ".";
	...
	////////////////////////////////////////////////////////=============================================///////////////////////////////////////////////////////	
		...
		case "work_2":
    		...
    		{					
				...
				{
					notification("Недостаточно развит навык (5)", SKILL_LEADERSHIP);
		...
		case "Helen_room":
			dialog.text = "Конечно, Элен. А на какой срок? И что, вы со старушкой Глэдис поссорились?";
			if(!isDay())
			{
				link.l1 = "Да просто поспать до утра. А если сейчас зайду в дом, то непременно разбужу её.";
				link.l1.go = "Helen_room_day";
			}
			else
			{
				link.l1 = "Нет-нет, мне просто нужно отчалить в ночь, не хочу её будить.";
				link.l1.go = "Helen_room_night";
				link.l2 = "Хочу хорошенько выспаться и без спешки полежать ещё в кровати, когда проснусь. Я возьму комнату на сутки?";
		...
		case "room":
			...
			if (CheckAttribute(pchar, "GenQuest.Unwantedpostor") && !CheckAttribute(pchar, "questTemp.Sharlie.Lock") && abs(50-sti(pchar.reputation.nobility)) > 20)
			{
				...
				dialog.text = "Прошу простить, капитан, к сожалению, свободной комнаты нет. Постоялец всё никак не съедет. Ещё и деньги не платит...";
				link.l1 = "Видимо, придётся ночевать в общей зале...";
				link.l1.go = "Unwantedpostor_hall_day_wait";
				link.l2 = "Очень жаль. Давай поговорим о чём-нибудь другом.";
				link.l2.go = "int_quests";
				if(pchar.GenQuest.Unwantedpostor != "Lose")
				{
					link.l3 = "Так позови стражу. Что это за постоялец такой: денег не платит, съезжать не хочет...";
		...
		case "Marginpassenger_1":
			...
			{
				AddQuestUserData("Marginpassenger", "sBaron", "в Пуэрто-Принсипе, к Чёрному Пастору");
			}
			else
			{
		...
		case "Unwantedpostor":
				dialog.text = "Ага - и распугать всех посетителей тем, что у меня стража людей из таверны уводит? А часть моих посетителей и вовсе с солдатами якшаться не любит. Не хотелось бы репутацию портить...";
				link.l1 = "Хм, а что, если я решу твою проблему?";
				link.l1.go = "Unwantedpostor_1";
		break;
		
		case "Unwantedpostor_1":
				dialog.text = "Решите? Он ведь вооружён, наёмник вроде какой-то...";
				link.l1 = "Хм, ты прав, с вооружённым наёмником мне спорить что-то не хочется...";
				link.l1.go = "Exit";
				link.l2 = "Ну, я тоже не монахиня... И я всё ещё хочу снять комнату.";
				link.l2.go = "Unwantedpostor_2";
		break;
		
		case "Unwantedpostor_2":
				dialog.text = "Ну, если сможете его, кхм... убедить, чтобы он съехал, то до утра комната ваша. Бесплатно, разумеется.";
				link.l1 = "Знаешь, я передумал"+GetSexPhrase("","а")+". Не хочу никого убеждать.";
				link.l1.go = "Exit";
				link.l2 = "Идёт. Сейчас твой постоялец вылетит из таверны, как миленький...";
		...
		case "Helen_room_night":
			...
			link.l2 = "Хотя знаешь... В другой раз. Проведу остаток дня на ногах.";
		...
		case "Helen_room_day":
			...
			link.l2 = "Впрочем, что-то мне не спится. Похожу по городу, подышу ночным воздухом.";
		...
		case "Helen_room_day_next":
			...
			link.l2 = "Впрочем... я могла бы за целые сутки сделать много чего полезного. Так что в другой раз.";
			link.l2.go = "Helen_Next_day_exit";
		break;
		
		case "Helen_night_exit":
			dialog.text = "Как скажешь, Элен.";
			link.l1 = "Угу.";
			link.l1.go = "exit";
		break;
		
		case "Helen_day_exit":
			dialog.text = "Сон заодно нагуляешь.";
			link.l1 = "Ха-ха, возможно.";
			link.l1.go = "exit";
		break;
		
		case "Helen_next_day_exit":
			dialog.text = "Тебе виднее.";
			link.l1 = "Ну, до встречи.";
			link.l1.go = "exit";
		break;
		
		case "Helen_rumours_tavern":
			dialog.text = "Конечно, Элен, внимательно тебя слушаю.";
