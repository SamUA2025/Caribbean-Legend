	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if(NPChar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Капитан МакАртур, каждый раз напоминаю - уж вам нет необходимости отмечаться у меня в бумагах каждый раз, не волнуйтесь.";
					link.l1 = "Перестаньте, мистер "+npchar.lastname+ ". Я такая же, как и все.";
					Link.l1.go = "node_2";
					break;
				}
...
//-------------------------------------------//Jason, почта---------------------------------------------------	
				...
				{
					dialog.text = "Прекрасно. Я к вашим услугам, " + GetFullName(PChar) + ".";
					Link.l1 = "Я доставил"+GetSexPhrase("","а")+" почту для вашей колонии.";
...
//-------------------------------------------//Jason, эскорт---------------------------------------------------	
				...
				{
					dialog.text = "Прекрасно. Я к вашим услугам, " + GetFullName(PChar) + ".";
					Link.l1 = "Я наш"+GetSexPhrase("ёл","ла")+" пропавшее судно и доставил"+GetSexPhrase("","а")+" его в ваш порт. Думаю, его капитан уже доложил вам об этом.";
				...
				{
					dialog.text = "Прекрасно. Я к вашим услугам, " + GetFullName(PChar) + ".";
					Link.l1 = "Я капитан судна сопровождения. Я доставил"+GetSexPhrase("","а")+" в порт торговые корабли и хочу отчитаться.";
...
// <-- эскорт
			...
			{
				NPChar.quest.meeting = "1";
				dialog.text = "Как приятно слышать, что это действительно так. Многие бы зазнались на вашем месте. Итак, чем я и мои записи могут быть вам полезны?";
			}
			else dialog.text = "Прекрасно. Я к вашим услугам, " + GetFullName(PChar) + ".";
			...
			link.l61 = "Вы можете рассказать что-нибудь об уникальных кораблях и их легендарных капитанах?";
		...
		case "Work_check"://фейс-контроль - общая проверка профпригодности ГГ и выбор задания
			...
			if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
			{
				if (!CheckAttribute(npchar, "work_date") || GetNpcQuestPastDayParam(npchar, "work_date") >= 2)//проверка повтора
				{
					...
					link.l2 = "Я хочу подзаработать эскортом купцов или доставкой почты.";
					Link.l2.go = "Check_other";		//доставка почты, эскорт
					Link.l3 = "Ну, может вы что-нибудь сами предложите?";
				...
				case "cureer":
					if (pchar.questTemp.WPU.Postcureer == "begin" || pchar.questTemp.WPU.Postcureer == "late" || pchar.questTemp.WPU.Postcureer == "lost" || pchar.questTemp.WPU.Postcureer == "fail" || CheckAttribute(pchar, "questTemp.WPU.Postcureer.LevelUp")) { // если заняты
						...
						if (sti(pchar.questTemp.WPU.Postcureer.count) > 3 && drand(1) == 1) { //если 2 уровень
							...
							link.l1 = "Это интересно. Пожалуй, я соглас"+GetSexPhrase("ен","на")+". Куда нужно отправиться?";
					...
					if (pchar.questTemp.WPU.Escort == "begin" || pchar.questTemp.WPU.Escort == "late" || pchar.questTemp.WPU.Escort == "win" || CheckAttribute(pchar, "questTemp.WPU.Escort.LevelUp")) { // если заняты
						...
						if (sti(pchar.questTemp.WPU.Escort.count) > 3 && drand(1) == 1) { // 2 уровень
							...
							if (sti(RealShips[sti(pchar.Ship.Type)].BaseType) == SHIP_GALEON_H && 2500 - makeint(GetCharacterFreeSpace(pchar, GOOD_RUM)) < 0 && !CheckAttribute(pchar, "questTemp.WPU.Fraht.TargetPortmanID")) { // если на ТГ
								dialog.text = "У меня есть работа, как раз для вас. В нашем порту стоят на рейде два торговых судна. Они давно должны были отправиться в путь, но корабль сопровождения получил повреждения и починится ещё не скоро\nВаш корабль идеально подходит для эскорта этих судов, так как мне необходимо разместить ещё дополнительную партию груза в вашем трюме. Оплата, безусловно, будет двойной - за фрахт и за сопровождение.";
...
/// -------------------------------работа курьером по доставке почты-------------------------------------
		...
		case "Postcureer":
			...
			link.l2 = "Гм... но мне совсем в другую сторону. Вынужден"+GetSexPhrase("","а")+" отказаться.";
			link.l2.go = "exit";
		break;
	...
	case "Escort_LevelUp":
		...
		switch (drand(2))
		{
			case 0://приключения на необитаемых островах
				...
				link.l1 = "Конечно же я соглас"+GetSexPhrase("ен","на")+"!";
				link.l1.go = "Escort_LevelUp_1";
				link.l2 = "Сожалею, но мне это не подходит. Вынужден"+GetSexPhrase("","а")+" отказаться.";
	...
	case "Escort_LU2_complete":
		switch (sti(pchar.questTemp.WPU.Escort.LevelUp_2.Qty))
		{
			...
			case 3:
				...
				link.l1 = "Благодарю за лестную оценку моих действий! Всегда рад"+GetSexPhrase("","а")+" помочь. А теперь, " + GetAddress_FormToNPC(NPChar) + " позвольте откланяться.";
				link.l1.go = "Escort_LU2_complete_1";
			break;
		}
	break;
...
//-------------------------------- квест доставки судового журнала до рассеяного кэпа ---------------------
		...
		break;
		case "SeekShip_2":
			dialog.text = XI_ConvertString(npchar.quest.PortmansSeekShip.shipTapeName+"Acc") + " '" + npchar.quest.PortmansSeekShip.shipName + "' украли ночью, " + FindRussianDaysString(rand(5)+10) + " назад. Вахтённый убит.";
...
*/
		...
		case "ShipStockReturn_1": // hasert сторож
            ok = (sFrom_sea == "") || (Pchar.location.from_sea == sFrom_sea);
		    if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)
			{
					...
					for(i=1; i<MAX_CHARACTERS; i++)
					{
						...
						{    
							if (chref.ShipInStockMan == NPChar.id)
							{
								...
								{  
								Link.(attrL)	= XI_ConvertString(RealShips[sti(chref.Ship.Type)].BaseName) + " '" + chref.Ship.Name + "' и мой офицер " + GetFullName(chref) + ".";
		...
		case "ShipStockManBack2": // hasert новый кейс для сторожа.
			...
			if (q == 0)
				{
				Link.(attrL) = RandSwear() + "Я забыл" + GetSexPhrase("","а") + " привести с собой офицера для этого корабля.";
				Link.(attrL).go = "exit";
				}
			else
				{
		...
		case "ShipStock_HWICHoll_1":
			dialog.text = "Да. О своём корабле не беспокойтесь - мы о нём позаботимся. Можете спокойно отправляться к минхеру Роденбергу.";
		...
		case "ShipStockManMutiny":
			...
			NPChar.Portman    = sti(NPChar.Portman) - 1;
            pchar.ShipInStock = sti(pchar.ShipInStock) - 1;
			dialog.text = "Вахтённый офицер " + GetFullName(chref) + " сообщил" + NPCharSexPhrase(chref, "", "а") + ", что " + NPCharSexPhrase(chref, "ему", "ей") + " немедленно нужно было выйти в море по вашему приказу. Я не стал " + NPCharSexPhrase(chref, "ему", "ей") + " препятствовать.";
		...
		case "UniqueShips":
			dialog.text = "Штучные корабли и люди, которым они доверены, не могут остаться на архипелаге незамеченными. Однако, эта информация весьма ценна и ей не разбрасываются.";
			if (sti(pchar.Money) >= 25000)
			{
				link.l1 = "Я понимаю. 25000 песо будет достаточно для демонстрации моих серьёзных намерений?";
				link.l1.go = "UniqueShips_2";
			}
			else
			{
				link.l1 = "Ладно, тогда в другой раз.";
				link.l1.go = "node_2";
			}
		break;
		...
		case "UniqueShips_SantaMisericordia":
			...
			dialog.text = "Святое Милосердие было послано Эскориалом из Европы для надзора за испанскими колониями. Его капитан, дон Аламида - прославленный слуга испанской короны, безжалостный борец с коррупцией и преступностью, ревнитель католической веры. Святое Милосердие курсирует между колониями, начиная и завершая свой путь в Гаване, не отвлекаясь ни на что\nВ каждом порту Аламида проводит по нескольку дней, иногда дольше. Говорят, в эти моменты его можно даже встретить на улице, но местные не особо горят желанием - в испанцев сеньор вселяет лишь страх и трепет. На Святом Милосердии опытная команда, отобранная Аламидой лично. Каждый из матросов готов защищать Родину и капитана ценой своей жизни\nГоворят, дон Фернандо отдал приказ ни при каких обстоятельствах не допустить захвата корабля, и есть основания полагать, что если экипаж столкнётся с угрозой абордажной схватки с превосходящими силами противника, они предпочтут отправить 'Святое Милосердие' на дно вместе с ним.";
