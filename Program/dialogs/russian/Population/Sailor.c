	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				dialog.text = LinkRandPhrase("Приветствую, капитан! Что вам угодно от простого матроса?","Здравствуйте, "+GetSexPhrase("сударь","сударыня")+". Чем могу быть вам полезен?","Доброго дня, уважаем"+GetSexPhrase("ый","ая")+". Вы что-то хотели?");
		...
		case "crew_1":
			...
			if (GetFreeCrewQuantity(pchar) >= sti(npchar.quest.crew.qty))
			{
				...
				link.l1.go = "crew_2";
				link.l2 = "Сожалею, матрос, но я рассчитывал"+GetSexPhrase("","а")+" на несколько... иное. Придётся вам поискать другого капитана.";
				link.l2.go = "exit";
			}
			else
			{
