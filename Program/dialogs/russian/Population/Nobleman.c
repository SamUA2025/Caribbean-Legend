	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
            ...
            if(NPChar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Добрый день, "+GetAddress_Form(NPChar)+"! Приятно встретить "+GetSexPhrase("представителя дворянского сословия","красивую девушку")+" на улицах нашего города! Позволите отнять несколько минут вашего драгоценного времени?";
				...
				dialog.text = RandPhraseSimple("Здравствуйте, "+GetAddress_Form(NPChar)+". Вы что-то от меня хотели?", "Что вам угодно, судар"+GetSexPhrase("ь","ыня")+"?");
			...
			else //--> повторные обращения
			{
				...
				"Нет, это уже начинает действительно утомлять! Вы что, не понимаете с первого раза? Вы "+GetSexPhrase("тугодум","ненормальная")+"?", 
					"Судар"+GetSexPhrase("ь","ыня")+", я начинаю подозревать то, что вы не тупица, а банальн"+GetSexPhrase("ый","ая")+" невежа и хам"+GetSexPhrase("","ка")+". Предупреждаю: не лезьте на рожон!",
					"Так, ну всё. Теперь я проучу тебя, "+GetSexPhrase("наглец","стерва")+"!", "block", 1, npchar, Dialog.CurrentNode);
		...
		case "question":
			dialog.text = LinkRandPhrase("Тогда выкладывайте.","Ох, ну хорошо. Что вы хотите?","Вопросы задавать будем? Ладно, моря"+GetSexPhrase("к","чка")+", слушаю.");
...
//--------------------------------------------дворянин-пассажир---------------------------------------------------
		case "passenger":
			...
			{
				dialog.text = ""+GetSexPhrase("Сударь","Сударыня")+", мне срочно нужно добраться в " + XI_ConvertString("Colony"+pchar.GenQuest.Noblepassenger.City+"Acc")+", это на "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Noblepassenger.City)+"Voc")+", за "+FindRussianDaysString(sti(pchar.GenQuest.Noblepassenger.DaysQty))+". Ваше судно выглядит вполне надёжным, в отличие от большинства ошивающихся у нас лоханок. Я готов выложить за это путешествие "+FindRussianMoneyString(sti(pchar.GenQuest.Noblepassenger.Money))+". Что скажете?"; // belamour gen
			}
			else
			{
				dialog.text = "Приветствую, "+GetSexPhrase("сударь","сударыня")+"! Мне необходимо срочно попасть в " + XI_ConvertString("Colony"+pchar.GenQuest.Noblepassenger.City+"Acc")+"! Это на "+XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Noblepassenger.City)+"Voc")+", если что. Да, да, сам знаю - эти мерзавцы так и ждут, что мы туда явимся. Но очень уж срочная и важная у меня там встреча! Я человек не скупой - платить хоть и не люблю, но всегда плачу. Бросайте все свои дела, и, как только прибудем на место, я выдам вам " + FindRussianDublonString(sti(pchar.GenQuest.Noblepassenger.Money)) + ".";
			}
			link.l1 = "Хм. Нам с вами по пути, так что я готов"+GetSexPhrase("","а")+" взять вас на борт на этих условиях.";
...
// <--
			...
			sTemp = XI_ConvertString("Colony" +pchar.GenQuest.Noblepassenger.City+"Gen") + ", что на " + XI_ConvertString(GetIslandByCityName(pchar.GenQuest.Noblepassenger.City)+"Voc")+"."; // belamour gen
		...
		case "passenger_3":
			...
			dialog.text = "Вот мы и на месте, замечательно! Путешествие на вашем корабле меня весьма и весьма устроило. Благодарю. Вот ваши деньги, судар"+GetSexPhrase("ь","ыня")+".";
...
//-------------------------------------------------помощь деньгами------------------------------------------------
		...
		case "donation_1":
			...
			link.l2 = "Я бы помог"+GetSexPhrase("","ла")+", но я сам"+GetSexPhrase("","а")+" сейчас на мели, нет ни песо лишнего в кармане.";
		...
		case "donation_2":
			...
			link.l1 = "Да не за что, сударь. Уверен"+GetSexPhrase("","а")+", вы бы на моём месте поступили точно так же.";
...
//------------------------------------------привезти рабов под заказ--------------------------------------------
		...
		case "slaves_6":
			...
			dialog.text = "Ещё бы... А теперь прошу меня простить - дела ждут. Всего доброго!";
...
void LombardText()
{
	switch (drand(5))
	{
		...
		case 2:
			...
			pchar.GenQuest.Noblelombard.Text = RandPhraseSimple("вернётся моя экспедиция из глубин Мэйна с золотыми слитками","я получу причитающееся мне наследство");
		break;
		case 3:
