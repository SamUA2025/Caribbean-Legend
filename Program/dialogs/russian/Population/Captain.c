	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = "Хм. Ты ходишь под флагом "+NationNameGenitive(sti(pchar.nation))+", приятель. У меня нет никакого желания с тобой общаться - потом проблем не оберёшься... Ступай своей дорогой!";
					link.l1 = "Ну надо же, сам - капитан, а всё туда же...";
					link.l1.go = "exit";
					break;
				}
				if (CheckAttribute(npchar, "quest.march") && !CheckAttribute(pchar, "questTemp.Sharlie.Lock") && GetCompanionQuantity(pchar) < 5)//боевые генераторные квесты 2015
				{
					...
					link.l2 = "Есть. Я готов"+GetSexPhrase("","а")+" вас выслушать. Куда отправимся? В таверну?";
					link.l2.go = "march";
					break;
				}
				dialog.text = "Ха, приветствую вас, коллега! Приятно видеть преуспевающего капитана!";
				link.l1 = "Рад"+GetSexPhrase("","а")+" встрече, уважаемый! Моё имя - "+GetFullName(pchar)+". Не найдётся минутки поболтать?";
				link.l1.go = "question";
		...
		case "question":
			dialog.text = NPCStringReactionRepeat(""+GetFullName(npchar)+" к вашим услугам, "+GetSexPhrase("милейший","красавица")+"! Что вы хотели узнать?", 
		...
		//квестовая часть
			case "march":
				dialog.text = "Нет, капитан, на улице и в таверне много лишних ушей, а они нам ни к чему, уверяю вас. Дело пойдёт о том, чтобы кое-кого хорошенько пощипать, так что... давайте лучше побеседуем у меня на корабле.";
