	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if(npchar.quest.meeting != "0" && CheckAttribute(npchar, "quest.trial_usurer"))
			{
				...
				link.l1 = "Полагаю, вы знакомы с господином по имени Жерар Лекруа? Он должен был оставить у вас вознаграждение для меня...";
				link.l1.go = "Trial";
			...
			{
				dialog.text = RandPhraseSimple("Добрый день, "+GetSexPhrase("моряк","морячка")+"! Меня зовут "+GetFullName(npchar)+", и я - смотритель этого маяка. Каким ветром вас сюда занесло?", "Здравствуйте! Давненько я не видал новых лиц на моём маяке... Позвольте представиться - "+GetFullName(npchar)+", а этот маяк - мой дом и моя работа. Чем могу служить?");
				link.l1 = "Здравствуй, "+npchar.name+"! Моё имя - "+GetFullName(pchar)+", я капитан корабля. Гулял"+GetSexPhrase("","а")+" по этой чудесной бухте, решил"+GetSexPhrase("","а")+" заглянуть, посмотреть - кто тут живёт...";
				link.l1.go = "Lightman_meeting";
				npchar.quest.meeting = "1";
			}
			else
			{
				dialog.text = "А-а, "+GetSexPhrase("мой старый знакомый","моя старая знакомая")+", капитан "+GetFullName(pchar)+"! Проходите, проходите! Что привело вас ко мне на этот раз?";
				link.l1 = "Приветствую, старый морской волк! Ты ещё не воешь от одиночества на своём маяке?";
		...
		case "Lightman_talk":
			dialog.text = "Что привело вас ко мне на сей раз?";
			link.l1 = RandPhraseSimple("Не расскажешь, что интересного в вашей колонии случилось за последнее время?","Давно не был"+GetSexPhrase("","а")+" на суше... Что новенького в ваших краях?");
			...
			if (CheckAttribute(npchar, "artefact"))
			{
				...
				{
					link.l3 = "Ну что, "+npchar.name+", мой амулет ещё не привезли?";
...
//------------------------------------------------заказ артефактов------------------------------------------------
		...
		case "exit_artefact":
			...
			{
				link.l1 = "Всё это, конечно, хорошо, но я вот что "+ GetSexPhrase("хотел","хотела") +" спросить - полагаю, вы знакомы с господином по имени Жерар Лекруа? Он должен был оставить у вас вознаграждение для меня...";
				link.l1.go = "Trial";
				DelLandQuestMark(npchar);
				break;
			}
		...
		case "amulet_8":
			dialog.text = "Якорь? И пусть судно остаётся на плаву под градом ядер, ха-ха! Хорошо...";
			link.l1 = "Когда мне вернуться к тебе за своим заказом?";
			link.l1.go = "Trade_artefact_2";
		...
		case "obereg_8":
			dialog.text = "Четки торговца? Деньги любят счёт, не правда ли? Хорошо...";
			link.l1 = "Когда мне вернуться к тебе за своим заказом?";
			link.l1.go = "Trade_artefact_2";
		...
		case "Trade_artefact_3":
			...
			dialog.text = "Да, он у меня. Вы приготовили 100 дублонов, капитан?"; // Addon-2016 Jason
				...
				{
					link.l1 = "Эх, дублоны обменять забыл! Сейчас принесу...";
					link.l1.go = "exit";
				}
				// <-- legendary edition
			}
		...
		case "Trade_artefact_4":
			RemoveDublonsFromPCharTotal(100); // Addon-2016 Jason belamour legendary edition
		...
		case "caleuche_1":
			...
			{
				dialog.text = "Хм... Держал я много самых разных амулетов в своих руках, но такой на Карибах встечаю впервые. Извини, дружище, ничем помочь не могу - сам не знаю, что это за диковина. Но то, что вещь не простая - факт. Спроси ещё у кого-нибудь - может, кто-то встречал подобное.";
		...
		case "caleuche_13":
			dialog.text = "Не за что, кэп, захаживай ещё, поболтаем.";
