...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "hired":
			...
			{
				Link.l66 = ""+npchar.name+", я собираюсь отправиться в старый индейский город Тайясаль. Не буду скрывать: это крайне опасное путешествие, и более того - необычное: через телепортационный истукан. Ты готов последовать за своим капитаном?";
				if(npchar.id == "Folke")
				{
					Link.l66 = ""+npchar.name+", у меня к тебе серьёзная просьба - настоящее дело, самоубийственное, но чертовски выгодное. Мне нужен верный человек в походе на древний город индейцев. Ты со мной?";
				}
				if(npchar.id == "FMQT_mercen")
				{
					Link.l66 = "Клод, ты как-то жаловался, что наёмникам мало платят.";
				}
		...
		case "tieyasal":
			if(npchar.id == "Folke")
			{
				dialog.text = "Дьявол, кэп! Я думал, что вы избавитесь от меня при первом удобном случае, но вы, как всегда, меня удивили. Разумеется, я с вами, и спасибо за то, что подарили идиоту настоящую жизнь морского волка!";
				Link.l1 = "Так держать, "+npchar.name+"!";
				Link.l1.go = "folke_tieyasal";
				break;
			}
			if(npchar.id == "FMQT_mercen")
			{
				dialog.text = "А? Я действительно такое говорил?";
				Link.l1 = "Было дело. Я-то тебя слушал внимательно тогда, и готов исправить несправедливость.";
				Link.l1.go = "Duran_tieyasal";
				break;
			}
			dialog.text = LinkRandPhrase("Простите, кэп, но на такое я не подписывался. Не сочтите за трусость, но идти на верную смерть я не собираюсь. Знаете ли, ещё пожить охота. Можете меня уволить.","Капитан, прошу меня простить, но - нет. Море, абордаж, вражеские города - все что угодно, но с индейской нечистью я связываться не желаю. Если вас мой ответ огорчил - давайте расторгнем наш контракт.","Вы просите невозможного, капитан, и сами это знаете. Из этого похода никто не вернётся. Думаю, что даже вы. Так что я - пас. Если считаете, что я отказом нарушаю условия контракта - списывайте меня на берег.");
			link.l1 = LinkRandPhrase("У меня нет к тебе претензий, офицер. Я всё понимаю.","Жаль, конечно, но я все понимаю. Поэтому и предупредил.","Я надеялся на тебя... Но возражения принимаются, офицер.");
			link.l1.go = "tieyasal_1";
		break;
		
		case "Duran_tieyasal":
			dialog.text = "Хе! Довольствие повысите, значит?";
			Link.l1 = "Ни в коем случае! Я отправляюсь в древний город индейцев с помощью их же магии. Дело смертельно опасное, зато золота там будет...";
			Link.l1.go = "Duran_tieyasal_2";
		break;
		
		case "Duran_tieyasal_2":
			dialog.text = "Золото меня не интересует. Для меня будет честью выручить вас, кэп, как когда-то выручили меня вы. Когда отправляемся?";
			Link.l1 = "Отлично сказано, Клод! Будь в полной готовности - уже скоро.";
        ...
        case "WantToGo":
			...
			{
				Link.l2 = LinkRandPhrase("Да-а, вот видишь, как всё обернулось... а я ведь на тебя рассчитывал"+GetSexPhrase("","а")+"... Ну, раз решение принято - давай расставаться. Вот тебе "+sti(NPChar.rank)*250+" песо за верную службу. И не держи зла, если когда обидел"+GetSexPhrase("","а")+".", "Ну что ж, я зла не держу и худого не помню. Раз решил - иди. Да держи "+sti(NPChar.rank)*250+" песо на дорожку. Это всегда пригодится.", "Ясно... рано или поздно это должно было случиться. Презентую тебе "+sti(NPChar.rank)*250+" песо на обустройство личной жизни. Да здоровье поправь, морская служба она не сахар - на всю жизнь след оставляет..."); 
	...
	case "WantToGo_Stay_2":
		...
		{
			Link.l1 = RandPhraseSimple("Вот и договорились.", "Хорошо. Хотя мог бы быть поскромнее...", "Экие у тебя аппетиты! Ну, раз обещал"+GetSexPhrase("","а")+" - держи.");	
	...
	case "WantToRemove":
		...
		if (Npchar.alignment == "good")
		{
			...
			if (sti(Pchar.money) >= sti(NPChar.rank)*500)
			{
				Link.l2 = "Я давно знал"+ GetSexPhrase("","а") +", что ты уйдёшь... такая служба не для тебя. Но упрекнуть мне тебя не в чём. Вот "+sti(NPChar.rank)*500+" песо - пригодятся, пока найдёшь работу по душе.";
		...
		else
		{
			...
			if (sti(Pchar.money) >= sti(NPChar.rank)*500)
			{
				Link.l2 = "Я знал"+ GetSexPhrase("","а") +", что ты уйдёшь... такая служба не для тебя. Но упрекнуть мне тебя не в чём. Вот "+sti(NPChar.rank)*500+" песо - пригодятся, пока будешь искать новое место.";
				Link.l2.go = "Im_kind_A2";
			}
		...
		case "WantToGo_Munity":	//zagolski. отыгрыш бегство офицера
			...
			/* if(CheckCharacterPerk(pchar, "IronWill"))
			{
				...
				log_info(GetFullName(Npchar)+" повысил свою лояльность");
				log_testinfo("Лояльность стала: " + Npchar.loyality + " из " + MAX_LOYALITY);
			}
			else
			{ */
		...
		case "price":
			...
			if(Npchar.rank >= 15)
			{
				dialog.text = "Думаю, мы с вами сойдемся на " + (makeint(Npchar.quest.OfficerPrice) / 100 + 30) + " дублонов.";
			}			
		...
		case "trade":
			if (GetSummonSkillFromNameToOld(pchar, SKILL_COMMERCE) >= Rand(12) && (sti(Npchar.quest.OfficerPrice) - GetCharacterSkillToOld(Pchar, "commerce")*100) >= sti(Npchar.quest.OfficerLowPrice))// bug fix
			{
				...
				if(Npchar.rank >= 15)
				{
					dialog.text = "Хорошо... Пожалуй, я готов согласиться даже на " + (makeint(Npchar.quest.OfficerPrice) / 100 + 30) + " дублонов. Ну как?";
				}
        ...
        case "stay_follow":
			...
			if(CheckAttribute(NPChar, "equip.gun"))
			{
				...
				{
					Link.l4 = "Нужно изменить тип боеприпаса для твоего пистолета.";
			...
			if(CheckAttribute(NPChar, "equip.musket"))
			{
				...
				{
					Link.l5 = "Нужно изменить тип боеприпаса для твоего мушкета.";
					Link.l5.go = "SetMusketBullets";
				}
					Link.l6 = "Нужно изменить тип твоего приоритетного оружия для боя.";
		...
		case "CompanionTravel_ToSquadron":
			...
			{
				dialog.text = "Кэп, у вас же и так эскадра из пяти коралей, куда-же шестой ещё? В общем, я буду ждать вас тут, на своём корабле - все эти ваши новые компаньоны не надежны - их судна очень скоро пойдут ко дну и вы за мной вернётесь.";
