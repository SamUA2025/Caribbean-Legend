...
// ------------------------------------------блок angry-----------------------------------------------
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "shark_6":
			...
			link.l1 = "Мы с Лесным Дьяволом хотим предложить его кандидатуру на пост лидера Берегового братства. Как я уже сказал, есть серьёзные основания полагать, что Блейза убил кто-то другой.";
			link.l1.go = "shark_7";
		break;
...
//--------------------------------------------Похититель------------------------------------------------------
		...
		case "Marginpassenger_offer":
			...
			switch (drand(1))
			{
				case 0: //бухта на южном мейне
					...
					dialog.text = "Приятно иметь дело со сметливым человеком, парень! Теперь слушай: через несколько дней к " + XI_ConvertString(pchar.GenQuest.Marginpassenger.Shore+"Dat") + " подойдёт испанская экспедиция из Мэйна, с запасами ценной древесины, и будет ждать корабля, который должен прийти и забрать груз. Если за неделю ты успеешь туда добраться - у тебя будет шанс забрать весь товар себе\nНа твоём месте я бы уже бежал к своей посудине. И потрудись доставить пленника сюда."; // belamour gen
		...
		case "Marginpassenger_offer_1":
			...
			AddQuestUserData("Marginpassenger", "sName", "Захарии Марлоу")); // belamour gen : как-то странно, наводку даёт Захар, а в сж Добряк дефолтом
