		...
		case "First time":
			...
			if (CheckAttribute(pchar, "questTemp.Mtraxx") && !CheckAttribute(npchar, "quest.mtraxx_complete")) 
			{
				if (pchar.questTemp.Mtraxx == "full_complete_end" || pchar.questTemp.Mtraxx == "full_complete")
				{
					...
					link.l1 = "Нет, не в курсе... Маркус, я пришёл сказать, что выхожу из игры. С меня довольно. Мертвецы снятся мне по ночам, и черти с раскалёнными жаровнями мерещатся... Слишком много крови пролилось в последнее время.";
...
//--> -----------------------------Сага - Искушение Барбазона завершающие этапы---------------------------
		...
		case "shadows_15":
			dialog.text = "На счёт Элен МакАртур в архивах Блейза ничего не было. Всё-таки странно, что Беатрисс ничего не сказала Блейзу о рождении племянницы. Это на неё не похоже.";
		...
		case "Mtraxx":
			...
			{
				dialog.text = "Ты совсем сдурел? За тобой пол-испанского мира по пятам бегает. От тебя больше убытков будет, чем пользы. Реши свои проблемы, затем заявляйся. У меня всё! Исчезни отсюда!";
				link.l1 = "Кхм... Понятно.";
				link.l1.go = "exit";
				break;
			}
		...
		case "mtraxx_2":
            dialog.text = "В точку, приятель. Даже здесь за него ювелиры да ростовщики отваливают немалые деньги, а в старушке Европе можно выручить в десять раз больше. Янтарь не редкость на Карибах, но вот именно голубой крайне трудно найти. И тут какой-то кастильский проходимец бахвалится, что у него скоро голубого янтаря будет больше, чем жёлтого у всех ростовщиков Кубы и Эспаньолы, вместе взятых.";
		...
		case "mtraxx_5":
			...
			else 
			{
				if(bOk) 
				{
					...
					{
						sTemp = ". Подожди-ка, возьми торговую лицензию сроком на 60 дней."; 
				...
				else // нет флага
				{
					...
					{
						sTemp = ". Подожди-ка, возьми торговую лицензию сроком на 60 дней, а также испанский флаг для высадки в Санто-Доминго. Впредь рекомендую обзавестись своим, поможет одурачить простофиль на торговых лоханках. Этот вернешь мне обратно.";
						GiveNationLicence(HOLLAND, 60);
						SetCharacterPerk(pchar, "FlagSpa");
						log_info("Вы получили испанский флаг");
					...
					{	
						...
						{
							sTemp = ". Подожди-ка, а как ты торговать собрался не имея нужного флага?! Вот возьми, торговая лицензия сроком 60 дней, больше чем у твоей. А также испанский флаг для высадки в Санто-Доминго. Впредь рекомендую обзавестись своим, поможет одурачить простофиль на торговых лоханках. Этот вернешь мне обратно.";
							GiveNationLicence(HOLLAND, 60);
							SetCharacterPerk(pchar, "FlagSpa");
							log_info("Вы получили испанский флаг");
							pchar.questTemp.GiveMeSpaFlag = true;
						...
						{
							sTemp = ". Подожди-ка, а как ты торговать собрался не имея нужного флага?! Вот возьми, испанский флаг для высадки в Санто-Доминго. Впредь рекомендую обзавестись своим, поможет одурачить простофиль на торговых лоханках. Этот вернешь мне обратно.";
							SetCharacterPerk(pchar, "FlagSpa");
							log_info("Вы получили испанский флаг");
							pchar.questTemp.GiveMeSpaFlag = true;
						}
			...
			// <-- legendary edition
            dialog.text = "Тебе нужна бордельная девка по имени Габриэла Чападо. Назовёшь моё имя и пароль - 'янтарное ожерелье'. Она сообщит подробности. И не теряй времени - информация пока свежая, а вот через день-другой-третий может и устареть"+sTemp;
		...
		case "mtraxx_8":
			if (sti(pchar.questTemp.Mtraxx.JewQty) > GetCharacterItem(pchar, "jewelry7"))
			{
				...
				if(CheckAttribute(pchar, "questTemp.GiveMeSpaFlag")) 
				{
					...
					log_info("Вы отдали испанский флаг");
				}
			}
			else
			{
		...
		case "mtraxx_24":
			...
			else 
			{
				...
				else // нет флага
				{
					if(!CheckCharacterItem(pchar, "HolTradeLicence")) 
					{
						sTemp = ", а также торговую лицензию сроком на 40 дней. К тому же, как ты собрался дурить растяп в форте?! Подведешь ты меня однажды с таким разгильдяйством... На вот... Вернешь по приезду обратно. Что за поколение нынче пошло?!";
						GiveNationLicence(HOLLAND, 40);
						SetCharacterPerk(pchar, "FlagSpa");
						log_info("Вы получили испанский флаг");
						pchar.questTemp.GiveMeSpaFlag = true;
					...
					{	
						...
						{
							sTemp = ". Скажи мне, как ты собрался дурить растяп в форте?! Твоя бумажка тебе не поможет без нужного флага. Чую, подведешь ты меня однажды с таким разгильдяйством... На вот, торговая лицензия сроком на 40 дней, больше чем у твоей. А также испанский флаг для высадки в Маракайбо. Вернешь по приезду обратно. Что за поколение нынче пошло?!";
							GiveNationLicence(HOLLAND, 40);
							SetCharacterPerk(pchar, "FlagSpa");
							log_info("Вы получили испанский флаг");
							pchar.questTemp.GiveMeSpaFlag = true;
						...
						{
							sTemp = ". Скажи мне, как ты собрался дурить растяп в форте?! Твоя бумажка тебе не поможет без нужного флага. Чую, подведешь ты меня однажды с таким разгильдяйством...  На вот... Вернешь по приезду обратно. Что за поколение нынче пошло?!";
							SetCharacterPerk(pchar, "FlagSpa");
							log_info("Вы получили испанский флаг");
							pchar.questTemp.GiveMeSpaFlag = true;
						}
		...
		case "mtraxx_27":
			...
			{
				dialog.text = "С прибытием, бравый корсар, ха-ха! Пикар уже обо всём рассказал. Отличная работа!";
				link.l1 = "...";
				link.l1.go = "mtraxx_28";
			}
		break;
		...
		case "mtraxx_43":
			...
			link.l2 = "Знаешь, Маркус, я тут всё взвесил и эта работа не для меня. Извини, но я уже всё решил.";
		...
		case "mtraxx_44a":
            dialog.text = "Что?! Я вытащил тебя с улицы, набил твои карманы янтарём и драгоценными камнями! Это так ты мне отплатил?!";
			link.l1 = "Мерида показала мне, что я недостаточно крут для такой работы. Не так крут как ты и твои парни. Не держи зла.";
			link.l1.go = "mtraxx_45a";
		break;
		
		case "mtraxx_45a":
            dialog.text = "Слабак! Ну и проваливай! Иди, горбаться на фрахтах или, что ещё лучше, упейся вусмерть как остальные неудачники!";
			link.l1 = "...";
		...
		case "mtraxx_50":
            dialog.text = "Помню. И?";
			link.l1 = "В Бриджтауне ко мне подошёл Игнасио Марко. Это тот самый португалец, капер на службе Англии. Пригласил в таверну и предложил совместно ограбить голландский караван у берегов необитаемого острова, что находится между Кюрасао и Тринидадом.";
		...
		case "mtraxx_53x":
			...
			link.l1 = "Бумага стерпит... А вот с этим шакалом Игнасио надо что-то сделать. Ты понимаешь, о чём я.";
			link.l1.go = "mtraxx_54";
		break;
		
		case "mtraxx_53y":
            dialog.text = "Хех! Представляю, как бушевали кастильцы, когда поняли, что упустили тебя! Молодец!";
			link.l1 = "Пусть лопнут от злости. А вот с этим шакалом Игнасио надо что-то сделать. Ты понимаешь, о чём я.";
			link.l1.go = "mtraxx_54";
		break;
		...
		case "mtraxx_63":
            dialog.text = "Выяснил я твой вопрос. Один книжный червь просветил. Горд - это укреплённый посёлок свободного ярла.";
		...
		case "mtraxx_64":
			...
			link.l1 = "Пока не знаю, надо сопоставить записи. Значит, горд - это посёлок... Спасибо, Маркус!";
		...
		case "mtraxx_71":
            dialog.text = "Есть у меня подозрение, что Марко связан кое с кем... И ты это выяснишь. Отправляйся сейчас на Мартинику. Корабль оставь в Сен-Пьере, а сам через джунгли топай в посёлок Ле Франсуа. Мне донесли, что наш тип там ошивается. К пиратам с расспросами не приставай, высматривай сам. Найдёшь - проследи за ним: куда пойдёт, с кем встретится. И сразу предупреждаю: не вздумай напасть на него самого или его корабль - испортишь мне весь план, который я разрабатываю\nТолько понаблюдай за ним, а потом - сразу сюда, в Ла Вегу. Всё понял? Никакой самодеятельности. Я понимаю, что у тебя руки чешутся, но будь добр - сдержись. А теперь вперёд. Трёх недель тебе должно хватить. Удачи.";
		...
		case "mtraxx_76":
            dialog.text = "Уймись! Ты до Барбазона ещё ростом не вышел. Он тебя перекусит и выплюнет, как акула весло. Мы отплатим ему той же монетой - ударим по его порученцам. И счёты сведём, и Жаку на хвост наступим, и золотом разживёмся, если не напортачим. Готов слушать? Тебя ждёт жаркое дело.";
			link.l1 = "Не впервой!";
		...
		case "mtraxx_85f":
			...
			log_info("Вы получили испанский флаг");
			pchar.questTemp.GiveMeSpaFlag = true;
            dialog.text = "Какого чёрта? Вот треснуть бы тебя кочергой от камина, Принц. Не думал, что об элементарных вещах придётся рассказывать. Держи испанский флаг, а то всё дело завалишь, дубина, так и не начав. Вернёшь мне его обратно. Теперь дуй отсюда!";
			link.l1 = "Я уже в море, Босс";
		...
		case "mtraxx_board_4": // belamour legendary edition увеличиваем срок до 3х месяцев
            dialog.text = "Я так точно в Ла Вегу, буду тебя там ждать в течение трёх месяцев. Приходи, как будешь готов. Желательно на корабле посильнее, но только на одном - и так будет целая эскадра. Я собираю всех своих людей в поход. Под моим началом пойдёшь ты, Жан Красавчик, Лысый Джеффри, Тесак Пелли, Люк Лепрекон. Готовься. Нас ждёт серьёзная экспедиция и немаленькая добыча. Все подробности узнаешь позже.";
			link.l1 = "Хорошо, босс! Буду через три месяца в Ла Веге.";
		...
		// belamour legendary edition -->
		case "mtraxx_94a":
            dialog.text = "Ты... ты! Сдурел, Принц?! Подставляешь меня?! А знаешь что, чёрт с тобой! У тебя был шанс войти в Братство, был шанс вырваться из рядов всего этого бесполезного быдла. И ты этот шанс только что бездарно и трусливо упустил. Проваливай со своим нытьём! Слабак! Ещё раз увижу - собак спущу!";
			link.l1 = "...";
		...
		// <-- legendary edition
		case "mtraxx_97":
            dialog.text = "Принц, ну наконец-то! Вы поглядите, парни - мы ему подарок приготовили, а он шляется неизвестно где! Садись, мы начинаем!\nИтак, бравые корсары! Нас ждёт золото Картахены, ха-ха! И мы его получим, клянусь громом! Но тот, кто умеет хорошо работать, должен хорошо отдыхать! Поэтому сегодня мы чудно проведём время в этом прекрасном заведении. Налейте себе рому, да побольше! Кружки наполнить до краёв! За ветер добычи, за ветер удачи! Пьём до дна!\nБерни, Принц, Тесак, Лысый, Лепрекон! Для вас самое лучшее пойло из погребов нашей таверны, и эти самые прекрасные дамы на острове, ха-ха-ха!\nНу а для тебя, Шарль, от нас с ребятами - шикарная шляпа. Ну-ка примерь, и не спорь!\nКорсары, запевай песню! Нашу, любимую!..";
			link.l1 = "";
		...
		case "mtraxx_107":
			...
			{
				dialog.text = "Хо-хо, вот это молодец! Пятьдесят тысяч песо оставишь себе, пятьдесят отдаём Тесаку - это вам обоим премия за смекалку и сноровистость, а двести пятьдесят тысяч давай мне - пойдёт в общий котёл.";
				link.l1 = "Держи!";
		...
		case "mtraxx_110":
            dialog.text = "Поскольку операция задумана и разработана мной, а также поскольку 'Красный дракон' выполнил наиболее важную задачу - разрушение форта, то мне и команде 'Красного дракона' положена половина добычи. Остальное делится между капитанами остальных четырёх кораблей, согласно количества их команды. На твоих "+GetCrewQuantity(pchar)+" человек приходится "+sti(pchar.questTemp.Mtraxx.Cartahena.Gold)+" мер золота и "+sti(pchar.questTemp.Mtraxx.Cartahena.Money)+" песо.";
		...
		case "PZ1":
			...
			{
				dialog.text = "Давненько ты к нам не заглядывал, Принц.";
				link.l1 = "Не уверен, что это имя приносит мне удачу, Маркус.";
				link.l1.go = "PZ_Prince1";
			}
			else
			{
				dialog.text = "Де Мор! Что привело тебя ко мне?";
				link.l1 = "Здравствуй, Маркус. Да вот хотел кое-что спросить у тебя.";
				link.l1.go = "PZ_DeMaure1";
			}
		break;
		
		case "PZ_Prince1":
			dialog.text = "А как по мне - наоборот. Но ладно. Какими судьбами у меня?";
			link.l1 = "Ты слышал о борделе в Тортуге?";
			link.l1.go = "PZ2";
		break;
		
		case "PZ_DeMaure1":
			dialog.text = "Ну так изволь, спрашивай. Ты же знаешь, не люблю я эти ваши здравствуйте, до свидания, а можно ли спросить?";
			link.l1 = "Ты слышал о борделе в Тортуге?";
			link.l1.go = "PZ2";
		break;
		
		case "PZ2":
			dialog.text = "Ты что, нажрался? Что это вообще за вопрос такой? Не трать моё время, если ты и правда хотел поговорить именно об этом.";
			link.l1 = "Видимо, ты не знаешь? Там недавно открылся новый бордель, очень дорогой, где собраны только красивые, здоровые и хорошо одетые девушки.";
			link.l1.go = "PZ3";
		break;
		
		case "PZ3":
			dialog.text = "Ах, это! Слышал, конечно, а кто ж не слышал-то?";
			link.l1 = "Ну-у, я узнал о нём лишь на днях.";
			link.l1.go = "PZ4";
		break;
		
		case "PZ4":
			dialog.text = "Ха-ха-ха, ну ты даёшь, разрази меня гром! И всё же. Почему ты решил обсудить это со мной? Ты бы ещё к Хоукам наведался с таким вопросом. Или... ты уже был там?";
			if (CheckAttribute(pchar, "questTemp.PZ.EliteWhoreFucked"))
			{
				link.l1 = "Был. Принимают там только дублоны, кстати.";
				link.l1.go = "PZ_Fucked1";
			}
			else
			{
				link.l1 = "Был, но услугами не пользовался. Однако...";
				link.l1.go = "PZ_NoFucked1";
			}
		break;
		
		case "PZ_Fucked1":
			dialog.text = "Так, значит, ты пришёл похвастаться! Ха, вот же плут.";
			link.l1 = "На самом деле, нет. Я и правда хотел узнать кое-что у тебя, Маркус.";
			link.l1.go = "PZ5";
		break;
		
		case "PZ_NoFucked1":
			dialog.text = "Не пользовался?! Ты глупец, де Мор. Я бы на твоём месте...";
			link.l1 = "... устроил бы знатный дебош. Однако не совсем это я хотел с тобой обсудить.";
			link.l1.go = "PZ5";
		break;
		
		case "PZ5":
			dialog.text = "И что же это? Давай, не томи.";
			link.l1 = "Я ищу хозяйку этого заведения. Мне сказали, что она хотела встретиться с тобой и предложить вложиться в её дело. Её ни с кем не перепутаешь - она китаянка.";
			link.l1.go = "PZ6";
		break;
		
		case "PZ6":
			dialog.text = "Хо, надо же. Но нет, никаких китаянок у меня в последнее время не было. Однако я совершенно не против как познакомиться с ней, так и, собственно, вложиться - её официальное приглашение не помешает - в конце концов, просто так во владения Левассера не сунешься, и дело отнюдь не в том, что, как подданный твоего короля, он будет вынужден открыть огонь из своей черепашки или прибрежных судов по моему 'Дракону', как только увидит его.";
			link.l1 = "Значит, она решила сначала заглянуть в Порт-о-Пренс...";
			link.l1.go = "PZ7";
		break;
		
		case "PZ7":
			dialog.text = "Думаешь? Напрасно. Даже если у де Мюссака больше денег, в чём я, кстати, сомневаюсь, то он явно не сможет разбазарить какую-то хоть мало-мальски крупную часть городского бюджета на бордель, даже если он потом будет приносить немало золота.";
			link.l1 = "Хорошо подметил. Там вообще многое не продумано - сплошные амбиции да убытки.";
			link.l1.go = "PZ8";
		break;
		
		case "PZ8":
			dialog.text = "Знаешь, всегда удивлялся, как вообще другие бордели держатся на плаву. Не женское это дело - думать и, особенно, руководить, попомни мои слова.";
			link.l1 = "Ну, это ты зря. Спасибо, что уделил время, Маркус! Если встречу её первым - передам, что ты будешь рад её видеть.";
			link.l1.go = "Exit";
