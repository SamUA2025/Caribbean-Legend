	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (!bDisableMapEnter)
			{
				...
				{
					dialog.text = "Здравствуйте, капитан. Чёрт возьми, вы даже не представляете, как я рад вас видеть!";
				...
				if (CheckAttribute(PChar, "questTemp.WPU.Escort.LevelUp_1WM") && pchar.questTemp.WPU.Escort == "wait" && NPChar.id == "WMCaptain")
				{
					...
					{
						link.l1 = "Да. Всё, что нужно для ремонта вашего судна находится у меня в трюмах.";
				...
				{
					dialog.text = "Здравствуйте, " + GetAddress_Form(NPChar) + ". Меня зовут " + GetFullName(NPChar) + ", я капитан этого корабля. Хочу вам выразить свою безграничную признательность за помощь в бою с этим проклятым пиратским отродьем - без вашего вмешательства всё могло бы закончится для нас весьма плачевно. Вы просто спасли нас, чёрт возьми!";
					link.l1 = "Не стоит благодарности, " + GetAddress_FormToNPC(NPChar) + ". Позвольте представиться - " + GetFullName(PChar) + ", я прибыл к вам на помощь по просьбе начальника портового управления "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.StartCity+"Gen")+". Он вкратце рассказал о злоключениях вашего каравана и отправил меня на розыск вашего судна.";
				...
				if(CheckAttribute(NPChar, "surrendered"))
				{
					...
					//выкуп
					link.l1 = "Давайте решим всё мирно. Выкуп вполне меня устроит. Сумма в "+Pchar.GenQuest.MoneyForCaptureShip+" песо меня вполне устроит, "+GetAddress_FormToNPC(NPChar)+". И после этого можете проваливать отсюда ко всем чертям!!";
				...
				if (CheckAttribute(PChar, "questTemp.HWIC.Self") && pchar.questTemp.HWIC.Self == "MeetingStivesant" && NPChar.id == "Stivesant")
				{
					...
					link.l1 = "Добрый день, минхер Стайвесант. Позвольте представиться - " + GetFullName(pchar) + ". Лонгвэй не придёт, потому что его нет на 'Мейфенг'.";
				...
				if (CheckAttribute(PChar, "questTemp.HWIC.Self") && pchar.questTemp.HWIC.Self == "FleutOnTrinidad" && NPChar.id == "Keller")
				{
					...
					link.l1 = "Добрый день, минхер Келлер. Позвольте представиться - " + GetFullName(pchar) + ". Я пришёл к вам за помощью в одном необычном деле.";
					link.l1.go = "Keller_1";
					break;
					//Голландский гамбит
				}
				...
				if(npchar.id == "FishingBoatCap")
				{
					if(!CheckAttribute(pchar, "GenQuest.FishingBoatSecondTime"))
					{
						...
						link.l1 = "И вам доброго дня. А что вы делаете в этой бухте?";
					...
					else
					{
						...
						dialog.text = "Что ещё хотите обсудить?";
						...
						{
							link.l2 = "Покажите ещё раз, пожалуйста, что у вас там за побрякушки в продаже. ";
							link.l2.go = "Торговля";
							if(pchar.GenQuest.FishingBoatDialogEnb != "нет особых товаров")
							{
								link.l3 = "Хм, может быть поищете ещё по трюму - авось что-нибудь да найдётся на продажу?";
								link.l3.go = "Особые товары";
							}
						}
						link.l4 = "Ничего особенного, просто "+ GetSexPhrase("зашёл","зашла") +" поприветствовать вас!";
						link.l4.go = "exit";	
						break;
					}
				}
				...
				{
					Dialog.text = "Генерал-губернатор, Ваша Светлость, приветствую вас! Чем я могу быть для вас полезен?";
					link.l1 = "У меня есть к вам дело!";
					link.l1.go = "quests";
				...
				{
					Dialog.text = "Приветствую вас на борту моего судна, вице-адмирал! Что я могу сделать для вас?";
					link.l1 = "У меня есть к вам дело!";
					link.l1.go = "quests";
					link.l2 = "Ничего особенного, просто "+ GetSexPhrase("зашёл","зашла") +" поприветствовать вас!";
					link.l2.go = "exit";
				...
				{
					Dialog.text = "Рад видеть товарища по службе, капитан! Чем могу помочь?";
					link.l1 = "У меня есть к вам дело!";
					link.l1.go = "quests";
					link.l2 = "Ничего особенного, просто "+ GetSexPhrase("зашёл","зашла") +" поприветствовать вас!";
					link.l2.go = "exit";
        ...
        case "quests":
            ...
            if (!CheckAttribute(NPChar, "MainCaptanId")  || NPChar.MainCaptanId == pchar.GenQuest.CaptainId)
            {
                ...
                {
                    link.l2 = RandPhraseSimple("Вы в курсе, кто я? Думаю, ясно, что мои корабельные орудия могут сделать решето из вашего корыта. Давайте решим всё мирно. Выкуп вполне меня устроит.",
		...
		case "Dice_Node_100":
		    ...
		    {
                dialog.text = "О тебе ходит слава непревзойдённ"+ GetSexPhrase("ого шулера","ой каталы") +". Я не буду с тобой играть в кости вообще.";
        ...
        case "price_1":
			...
			{
                Dialog.Text = "Я же вам всё уже сказал. Не беспокойте меня по пустякам.";
        ...
        case "Talk_board":
            ...
            else
            {
                ...
                {
                    Dialog.text = "Напрасно вы затеяли своё грязное дело, капитан, на борту моего корабля. Я проявлю милосердие позволив вам вернуться на свой корабль и утонуть вместе с ним.";
		...
		case "surrender_goaway":
			Dialog.Text = "Ну, что вы ещё от меня хотите?";
			Link.l1 = "Ничего.";
			Link.l1.go = "exit";
			Diag.TempNode = "surrender_goaway";
		break;
		...
		case "surrender2":			
			dialog.text = "Мне не остаётся ничего другого, кроме как подчиниться вам.";
		...
		case "QuestAboardCabinDialog_3_1":
			Dialog.Text = ""+ GetSexPhrase("Пришёл","Пришла") +", наследил"+ GetSexPhrase("","а") +", всё отнял"+ GetSexPhrase("","а") +" и ещё меня в рабы? Фиг!";
		...
		case "Siegehelp":		
            ...
            if (NPChar.id == rchar.id)
            {
    			if(CheckAttribute(pchar, "questTemp.Patria.GenGovernor") && npchar.nation == FRANCE)
				{
					dialog.text = "Ваше сиятельство, генерал-губернатор! Я вас внимательно слушаю, но очень прошу вас покороче: мы ведём бой с " + fort +
				...
				else
				{
					if(isMainCharacterPatented() && sti(Items[sti(pchar.EquipedPatentId)].TitulCur) > 4 && sti(Items[sti(pchar.EquipedPatentId)].Nation) == npchar.nation)
					{
						dialog.text = "Господин вице-адмирал! Я вас внимательно слушаю, но у меня времени мало: мы ведём бой с " + fort +
                          ", и мне нужно руководить ходом сражения.";
					}
					else
					{
		...
		case "Siegehelp_0":
            Dialog.text = "Это опять вы? Мне казалось, мы всё уже обсудили?";
		...
		case "WM_Captain":
			...
			link.l1 = "Как я слышал, вам пришлось выдержать бой с пиратами, а затем ещё и шторм...";
			link.l1.go = "WM_Captain_1";
			pchar.quest.DesIsland_Over.over = "yes";//снять прерывание
		break;
		
		case "WM_Captain_1":
			dialog.text = "Всё верно. Мой корабль сильно пострадал в бою - нам даже пришлось отбивать абордажную атаку этих нехристей. А потом налетел шквал... ох, даже вспоминать об этом не хочу. Нас вынесло к этому негостеприимному берегу - мы целый день в шторм стояли на якорях, и молили Пресвятую Деву, чтобы цепи выдержали\nИ вот мы здесь. Мы уже неделю чиним наш корабль, но - сами видите: команды осталось чуть больше полусотни человек, для починки нужны доски - а где их взять? Только в джунглях. Мы целыми днями работаем, и не заготовили ещё и пятой части нужного количества материала\nА отправиться в путь с такими повреждениями - равносильно самоубийству. Так и живём, капитан...";
			link.l1 = "М-да... Положение незавидное. Давайте подумаем, что можно предпринять, и как я могу вам помочь.";
			link.l1.go = "WM_Captain_2";
		break;
		
		case "WM_Captain_2":
			dialog.text = "Эх, если бы у нас были доски и парусина в необходимом количестве - мы бы за неделю отремонтировали судно - было бы как новенькое\nКапитан, раз вы специально пришли мне на выручку - не могли бы вы отправиться в ближайший порт, закупить необходимого материала для починки и привезти сюда? Таким образом, проблема была бы решена. Финансовая сторона пусть вас не волнует - я компенсирую все ваши затраты на покупку.";
		...
		case "WM_Captain_3":
			...
			dialog.text = "Я уже давно всё посчитал. С учётом заготовленного, мне необходимо "+FindRussianQtyString(sti(pchar.questTemp.WPU.Escort.Planks))+" досок, "+FindRussianQtyString(sti(pchar.questTemp.WPU.Escort.Sailcloth))+" парусины и "+FindRussianQtyString(sti(pchar.questTemp.WPU.Escort.Linen))+" хлопка. Имея этот набор, мы бы в недельный срок управились с починкой и вышли в море.";
		...
		case "WM_Captain_6":
			...
			dialog.text = "Ох, даже и не знаю, как мне вас благодарить, капитан... Прикажите сгрузить материалы в бухте под названием " + XI_ConvertString(pchar.questTemp.WPU.Current.TargetIslandID.Shore) + " - мы там всё подготовили для ремонта.";
		...
		case "Repair_start":
			dialog.text = "Ну вот, всё готово к проведению ремонтных работ. Думаю, дней за пять управимся.";
			link.l1 = "Вот и отлично! Я давно хотел получше исследовать этот остров, так что время не пропадёт даром.";
		...
		case "Repair_end":
			dialog.text = "Вот и всё. Починка окончена, мой корабль готов к выходу в море. Когда отправляемся, капитан?";
		...
		case "WMShip_final":
			...
			dialog.text = "Хочу ещё раз вас поблагодарить, капитан, за то, что вытащили меня и мою команду из этой передряги. Как я и обещал - я компенсирую ваши затраты на материалы для починки моего судна. Здесь "+FindRussianMoneyString(sti(pchar.questTemp.WPU.Escort.LevelUp_1Money))+", возьмите, пожалуйста. И ещё вот этот подарок лично от меня.";
			link.l1 = "Рад"+GetSexPhrase("","а")+" был"+GetSexPhrase("","а")+" помочь вам.";
		...
		case "VSP_Captain":
			dialog.text = "Вот как! А я думал, что про нас все забыли. Да, моему кораблю не повезло больше всех. После того, как шторм вынудил пиратов прекратить атаку каравана, мы сутки боролись с разбушевавшейся стихией. Нас вынесло к этому негостеприимному берегу\nМы несколько дней отдыхали, чинили корабль, повреждённый бурей, уже собирались выйти в море, как на нас снова напали эти богоотступники. Очевидно, они всё это время были на другой стороне острова\nА потом опять налетел шторм - мы уже решили, что Господь окончательно прогневался на нас и готовились к худшему. Но тут появились вы, слава Пресвятой деве Марии! Спасибо вам ещё раз, капитан!";
			link.l1 = "Всё уже позади, " + GetAddress_FormToNPC(NPChar) + ". Теперь я должен сопроводить вас до "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Escort.StartCity+"Gen")+", дабы уберечь от иных возможных бед. Вы готовы отправиться в путь немедленно? Шторм уже пошёл на убыль.";
			link.l1.go = "VSP_Captain_1";
		break;
		
		case "VSP_Captain_1":
			dialog.text = "Конечно! Чем быстрее мы покинем эти места, тем спокойнее будет моя исстрадавшаяся команда. Отправляемся!";
			link.l1 = "Следуйте за моим кораблём. И успокойте вашу команду - более вам ничего не угрожает.";
		...
		case "SShip_final":
			...
			dialog.text = "Хочу ещё раз вас поблагодарить, капитан, за то, что вытащили меня и мою команду из этой передряги. Примите от меня эти деньги - "+FindRussianMoneyString(sti(pchar.questTemp.WPU.Escort.LevelUp_1Money))+", ещё вот этот подарок. Это немного, но это всё, чем я могу отблагодарить вас за своё спасение. И прошу вас не отказываться\nМоя бывшая команда восхищена вами - как вы дрались на берегу с индейцами, и как расправились с пиратским кораблём. Все они хотят остаться у вас на службе. Надеюсь, вы не откажете морякам, потерявшим своё судно, в их маленькой просьбе - служить вам верой и правдой?";
			link.l1 = "Нет, конечно. Я с удовольствием оставлю их в своей команде. Рад"+GetSexPhrase("","а")+" был"+GetSexPhrase("","а")+" помочь вам!";
			link.l1.go = "WMShip_final_1";
		break;
		...
		case "Stivesant_2":
			...
			link.l1 = "В Виллемстаде совершён переворот. Лукас Роденбург занял место губернатора, Матиас Бек посажен им в тюрьму на основании абсурдного подозрения в связи с английской разведкой.";
			link.l1.go = "Stivesant_3";			
		break;
		
		case "Stivesant_3":
			dialog.text = "Хм... Минхер, вы рассказываете удивительные вещи. Я не так давно получал новости из Виллемстада, и ни о чём подобном в них не говорилось.";
			link.l1 = "Выслушайте меня до конца, минхер Стайвесант. Новости вы наверняка получили через посредничество Роденбурга, а он уверен, что вы до Кюрасао не дойдёте.";
			link.l1.go = "Stivesant_4";			
		break;
		
		case "Stivesant_4":
			dialog.text = "Сударь, ваши слова мне нравятся всё меньше и меньше. Кто вы вообще такой?";
		...
		case "Stivesant_5":
			...
			link.l1 = "Решил сместить вас с поста директора Компании самым простым способом - путём физического устранения. Кроме вас, нет более человека в регионе, способного оказать сколь бы то ни было значительное сопротивление влиянию Роденбурга на Архипелаге.";
			link.l1.go = "Stivesant_6";			
		break;
		
		case "Stivesant_6":
			dialog.text = "Ушам не верю!";
			link.l1 = "Я не поверил в связь Бека с английской разведкой и предпринял расследование. Узнав, что 'Мейфенг', гружёная под завязку отборными головорезами, вышла в направлении Филипсбурга, я догнал её и дал бой. Захватив в плен Лонгвэя, я получил от него всю эту информацию, в том числе и где вас искать.";
			link.l1.go = "Stivesant_7";			
		break;
		
		case "Stivesant_7":
			dialog.text = "Хм... Похоже на правду. Никто, кроме Роденбурга, не знал и не мог знать ни о времени, ни о маршруте следования моего галеона! Проклятье! Это измена! Где Лонгвэй?!";
			link.l1 = "Я отпустил его в обмен на чистосердечное признание. Надо сказать, он не горел желанием выполнять приказ Роденбурга, но не повиноваться тоже не мог. Он сказал, что на службу к Роденбургу более не вернётся - последнее грязное поручение переполнило чашу его терпения.";
			link.l1.go = "Stivesant_8";			
		break;
		
		case "Stivesant_8":
			dialog.text = "Так... Капитан " + GetFullName(pchar) + ", вы отправитесь со мной в Виллемстад. Становитесь на 'Мейфенг' впереди моего корабля. Если всё сказанное вами окажется правдой - Лукас должен быть арестован, и вы окажете в этом содействие. Но, если вы мне солгали... пеняйте на себя.";
		...
		case "Keller_1":
			...
			link.l1 = "Минхер Келлер, помните, несколько месяцев назад вы встретили по пути в Виллемстад флейт. На нём была семья еврейских беженцев. Вы отобедали на борту этого корабля, затем отправились в дальнейший путь.";
			link.l1.go = "Keller_2";
		break;
		
		case "Keller_2":
			dialog.text = "Да, кажется, я припоминаю нечто подобное. Да, точно, вспомнил. И в чём же дело?";
			link.l1 = "Флейт был потоплен в этот же день пиратом. Спастись удалось только двоим беженцам - Соломону и его дочери Абигайль. Они укрылись на острове буквально в нескольких милях от места нападения. Мне нужно найти этот остров. Вспомните, пожалуйста, где было место вашей встречи с этим флейтом? Это очень важно.";
			link.l1.go = "Keller_3";
		break;
		
		case "Keller_3":
			dialog.text = "И всего-то? Я знаю этот остров. Он совсем близко отсюда, между Тринидадом и Кюрасао. Маленький, пустынный необитаемый островок. На нём нет даже индейцев.";
			link.l1 = "Вы можете назвать приблизительные координаты?";
			link.l1.go = "Keller_4";
		break;
		
		case "Keller_4":
			dialog.text = "Я назову вам точные. 12 гр. 48' северной широты и 64 гр. 41' градусов западной долготы. Ищите в этом районе. Остров маленький, но, думаю, вы его без труда найдёте.";
		...
		case "FishingBoat":
			...
			dialog.text = "Да ничего особенного, это рыбацкое судно, ловим тут рыбу. Надо же как-то на жизнь зарабатывать.";
			link.l1 = "Да, согласен, времена нынче такие...";
			link.l1.go = "FishingBoat_1";			
		break;
		
		case "FishingBoat_1":
			Diag.TempNode = "FishingBoat_1";
			dialog.text = "Так что вы хотите?";
			link.l1 = "Я хотел"+ GetSexPhrase("","а") +" бы узнать последние новости архипелага.";
            link.l1.go = "rumours_capitan";
			link.l2 = "Как у вас сегодня с уловом? Может продадите? Мне бы провиант не помешал...";
			link.l2.go = "FishingBoat_2";
			link.l3 = "Ничего особенного, просто "+ GetSexPhrase("зашёл","зашла") +" поприветствовать вас!";
    		link.l3.go = "exit";	
		...
		case "FishingBoat_2":
			if(GetDataDay() < 11) 
			{
				...
				dialog.text = "Улов сегодня хороший, только успевай сети вытаскивать. Можем и вам продать, даже дешевле, чем в магазине - всё равно мы и купцу местному рыбу продаём. Если перевести всё это в провиант... У нас сейчас его "+iTemp+", можем продать по цене в "+iMoney+" песо за штуку. Сколько возьмёте?";
				link.l1.edit = 3;
				link.l1 = "";	
				link.l1.go = "Количество вбитого провианта";
				link.l2 = "Знаете, я передумал покупать у вас провиант. ";	
				link.l2.go = "exit";
			...
			{
				Diag.TempNode = "FishingBoat_1";
				dialog.text = "Да что-то, если честно, вообще рыбы нет... Почти ничего и не наловили сегодня. Зато гарпуны лишние остались - думали крупную рыбу какую-нибудь половить или кита, если повезёт... Может вам гарпуны нужны?";
				link.l1 = "Нет, гарпуны меня не интересуют";
				link.l1.go = "exit";
				link.l2 = "Хм, гарпун - тоже полезная вещь в умелых руках. Пожалуй, куплю.";
				link.l2.go = "Гарпуны";
			}
			if(GetDataDay() > 20)
			{
				dialog.text = "Да что-то, если честно, вообще рыбы нет... Почти ничего и не наловили сегодня. Так что и продать особо нечего. Разве что... Есть у меня несколько побрякушек, может быть вас что-то заинтересует. Желаете посмотреть?";
				link.l1 = "Нет, побрякушки меня не интересуют.";
				link.l1.go = "exit";
				link.l2 = "Жаль, что улова нет. Ну, давайте, хоть побрякушки ваши посмотрю - вдруг что-то приглянется... ";
				link.l2.go = "Торговля";
			}	
		break;
		...
		case "Количество вбитого провианта":
			...
			if(iQty > iTemp)
			{
				dialog.text = "Ну, вы загнули... У меня столько нет! Может поменьше купите?";
				link.l1.edit = 3;
				link.l1 = "";	
			...
			if(iQty < 1)
			{
				dialog.text = "Шутите что ли? Вам нужен провиант или нет? Если да, то сколько?";
				link.l1.edit = 3;
				link.l1 = "";	
				link.l1.go = "Количество вбитого провианта повтор";	
				link.l2 = "Знаете, я передумал покупать у вас провиант.";	
			...
			{
				dialog.text = "Идёт! С вас "+sti(iQty*iMoney)+" песо.";
				if(sti(pchar.money) < iQty*iMoney)
				{
					link.l1 = "Хм, у меня нет столько денег. Давайте в другой раз. ";
					link.l1.go = "exit";
				}
				else
				{
					link.l1 = "По рукам! Перетаскивайте товар ко мне на корабль. ";
		...
		case "Гарпуны":
				...
				iTemp = 5+drand(5);
				pchar.GenQuest.FishingBoatITemp = iTemp;
				dialog.text = "У нас "+iTemp+" штук есть. Можем продать по сотне песо за гарпун. Сколько нужно?";
				link.l1.edit = 3;
				link.l1 = "";	
				link.l1.go = "Количество вбитых гарпунов";
				link.l2 = "Знаете, я передумал, гарпуны мне не нужны.";	
				link.l2.go = "exit";
		break;
		...
		case "Количество вбитых гарпунов":
			...
			if(iQty < 1)
			{
				dialog.text = "Шутите что ли? Вам нужны гарпуны или нет? Если да, то сколько?";
				link.l1.edit = 3;
				link.l1 = "";	
				link.l1.go = "Количество вбитых гарпунов заново";	
				link.l2 = "Знаете, я передумал, гарпуны мне не нужны.";	
			...
			if(iQty > 0 && iQty <= iTemp)
			{
				dialog.text = "Идёт! С вас "+sti(iQty*100)+" песо";
				if(sti(pchar.money) < iQty*100)
				{
					link.l1 = "Хм, у меня нет столько денег. Давай в другой раз. ";
					link.l1.go = "exit";
				}
				else
				{
					link.l1 = "По рукам! Я им найду применение.";
		...
		case "Особые товары":
			if(drand(2) == 0)
			{
				Diag.TempNode = "First time";
				Diag.CurrentNode = "First time";
				pchar.GenQuest.FishingBoatDialogEnb = "нет особых товаров";
				dialog.text = "Нет, больше ничего нет. Это же рыбацкое судно! Что здесь ещё может быть?";
			...
			if(drand(2) == 1)
			{
				if(ChangeContrabandRelation(pchar, 0) > 5)
				{
					...
					dialog.text = "Ну... есть тут у меня одна вещица... Придержал у себя, думал продать кому-нибудь. Но я за неё дорого возьму! И второй раз предлагать не стану. Будете смотреть?";
					link.l1 = "Показывайте, посмотрим, что за вещица.";
			...
			if(drand(2) == 2)
			{
				...
				if(ChangeContrabandRelation(pchar, 0) > 5)
				{
					...
					dialog.text = "Ну... Есть тут у меня один товар... Чисто случайно на берегу бухты нашёл. Это "+iQty+" "+sTemp+". Цена за штуку - "+iMoney+". Но продам только всё сразу и второй раз не предложу. И чтобы об этом никто посторонний не узнал... понимаете меня?";
					link.l1 = "Нет, сомнительный товар мне не нужен, спасибо. Никому не скажу, не переживайте. ";
					link.l1.go = "exit";
					if(sti(pchar.money) >= iMoney*iQty)
					{
						link.l2 = "Я вас понимаю. Но товар того стоит. Я беру его. Прикажите доставить его на мой корабль.";
		...
		case "Особые товары предметы":
			...
			if(ChangeContrabandRelation(pchar, 0) > 5)
			{
				...
				link.l2 = "Нет, мне это ни к чему. Не буду брать. ";
				link.l2.go = "exit";
			}
