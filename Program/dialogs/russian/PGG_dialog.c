...
//			link.l4.go = "rumours_capitan";
		...
		{
			link.l8 = PCharRepPhrase("Вот это рожа. Так и хочется её разукрасить.", "А... наш знакомый мерзавчик. Не желаете ли пару лишних дырок в груди?");
			link.l8.go = "outraged";
		}
		link.l9 = "Ничего.";
		link.l9.go = "exit";
...
//==================================================
	case "officer":
		...
		{
			Dialog.Text = RandPhraseSimple("Да у тебя и так полный комплект.", "Не думаю, что ты готов"+ GetSexPhrase("","а") +" командовать мной.");
		...
		Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + 
				...
				LinkRandPhrase("Ваше предложение весьма лестно для меня. Я согласен за весьма умеренную плату в размере " + NPChar.Quest.Officer.Price + " песо.", 
							"Хм... Вы выглядите бывал"+ GetSexPhrase("ым","ой") +" моря"+ GetSexPhrase("ком","чкой") +". Возможно, мне стоит принять предложение, если вы согласны заплатить мне " + NPChar.Quest.Officer.Price + " песо.", 
							"Думаю, мы сработаемся капитан. Всего " + NPChar.Quest.Officer.Price + " песо и мы ударим по рукам."));
...
//==================================================
	...
	case "companion_leave":
		Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar,
					RandPhraseSimple("Ты мне надоел"+ GetSexPhrase("","а") +"!!! Я выхожу из состава эскадры!", "Моё терпение лопнуло! Я отчаливаю."), 
	...
	case "companion_stay":
		...
		Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar,
					RandPhraseSimple("Премия? Отлично! Сумма в " + NPChar.Quest.Companion.Price + " песо, меня устроит.", 
						"Пожалуй, если ты выплатишь моей команде " + NPChar.Quest.Companion.Price + " песо, я уговорю их остаться."), 
					RandPhraseSimple("Хорошо! Я готов остаться за скромную премию в размере " + NPChar.Quest.Companion.Price + " песо!",
						NPChar.Quest.Companion.Price + " песо устроят на некоторое время меня и мою команду."));
...
//			link.l1 = "Выкладывай сейчас, некогда мне по гостям расхаживать.";
			Dialog.Text = PCharRepPhrase(RandPhraseSimple("У меня есть остренькое дельце для Вас и ваших головорезов.", 
						...
						RandPhraseSimple("Только между нами. Давайте-ка, обсудим всё у меня в кают-компании на '" + NPChar.Ship.Name + "' за бочонком отличного эля. Сегодня.", "Здесь говорить об этом не резон, потолкуем у меня на судне '" + NPChar.Ship.Name + "' в тишине да в спокойствии. Не затягивайте.");
			link.l1 = PCharRepPhrase(RandSwear() + RandPhraseSimple("К чёрту проволочки! Хорошее хочу знать быстро, а плохое ещё быстрее!", 
		...
		link.l2 = PCharRepPhrase(RandPhraseSimple("Я приду! Но, если ты попусту потратишь моё время, я вырву тебе язык!", 
	...
	case "Quest_1_Ship":
		...
		{
			Dialog.Text = RandPhraseSimple("Э, нет... Так дело не пойдёт... Не много ли тебе кораблей, "+ GetSexPhrase("приятель","подруга") +"?", 
...
//		Dialog.Text = "Так, " + GetFullName(PChar) + ", что-то зря я с тобой связался, добыча утекла сквозь пальцы как вода. Прощай!";
		...
		link.l1 = PCharRepPhrase(RandPhraseSimple("Да и чёрт с этим караваном! Ловцы жемчуга ничуть не хуже, а риска никакого!", 
...
//			Dialog.Text = "Итак, добыча составила " + PChar.GenQuest.PGG_Quest.Goods.Taken + " " + PChar.GenQuest.PGG_Quest.Goods.Text + ". Давай делить. ";
			...
			link.l2 = PCharRepPhrase(RandPhraseSimple("И ты ещё требуешь свою долю? Да тебя надо вздёрнуть на рее и не мешкать!", 
