...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				dialog.text = TimeGreeting() + "! Добро пожаловать, " + GetAddress_Form(NPChar) + " в таверну рудника Лос-Текес. Еда, выпивка и... девочки - всё по умеренным ценам! Меня зовут "+GetFullName(npchar)+" и я всегда к вашим услугам.";
			...
			else
			{
				...
				link.l2.go = "room";
				link.l3 = LinkRandPhrase("Расскажи мне последние новости?","Что новенького в этих краях?","Как течёт жизнь на суше?");
				link.l3.go = "rumours_tavern";
				link.l4 = "Нет, ничего не надо, "+npchar.name+". Я просто зашёл поздороваться с тобой.";
				link.l4.go = "exit";
			}
		...
		case "drink":
			...
			else
			{
				...
				dialog.text = "Пожалуйста, капитан. Всего за пять песо - пинта моего лучшего чёрного ямайского рома!";
		...
		case "MineFuckGirl":
			dialog.text = "Рада вас видеть, белый сеньор. И не смотреть на "+npchar.name+" так, "+npchar.name+" уметь делать всё не хуже белых девок из бордель. Ах, сеньор такой симпатичный... Обещать, вы остаться доволен.";
