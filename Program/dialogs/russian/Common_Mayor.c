	...
	switch (Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (CheckAttribute(pchar, "questTemp.Sharlie.Lock") && npchar.city == "FortFrance")
			{
				...
				{
					link.l3 = "Месье, я нашёл в джунглях труп бандита, убитого индейцами. На теле кроме прочего, были вот эти серьги - явно привезённые из Европы. Может, они принадлежали кому-то из знатных граждан вашего города?";
			...
			{
    			dialog.text = "Добро пожаловать на Карибы, капитан де Мор! Теперь уже - по-настоящему.";
			...
			if (CheckAttribute(pchar, "questTemp.Portugal") && pchar.questTemp.Portugal == "MartiniqueGovernor" && npchar.city == "FortFrance")
			{
				...
				link.l1 = "Я прибыл"+GetSexPhrase("","а")+" при первой же возможности, Ваше Превосходительство. Мне передали что-то о неотложном деле, я могу оказать вам услугу?";
				link.l1.go = "Portugal"; 
				break;
			}			
			//<-- Португалец
            ...
            if (npchar.quest.meeting == "0")
			{
				...
				{
					dialog.text = TimeGreeting() +", мисс МакАртур.";
					link.l1 = TimeGreeting() +", Ваша Светлость.";
			...
			else
			{
                ...
                {//->
                    ...
                    else
                    {
						...
						{
							link.l11 = "На берегу " + XI_ConvertString(PChar.GenQuest.PiratesOnUninhabited.StartShore + "Gen") + " я встретил"+ GetSexPhrase("","а") +" пиратов. Все " + PChar.GenQuest.PiratesOnUninhabited.PiratesCount + " человек сейчас у меня на борту. Хочу сдать их властям."; // belamour gen
						...
						if(CheckAttribute(pchar,"GenQuest.ShipWreck") && pchar.GenQuest.ShipWreck == "SailorsOnShore" && GetQuestPastDayParam("GenQuest.ShipWreck") < 25  && sti(npchar.nation) == sti(pchar.GenQuest.ShipWreck.Nation))
						{
							if(!CheckAttribute(pchar,"GenQuest.ShipWreck.SpeakMayor"))
							{
								...
								{
									link.l13 = "На берегу " +  XI_ConvertString(pchar.GenQuest.ShipWreck.Shore + "Gen") + " я встретил" + GetSexPhrase("","а") + " капитана по имени " + pchar.GenQuest.ShipWreck.Name + " и его моряков, высаженных на берег пиратом, которого зовут " + pchar.GenQuest.ShipWreck.BadName + ". " + // belamour gen
								...
								{
									link.l13 = "На берегу " + XI_ConvertString(pchar.GenQuest.ShipWreck.Shore + "Gen") + " я встретил"+ GetSexPhrase("","а") +" моряков с потерпевшего крушение судна '" + pchar.GenQuest.ShipWreck.ShipTypeName + "' во главе с их капитаном. " +  // belamour gen
										"К сожалению, перегруз команды на моём судне не позволил взять их на борт. Но я оставил" + GetSexPhrase("","а") + " им провизию и оружие. Если экспедиционное судно не успеет забрать их в течение " + (30 - iDay) + " дней, то не знаю, удастся ли застать их живыми.";
									link.l13.go = "ShipWreck1";
								}
							}	
						...
						if (CheckAttribute(PChar, "questTemp.WPU.Postcureer.LevelUp"))
						{
							...
							{
							link.l14 = "Я доставил"+GetSexPhrase("","а")+" вам депешу из города "+XI_ConvertString("Colony"+pchar.questTemp.WPU.Postcureer.StartCity+"Gen")+".";
						...
						{
							link.l15 = UpperFirst(GetAddress_FormToNPC(NPChar)) + ", я обнаружил"+GetSexPhrase("","а")+" банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит у " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Gen") + "."; // belamour gen
		...
		case "ShipWreck1":
			...
			{
				dialog.text = "Как вы сказали? " + pchar.GenQuest.ShipWreck.Name +"? Это хорошая новость! Ведь мы уже не надеялись увидеть его живым. Я немедленно организую спасательную экспедицию. И возьмите это скромное вознаграждение за Вашу расторопность и участие в их спасении."; // belamour gen
		...
		case "node_1":
            ...
            {//->
                ...
                else
                {
					...
					if(CheckAttribute(PChar, "GenQuest.JusticeOnSale.MayorWait") && CheckAttribute(PChar, "GenQuest.JusticeOnSale.CityId") && NPChar.location == PChar.GenQuest.JusticeOnSale.CityId + "_townhall")
					{
						link.l15 = "Милорд, я обнаружил"+GetSexPhrase("","а")+" банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит у " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Gen") + "."; // belamour gen
						link.l15.go = "JusticeOnSale_1";
					}
					//<-- Правосудие на продажу
					...
					if (CheckAttribute(pchar, "questTemp.Regata.ToPortRoyal") && NPChar.location == "Portroyal_townhall")
					{
						link.l4 = "Я прибыл"+GetSexPhrase("","а")+" по приглашению для участия в регате. Вот моё приглашение.";
						link.l4.go = "Regata";
					}
		...
		case "Helen_node_1":
            ...
            {//->
                ...
                {
                    if (CheckAttribute(aData, "PlayerHelpMayor")) DeleteAttribute(aData, "PlayerHelpMayor");
                    dialog.text = "Что вас ко мне привело? Я думал, вы решаете все свои вопросы с вашим... покровителем. Но говорите - всегда рад.";
					if (npchar.city != "Panama") // Addon-2016 Jason
					{
						link.l1 = "Я хочу поговорить о работе на благо короны "+ NationNameGenitive(sti(NPChar.nation))+".";
						link.l1.go = "work";
					...
					if(CheckAttribute(PChar, "GenQuest.JusticeOnSale.MayorWait") && CheckAttribute(PChar, "GenQuest.JusticeOnSale.CityId") && NPChar.location == PChar.GenQuest.JusticeOnSale.CityId + "_townhall")
					{
						link.l15 = "Милорд, я обнаружил"+GetSexPhrase("","а")+" "+GetSexPhrase("","а")+"банду контрабандистов, чьего главаря недавно поймала стража. Они планируют вызволить его. Их корабль '" + PChar.GenQuest.JusticeOnSale.ShipName + "' стоит у " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Gen") + "."; // belamour gen
						link.l15.go = "JusticeOnSale_1";
					}
					//<-- Правосудие на продажу
		...
		case "Play_Game":
			...
			{
    			dialog.text = "Сейчас не до этого! У нас проверка, дон де Аламида в городе. Ох, каждый год одно и то же, и за что мне всё это...";
				link.l1 = "Как скажете. Не буду отвлекать.";
				link.l1.go = "exit"; 
				break;
			}
			//<== прибыла инспекция на Святом Милосердии
        ...
        case "Card_Game":
            if (!CheckNPCQuestDate(npchar, "Card_date_Yet") || sti(PChar.rank) < 4 || isBadReputation(pchar, 50) || GetCharacterSkillToOld(pchar, SKILL_LEADERSHIP) < rand(5))
            {
				...
				// belamour legendary edition фикс проверки мундира с мушкетом
				if(IsUniformEquip()) dialog.text = "Прошу меня извинить, монсеньор, но сейчас у меня нет на это времени. Может быть, в другой раз."; // Jason НСО
			...
			{		 				
				...
				if (QuestName == "TakeArsenalship")
				{
					switch (pchar.GenQuest.TakeArsenalship)
					{
						...
						break;
						case "Late":
							dialog.text = RandPhraseSimple("Время, данное вам на поиск и уничтожение военного транспорта вышло. И как вы думаете, что мне докладывают? А я вам скажу - вражеский конвой благополучно дошёл до места назначения! Извольте объясниться, " + GetAddress_Form(NPChar) + ".",
				...
				if (QuestName == "TakePassenger")
				{
					switch (pchar.GenQuest.TakePassenger)
					{
						...
						case "Found":
							...
							link.l1 = "К сожалению, нет, " + GetAddress_FormToNPC(NPChar) + ". Мне не удалось захватить негодяя. Он утонул вместе с кораблём, на котором плыл.";
							link.l1.go = "All_Found";		
						break;
						case "Execute":
							dialog.text = RandPhraseSimple("Итак, что скажете? Вы захватили мерзавца?", "Скажите мне только одно - гнусный мерзавец в кандалах у вас в трюме?");
...
//navy <
    		...
    		{	
				...
				if(CheckAttribute(pchar, "questTemp.StatusCity") && pchar.questTemp.StatusCity == npchar.city)
				{
					...
					link.l1 = "Моё судно всегда в боевой готовности. Введите меня в курс предстоящего дела, Ваша Светлость.";
		...
		case "TakePassenger":
			...
			dialog.text = "Безусловно. Речь пойдёт о мерзавце по имени "+pchar.GenQuest.TakePassenger.Name+". "+sText+" Я давно разыскиваю его, и, наконец, мне удалось получить достоверные сведения, где его искать. Мне нужно, чтобы вы доставили сюда этого человека, причём живым. Я хочу показательно вздёрнуть его на площади нашего города. Вы готовы взяться за это дело?";
		...
		case "All_Late":
			...
			AddSimpleRumour(LinkRandPhrase("Вы знаете, губернатор " + GetFullName(npchar) + " очень недоволен капитаном " + GetMainCharacterNameDat() + ". Представляете, тот не успел выполнить его поручение!", 
				"Губернатор " + GetFullName(npchar) + " несколько раздражён тем, что некий капитан " + GetFullName(pchar) + " взялся за дело, но не успел выполнить его в срок. Нехорошо...", 
...
//<-- осады homo
		...
		case "Sharlie":
			...
			link.l1 = "Я его брат. Полагаю, что Мишель, скорее всего, должен быть где-то на Мартинике. Мне известно, что он попал в неприятности, я приехал сюда, чтобы разобраться, что произошло, и...";
		...
		case "Sharlie_1":
			...
			dialog.text = "Вот как? Мне ничего не известно о существовании братьев у месье де Монпе... Вот что, сударь: вы мне кажетесь очень подозрительным, и я вас арестую для дальнейшего разбирательства. Лейтенант! Взять этого человека под стражу!";
		...
		case "Sharlie_2":
			...
			link.l1 = "Благодарю, месье. С учётом того, что меня обобрали до нитки, это будет весьма кстати.";
			// belamour legendary edition -->
			link.l1.go = "Sharlie_5";		
			link.l2 = "Благодарю, месье. Может, вы сможете ещё как-нибудь выручить меня? Как дворянин дворянина! Сами же видите, в каком кошмарном я положении!";
			link.l2.go = "Sharlie_3";	
		break;
		
		case "Sharlie_3":
			dialog.text = "Ну, Шарль... Вот, держите ещё этот священный амулет, на удачу. Больше ничем помочь не могу, уж извините. Привыкайте, что в колониях вам придётся полагаться только на себя.";
			link.l1 = "Ну, хоть что-то. Спасибо за подарок, месье.";
		...
		case "Sharlie_4":
			...
			//Log_Info("Вы получили амулет 'Святая вода'");
		...
		//Sinistra --> Травля крысы		
		case "TK_TravlyaKrys":
			dialog.text = "Удивительный поворот судьбы, Шарль... Впервые вижу, чтобы новый человек так быстро поднимался в нашем мире. Уверен, мы с вами подружимся.";
			link.l1 = "Почту за честь. Возможно, у вас даже будет для меня поручение?";
		...
		case "TK_TravlyaKrys_2":
			dialog.text = "Безусловно! Ведь вы успешно раздавили одну из крыс Барбазона в морском бою у Ле Франсуа.";
			link.l1 = "Не могу сказать, что у меня остались самые лучшие воспоминания о том инциденте. А кто такой Барбазон?";
			link.l1.go = "TK_TravlyaKrys_3";
		break;
		
		case "TK_TravlyaKrys_3":
			dialog.text = "Жак Барбазон, он же Жак Добряк - главарь бандитского гнезда Ле Франсуа, этого грязного пятна на нашем дивном острове! Один из так называемых пиратских баронов, промышляет пиратством и контрабандой на Малых Антильских островах. В общем, самый заурядный преступник и, клянусь, висеть ему однажды на площади Сен-Пьера!";
			link.l1 = "Простите, Ваша Светлость, но разве не вы истинный хозяин Мартиники? Вы же можете просто взять штурмом это гнездо! Я там бывал и, поверьте, особого впечатления оно не производит.";
			link.l1.go = "TK_TravlyaKrys_4";
		break;
		
		case "TK_TravlyaKrys_4":
			dialog.text = "Вы бы лучше не бахвалились таким знакомством с географией моего острова, Шарль. Для дворянина всё же стыдно посещать подобные вертепы. Ну, а что касается вашего несколько наивного вопроса\nДорогой мой, не всё в нашем мире можно решить грубой силой. Существуют ещё политика и... деловые интересы. Но не буду вас утомлять лишними подробностями: у меня есть конкретное поручение как раз по части грубой силы.";
			link.l1 = "Хм. В чём будет заключаться моя задача?";
			link.l1.go = "TK_TravlyaKrys_5";
		break;
		
		case "TK_TravlyaKrys_5":
			dialog.text = "Барбазон привлекает к себе целый рой неудачников, жадных до золота и крови. Иногда он даже кидает со своего стола кость одному из них, чтобы полюбоваться, как грызётся вся свора. Выжившего он порой берёт в дело, но чаще выкидывает за порог, и, поверьте, для благополучия колонии нет ничего хуже, чем пират, которому нечего терять.";
			link.l1 = "Кажется, я понимаю, что конкретно требуется, но не уверен, что это посильное для меня дело. Я ведь только недавно начал постигать морскую науку...";
			link.l1.go = "TK_TravlyaKrys_6";
		break;
		
		case "TK_TravlyaKrys_6":
			dialog.text = "Глупости! У вас уже есть одна победа в кармане - это больше, чем у большинства бездельников из числа моих офицеров! За уничтожение ещё одной крысы Барбазона город заплатит вам пять тысяч песо, это уже без налогов. Восемь тысяч, если возьмёте пирата в плен, - для дальнейшей показательной казни, разумеется.";
			link.l1 = "Признаться, деньги мне не помешают. Где мне искать этого пирата?";
			link.l1.go = "TK_TravlyaKrys_7";
			link.l2 = "Простите, Ваша Светлость, однако я пока не готов к такого рода работе.";
			link.l2.go = "TK_TravlyaKrys_Otkaz";
		break;
		
		case "TK_TravlyaKrys_Otkaz":
			dialog.text = "Жаль, Шарль. В таком случае, не буду вас больше задерживать.";
			link.l1 = "До свидания, Ваша Светлость.";
			link.l1.go = "exit";
		break;
		
		case "TK_TravlyaKrys_7":
			dialog.text = "Рад, что вы согласились, Шарль. Разведка докладывает, что люггер 'Хейтер' прямо сейчас готовится к выходу из Ле-Франсуа. Медлить нельзя, так что выходите в море и вступайте в бой. Не бойтесь, друг мой, у вас всё получится.";
			link.l1 = "Дай Бог, Ваша Светлость.";
		...
		case "TK_TravlyaKrys_8":
			...
			sld.name = "Даниэль";
			sld.lastname = "Монбар";
			FantomMakeSmallSailor(sld, SHIP_LUGGER, "Хейтер", CANNON_TYPE_CANNON_LBS3, 20, 40, 20, 25, 35);
		...
		case "Portugal":
			...
			link.l1 = "Тогда я "+GetSexPhrase("весь","вся")+" внимание.";
			link.l1.go = "Portugal_1";
		break;
		
		case "Portugal_1":
			dialog.text = "Местные рыбаки ещё вчера вечером видели вблизи острова странное судно. По их словам, это чуть ли не линейный корабль, без флагов и сигнальных огней. Не думаю, однако, что это судно на самом деле больше фрегата, но и это уже чересчур!";
			link.l1 = "Да, у страха глаза, как известно, велики... но в чём суть дела?";
			link.l1.go = "Portugal_2";
		break;
		
		case "Portugal_2":
			dialog.text = "Я как раз перехожу к ней, капитан. Мои торговые партнёры взволнованы этими слухами - они полагают, что это пираты, либо испанский рейдер, а и то и то равно плохо скажется на торговле. Хуже всего другое - то, что в колонии сейчас нет военных кораблей, только пара шлюпов охранения да старая шхуна, годные лишь гонять контрабандистов\nА я не могу оставить это без внимания... во всяком случае теперь, когда о загадочном корабле судачит полгорода! Прошу вас, присоединитесь со своим судном к патрулированию - я знаю вас, как неплохого капитана, и, даже если там действительно небольшой фрегат, может ваши орудия убедят их поискать добычу в другом месте!";
		...
		case "Portugal_4":
			...
			link.l1 = "Хорошо, я всё равно планировал"+GetSexPhrase("","а")+" задержаться тут на несколько дней.";
			link.l1.go = "Portugal_5";
		break;
		
		case "Portugal_5":
			dialog.text = "Прекрасно, я в вас не сомневался, "+GetAddress_Form(NPChar)+" капитан! Помните - два дня в наших водах, и оговоренная сумма ваша. В случае если вы вступите в сражение - плата соответственно результату.";
			link.l1 = "Я понял"+GetSexPhrase("","а")+", Ваше Превосходительство. Наш контракт заключён, я отправляюсь на корабль.";
		...
		case "JusticeOnSale_1":
			...
			link.l1 = "Рад"+GetSexPhrase("","а")+" был"+GetSexPhrase("","а")+" помочь, разрешите откланяться?";
		...
		case "JusticeOnSale_4":
			dialog.text = "Вы правы, капитан! Мы не можем это так оставить! Я прошу вас отправиться к " + XI_ConvertString(PChar.GenQuest.JusticeOnSale.ShoreId + "Dat") + ", найти их корабль, и взять всю их шайку! Если будут сопротивляться - применяйте силу, не раздумывая!"; // belamour gen
			link.l1 = "Разрешите откланяться.";
			link.l1.go = "JusticeOnSale_5";
		break;
		
		case "JusticeOnSale_5":
			dialog.text = "Ступайте. Доплывите туда, и проследите, чтобы никто не ушёл от правосудия! Думаю, вам сначала надо будет высадиться на берег, а потом уже заняться кораблём.";
			link.l1 = "...";
		...
		case "Regata":
			...
			{
				notification("Требуется 20 уровень", "None");
				dialog.text = "А-а, замечательно, рад вас видеть, капитан! Я наслышан о ваших талантах, но приходите, когда действительно будете готовы.";
				link.l1 = "Да, конечно. Как скажете.";
				link.l1.go = "exit";
			}
			else
			{
			...
			link.l1 = "Да, сэр, "+GetSexPhrase("прочёл","прочла")+".";
			link.l1.go = "Regata_1";
		...
		//регата-финал
		case "Regata_Final":
			dialog.text = "А, вот и победитель"+GetSexPhrase("","ница")+" регаты! Добрый день, капитан " + GetFullName(pchar) + "! Поздравляю вас с этим значительным успехом - победители нашей регаты всегда пользуются заслуженной популярностью в английских колониях. ";
		...
		case "Regata_complete_1":
			...
			dialog.text = "Также вам положен комплект ценных призов: асоледа, миланский панцирь и буссоль. Теперь всё это - ваше.";
			// <-- legendary edition
			link.l1 = "Очень рад"+GetSexPhrase("","а")+", сэр. Право, даже не ожидал"+GetSexPhrase("","а")+".";
