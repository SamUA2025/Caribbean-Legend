...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
            ...
            {
                link.l1 = "Я направляюсь в Блювельд, паприку продавать. Скажите, мистер, нет ли у вас сейчас пассажиров до Блювельда? Ну, или до Порт-Ройаля. Было бы неплохо подвезти, глядишь, покрыли бы убытки на оплату команде. Я и так её сократил до минимума, но всё равно, канальи, денег-то просят...";
		...
		//Голландский гамбит
		case "PortofficeDone":
			dialog.text = "Пассажиров ни до Блювельда, ни до Порт-Ройаля нет, к сожалению. Зайдите завтра, или послезавтра - может, появятся.";
		...
		if(sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)//проверка на наличие корабля в порту
		{
			...
			if (CheckAttribute(pchar, "questTemp.Regata.Breach") || !CheckAttribute(pchar, "questTemp.Regata.Sentjons") || GetCompanionQuantity(pchar) > 1 || !bRegLugger || pchar.Ship.Name != "Сaнта-Kатepина")
			{
