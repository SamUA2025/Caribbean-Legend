void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Hold_GenQuest1":
			...
			link.l1 = "Я разыскиваю некоего господина по имени " + pchar.GenQuest.Hold_GenQuest.Name + "?"; // belamour gen
		...
		case "Hold_GenQuest2":
			...
			link.l1 = "Я капитан " + GetFullName(pchar) + ", у меня к вам дело относительно одного человека. Его зовут " + pchar.GenQuest.Hold_GenQuest.CapName + "."; // belamour gen
		...
		case "Hold_GenQuest40":
			...
			dialog.text = "Совершенно верно, я - " + pchar.GenQuest.Hold_GenQuest.Name + "! Но имя " + pchar.GenQuest.Hold_GenQuest.CapName + " мне не о чем не говорит."; // belamour gen
		...
		case "ShipWreck_19":
			dialog.text = "А что я могу сделать?.. по крайней мере сейчас. Я благодарен Богу, что хоть жив остался! Но, если вы поможете мне вернуть корабль и отомстить за унижение, то я готов"+GetSexPhrase("","а")+" отдать вам всё содержимое трюма, которое сумеет награбить этот негодяй.";
			...
			{
				AddQuestUserData("ShipWrecked", "count", pchar.GenQuest.ShipWreck.Qty + " его моряка");
			}
			// <-- belamour
			AddQuestUserData("ShipWrecked", "sBadName", pchar.GenQuest.ShipWreck.BadName);
			AddQuestUserData("ShipWrecked", "ShipName", pchar.GenQuest.ShipWreck.ShipTypeName);	
		...
		case "ShipWreck_43":
			...
			AddSimpleRumourCity(LinkRandPhrase(RandPhraseSimple("Говорят, что какой-то псих на острове объявился. Рассказывает, что потерял документы и пытается наняться капитаном на корабль до Дюнкерка...",
				...
				"Вас недавно какой-то псих искал, говорил, что якобы Вы ему денег должны..."), Locations[FindLocation(pchar.location)].fastreload, 5, 3,""); // belamour gen: pchar.location имеет вид _town, а запускать нужно в городе
			DialogExit();
			AddDialogExitQuestFunction("ShipWreck_GoOut");
		break;
		...
		case "Convict_11_2":
			dialog.text = "Да мы прихватили кое-что с " + GetStrSmallRegister(XI_ConvertString("MineType" + pchar.GenQuest.Convict.MineType + "Gen")) + ", чтоб на первое время не бедствовать, так что заплатим вдвойне. Только расчёт будет на месте. И не вздумайте привести стражу. Мы обид не прощаем..."; // belamour gen
			link.l1 = "Ну хорошо, ждите меня на берегу " + XI_ConvertString(pchar.GenQuest.Convict.ShoreLocation + "Gen") + ". Ближе к полуночи будет вам кораблик... И не надейтесь меня обмануть!"; // belamour gen
		...
		case "CaptainComission_7":
			...
			if(pchar.GenQuest.CaptainComission.GetSlave == "GetSlave")
			{
				...
				{
					dialog.text = LinkRandPhrase("А-а, приветствую вас, капитан " + GetFullName(pchar) + ". Как успехи?","Ну что, капитан, вызволили нашего страдальца?","Рад вас видеть, капитан, в добром здравии. Как продвигается наше общее дело?");	
					link.l1 = "Я привёз"+ GetSexPhrase("","ла") +" человека, о котором вы говорили. Он у меня на борту.";
		...
		case "CaptainComission_308":
			dialog.text = "Тогда давайте встретимся около " + XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Gen") + " сразу после полуночи, а то при моём нынешнем положении маячить на рейде слишком опасно. Как бы нас не обнаружил патруль."; // belamour gen
...
//			bQuestDisableMapEnter = true;
		...
		case "CaptainComission_310":
			dialog.text = "Я заберу ваше судно, а вас оставлю на берегу " + XI_ConvertString(pchar.GenQuest.CaptainComission.ShoreLocation + "Gen") + "."; // belamour gen
		...
		case "CaptainComission_321":		
			...
			dialog.text = "Ещё одна просьба. Власти наверняка организуют погоню, вы не могли бы сопроводить моё судно до " + XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Gen") + ", что на " + XI_ConvertString(GetIslandByCityName(sTemp) + "Voc") + "?"; // belamour gen
		...
		case "CaptainComission_380":
			...
			pchar.GenQuest.CaptainComission.ConvoyShore = SelectQuestShoreLocationFromSea(GetArealByCityName(pchar.GenQuest.CaptainComission.City));
			dialog.text = "Благодарю за спасение. Теперь нам нужно отправляться к " + XI_ConvertString(pchar.GenQuest.CaptainComission.ConvoyShore + "Dat") + ", там я укажу место тайника. Только нужно торопиться пока до него не добрались ищейки.";	// belamour gen	
		...
		case "ContraMeetManQuest_1":
			dialog.text = "Отлично! Нам нужно немедленно отправиться в таверну к контрабандисту - мне бы очень не хотелось задерживаться!";
				link.l1 = "Не вижу никаких проблем. Пойдём...";
		...
		case "ChurchGenQuest1_DialogShip_1":
			dialog.text = "...и шкоты! Шкоты поменять нужно!.. А, да - приветствую и всё такое. Не сочтите за грубость, г-"+ GetSexPhrase("н","жа") +" как-вас-там, но в данный момент я весьма занят и просил бы как можно быстрее изложить суть безделицы, которая привела вас на борт моего судна.";
				link.l1 = "Кхм. Собственно, дело и впрямь пустяшное. Насколько мне известно, в " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.ToColony + "Voc") + " вы получили от святого отца по имени " + PChar.GenQuest.ChurchQuest_1.ToName + " некоторые богословские труды, которые взялись доставить в " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.QuestTown) + ", поскольку направлялись в ту сторону."; // belamour gen
		...
		case "ChurchGenQuest1_DialogShip_2":
			dialog.text = "Я направлялся, да! Но этот проклятый шторм, чтоб его, чуть не прикончил мою старую посудину, и теперь я застрял здесь чёрт знает на сколько! Потому как эти мошенники в доках так и норовят залатать корпус гнилыми досками с разбитых лоханок, которые шторм прибил к берегу, а вместо парусов натянуть старую дерюгу, на которой околела ещё их прабабушка.";
				link.l1 = "Э-э... Сударь, в другой раз я с удовольствием выслушаю ваше мнение о прабабушке местных докеров, но сейчас я хотел"+ GetSexPhrase("","а") +" бы узнать судьбу книг и рукописей, которые вы получили в " + XI_ConvertString("Colony" + PChar.GenQuest.ChurchQuest_1.ToColony + "Voc") + "."; // belamour gen
...
//				link.l1.go = "ChurchGenQuest1_DialogShip_5_2_1"; // Сваливаем, поверили ему
		...
		case "PiratesOnUninhabited_5":
			dialog.text = "М-м, по ряду причин нам бы не хотелось попадаться на глаза представителям власти... Доставьте нас в какую-нибудь уютную бухту на обжитом острове или Мэйне. А дальше мы уж сами будем выбираться.";
			link.l1 = "Что ж, и рундуков, и гамаков на судне достаточно. Загружайтесь в шлюпку.";
		...
		case "PiratesOnUninhabited_12":
			...
			else
			{
				...
				link.l1 = "Ого! Поистине наглость человеческая безгранична. Единственное, что я могу добавить к уже сделанному - это не сообщать о вас властям.";
		...
		case "PiratesOnUninhabited_23":
			dialog.text = "Я понимаю Вас, капитан. Я Вас прекрасно понимаю... Кому охота возиться с бедными пиратами, по которым давно петля плачет?.. А если я предложу Вам обмен? Вы доставляете нас в любую бухту на обжитом острове или Мэйне, а я Вам за это даю карту клада... Идёт?";
		...
		case "PiratesOnUninhabited_38":
			...
			link.l1 = "Не захотел, остался недалеко от " + XI_ConvertString(PChar.GenQuest.PiratesOnUninhabited.StartShore + "Gen") + " меня дожидаться."; // belamour gen
		...
		case "PiratesOnUninhabited_40":
			dialog.text = "Раз уж ты ему так веришь, то можешь оставаться здесь. А мы заберём твой корабль, прихватим денежки и отправимся в направлении " + XI_ConvertString(PChar.GenQuest.PiratesOnUninhabited.StartShore + "Gen") + ". Нам очень хочется взглянуть в лицо " + PChar.GenQuest.PiratesOnUninhabited.MainPirateName + "."; // belamour gen
		...
		case "JusticeOnSale_1":
			...
			link.l1 = "Услугу? А в чём суть дела, и что вы предлагаете взамен?";
		...
		case "JusticeOnSale_3":
			dialog.text = "Наш капитан " + PChar.GenQuest.JusticeOnSale.SmugglerName + " попался патрулю из города " + XI_ConvertString("Colony" + PChar.GenQuest.JusticeOnSale.CityId + "Gen") + ", его бросили за решетку, но улик на него, чтобы к вдове тощей приставить, у них нет. Вытащить его самим нам не по силам\n" +
				"А вы человек уважаемый, вас в городе знают. Может, вы сможете договориться, внести залог, или просто выкупить его? Поверьте, мы сразу же уйдём, и никогда больше не вернемся на этот остров - ваша репутация ничуть не пострадает!"; // belamour gen
			link.l1 = "Положим, я смогу его вызволить. Но я не услышал"+GetSexPhrase("","а")+" ни слова о моей оплате!";
		...
		case "JusticeOnSale_4":
			...
			link.l3 = "Звонкая монета - радость моряка. Деньги, как известно, не пахнут - я соглас"+GetSexPhrase("ен","на")+".";
		...
		case "Unwantedpostor_room":
			dialog.text = ""+GetSexPhrase("А ты ещё кто такой? И с чего это вваливаешься в мою комнату?","Ого. Кто ты? Это, конечно, моя комната, однако я не против такой компании. Это доставка любви?")+"";
			link.l1 = ""+GetSexPhrase("Уже не твою. Трактирщик сдал комнату мне, а ты, поскольку за неё не платишь, сейчас уберёшься отсюда либо по-хорошему, либо по-плохому. Выбирай.","Размечтался. В нашем случае - уборка. Так что убирайся. Теперь это моя комната, ведь я, в отличие от тебя, всегда плачу трактирщикам.")+"";
		...
		case "Unwantedpostor_room_1":
			...
			dialog.text = ""+GetSexPhrase("Вот как? Ну, я выбираю по-плохому.","Дрянь! А ведь я хотел с тобой по-хорошему...")+"";
			link.l1 = ""+GetSexPhrase("Хозяин - барин...","И ты это называешь 'по-хорошему'? Мне жаль тебя. Хотя, пожалуй, нет.")+"";
		...
		case "Unwantedpostor_Win":
			dialog.text = ""+GetSexPhrase("Ладно, ладно, стой! Не убивай. Я ухожу."," Остановись! Что нам теперь, поубивать друг друга из-за этой комнаты?! Да подавись, она вся твоя. Стерва...")+"";
			link.l1 = ""+GetSexPhrase("Правильное решение. В другой раз думай головой: не все такие добрые, как я - могут и убить ненароком.","Мужчины.")+"";
		...
		case "Unwantedpostor_Lose":
			dialog.text = ""+GetSexPhrase("Ну что, выселил меня? Пошёл вон отсюда, пока я тебя не прирезал.","А разговоров-то было. Радуйся, что я ещё в хорошем настроении, а то ещё чего похуже бы с тобой сделал. Пошла вон отсюда!")+"";
			link.l1 = ""+GetSexPhrase("Чёрт...","Чёрт, недооценила тебя... Ладно, ладно, уже ухожу.")+"";
		...
		case "GuiltySailor":
			dialog.text = "А-а?.. "+GetSexPhrase("Пшёл вон, салага","Пшла вон, девка")+", я с кем попало... ик!.. не пью.";
			link.l1 = "Я тебе сейчас так тресну, что ты неделю ещё пить не будешь.";
			link.l1.go = "GuiltySailor_1";
		break;
		
		case "GuiltySailor_1":
			dialog.text = "Что?! Да ты знаешь, с кем разговариваешь? Я матрос с корабля '"+pchar.ship.name+"'! Стоит мне только сказать капитану - и ты даже из города свалить не успеешь! Катись отсюда, пока цел"+GetSexPhrase("","а")+"."; 
			link.l1 = "...";
		...
		case "GuiltySailor_3":
			dialog.text = "Капитан?..";
			link.l1 = "Месяц будешь драить палубу.";
		...
		case "ExpertSailor":
			...
			dialog.text = "Приветствую, капитан! Я знаю, что вы капитан, да.";
			link.l1 = "Приветствую. Откуда знаешь, что я капитан?";
			link.l1.go = "ExpertSailor_"+ExpertSailorVar;
		break;
		
		case "ExpertSailor_1": // матрос
			dialog.text = "Ха, я бывалый морской волк, не первый десяток лет хожу по морю. В основном занимался парусами: ходил и в штиль, и в шторм, и на люггере, и на линейном корабле. Правда в прошлом месяце ранение получил после тяжёлого боя, вот меня и списали на берег. Теперь тут вот сижу.";
			link.l1 = "Интересно. Мне бы специалист в таком деле не помешал. Не желаешь пойти ко мне в команду?";
		...
		case "ExpertSailor_2": // канонир
			dialog.text = "Ха, я бывалый морской волк, не первый десяток лет хожу по морю. В основном стоял у орудий: стрелял и из фальконетов, и из картаунов, и на люггере, и на линейном корабле. Правда в прошлом месяце ранение получил после тяжёлого боя, вот меня и списали на берег. Теперь тут вот сижу.";
			link.l1 = "Интересно. Мне бы специалист в таком деле не помешал. Не желаешь пойти ко мне в команду?";
		...
		case "ExpertSailor_3": // солдат
			dialog.text = "Ха, я бывалый морской волк, не первый десяток лет хожу по морю. В основном дрался в абордажах: и с пиратами сражался, и с регулярной армией, и на люггере, и на линейном корабле. Правда в прошлом месяце ранение получил после тяжёлого боя, вот меня и списали на берег. Теперь тут вот сижу.";
			link.l1 = "Интересно. Мне бы специалист в таком деле не помешал. Не желаешь пойти ко мне в команду?";
		...
		case "ExpertSailor_Again":
			dialog.text = "Удачи вам в море, капитан!";
		...
		case "NightAdventure_Sailor":
			...
			dialog.text = "Ты... ик!.. к-кто?..";
			link.l1 = "Я - капитан "+ GetFullName(pchar)+".";
			if (pchar.id == "Knippel")
			{
				link.l1 = "Извини, дружище, я очень спешу.";
				link.l1.go = "exit";
				LAi_CharacterDisableDialog(npchar);
				break;
			}
		...
		case "NightAdventure_Sailor_1_1":
			dialog.text = "Капитан?.. ик!.. К-капитан... проводите до т-таверны... ик!.. а? Пожалуйста. А то я... ик!.. к-кажется... з-заб... лу... дился в этой... в этой... к-колонии, да... ик!..";
			link.l1 = "Отстань, не до тебя сейчас.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Ладно, пошли. Сам ты, похоже, не дойдёшь...";
			link.l2.go = "NightAdventure_Sailor_1_2";
		break;
		
		case "NightAdventure_Sailor_1_2":
			dialog.text = "Только... ик!.. ме-е-едленно, по... пожалуйста. А то я... ик!.. в состоянии не... нестояния...";
			link.l1 = "Ладно, пойдём медленно.";
		...
		case "NightAdventure_Sailor_2_1":
			dialog.text = "Капитан?.. ик!.. К-капитан... помогите мне!";
			link.l1 = "Отстань, не до тебя сейчас.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Что у тебя стряслось?";
			link.l2.go = "NightAdventure_Sailor_2_2";
		break;
		
		case "NightAdventure_Sailor_2_2":
			dialog.text = "Там... в таверне... ик!.. шулер, во!";
			link.l1 = "Ну и что в этом такого? В любой таверне любой колонии шулеров, как грязи.";
			link.l1.go = "NightAdventure_Sailor_2_3";
		break;
		
		case "NightAdventure_Sailor_2_3":
			dialog.text = "К-капитан, вы... не понимаете... ик!.. Он м-меня... без единого песо оставил! Ик!.. А потом ещ-щё и... прогнал... ик!..";
			link.l1 = "Так, и что же ты теперь от меня хочешь?";
			link.l1.go = "NightAdventure_Sailor_2_4";
		break;
		
		case "NightAdventure_Sailor_2_4":
			dialog.text = "Сходите с-со мной... ик!.. Разберитесь с ним! П-пусть... ик!.. вернёт мне д-деньги! Ик!..";
			link.l1 = "Не нужно быть шулером, чтобы обыграть в доску пьяного человека. В следующий раз пей меньше.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Ну за что мне это, а?.. Ладно, пошли, посмотрим на твоего шулера.";
		...
		case "NA_SailorTavern":
			...
			link.l1 = "Да ладно, что уж... Не пей так больше. Или хотя бы приключений не ищи.";
		...
		case "NA_SailorTavern_1":
			...
			if(GetDataDay() >= 7  && GetDataDay() < 13) {GiveItem2Character(pchar, "slave_01");}
			if(GetDataDay() >= 13 && GetDataDay() < 19) {GiveItem2Character(pchar, "potion5");}
			if(GetDataDay() >= 19 && GetDataDay() < 25) {GiveItem2Character(pchar, "cartridge");}
			if(GetDataDay() >= 25) 
			{
				if(GetDataDay() == 25) {GiveItem2Character(pchar, "amulet_8");}
				if(GetDataDay() == 26) {GiveItem2Character(pchar, "amulet_9");}
				if(GetDataDay() == 27) {GiveItem2Character(pchar, "obereg_7");}
				if(GetDataDay() >  27) {GiveItem2Character(pchar, "obereg_11");}
			}
		...
		case "NA_SailorGambler":
			...
			{
				dialog.text = "К-капитан!.. Шулер!..";
				link.l1 = "Кто, я - шулер?";
				link.l1.go = "NA_SailorGamblerBelka";
			}
			else	
			{
				dialog.text = "В-вон он, капитан! Ик!.. За столом с-сидит...";
				link.l1 = "Сейчас разберёмся...";
		...
		case "NA_SailorGamblerBelka": 
			dialog.text = "Да нет же... ик!.. Шулер!.. Он исчез!..";
			link.l1 = "Ну, ушёл, значит, не стал тебя дожидаться.";
			link.l1.go = "NA_SailorGamblerBelka_1";
		break;
		
		case "NA_SailorGamblerBelka_1":
			dialog.text = "И что... теперь д-делать? Ик!..";
			link.l1 = "Ну, теперь ничего уже не сделаешь. Искать его ночью по всей колонии я точно не буду. Не пей так больше. Или хотя бы не играй в азартные игры в нетрезвом состоянии.";
		...
		case "NightAdventure_CardProf":
			dialog.text = "Предлагаю постучать костяшками. Ты как?";
			link.l1 = "Никак. Я капитан корабля и с шулерами не играю. Зато запросто могу борта помять, если деньги не вернёшь вон тому товарищу.";
			link.l1.go = "NightAdventure_CardProf_1";
		break;
		
		case "NightAdventure_CardProf_1":
			dialog.text = "С шулерами?! Это я-то шулер? Кому деньги вернуть - тому пьянице?! Обижаете, капитан...";
			link.l1 = "И не так ещё обидеть могу. Деньги возвращай.";
			link.l1.go = "NightAdventure_CardProf_2";
		break;
		
		case "NightAdventure_CardProf_2":
			dialog.text = "Да никакой я не шулер! Я честный игрок. Побеждает удача, а вашему другу просто не повезло, вот и всё.";
			link.l1 = "Удача, говоришь? А давай ты мне сейчас дашь свои кости, а я посмотрю, с какой удачей они будут выпадать.";
		...
		case "NightAdventure_CardProf_3":
			...
			{
				dialog.text = "Да пожалуйста, вот мои кости. Оба комплекта. Проверяйте. Мне скрывать нечего.";
				link.l1 = "Так... Пара... Ничего... Пара... Фул... Ничего... Ничего... Две пары... Сет... Ничего... Пара...";
				link.l1.go = "NightAdventure_CardProf_Ok";
			}
			if(pchar.GenQuest.NightAdventureVar == "NightAdventure_SailorShulerFight") // драка
			{
				dialog.text = "Да пожалуйста, вот мои кости. Оба комплекта. Проверяйте. Мне скрывать нечего.";
				link.l1 = "Так... Каре... Каре... Фул... Фул... Каре... Покер... Фул... Покер... Каре... Фул... Хм, и как же ты мне это объяснишь?";
				link.l1.go = "NightAdventure_CardProf_Fight";
			}
		break;
		// разошлись миром
		case "NightAdventure_CardProf_GiveMoney":
			dialog.text = "Всё я понимаю, здесь вся сумма. Удачи вам и вашему товарищу.";
			link.l1 = "И тебе не хворать.";
		...
		case "NightAdventure_CardProf_Ok":
			dialog.text = "Ну что? Убедились?";
			link.l1 = "Хм, и вправду вроде бы обычные кости...";
			link.l1.go = "NightAdventure_CardProf_Ok_1";
		break;
		
		case "NightAdventure_CardProf_Ok_1":
			dialog.text = "Я вам сразу об этом говорил. А товарищ ваш пьяный просто. Ещё и поражение своё признать не сумел, вас вот привёл.";
			link.l1 = "Ладно, прошу прощения за инцидент. Удачи.";
		...
		case "NightAdventure_CardProf_Ok_3":
			dialog.text = "Это... ик!.. что же п-получается?.. Он... ик!.. он не ш-шулер?..";
			link.l1 = "Да, судя по всему, он играл честно.";
			link.l1.go = "NightAdventure_CardProf_Ok_4";
		break;
		
		case "NightAdventure_CardProf_Ok_4":
			dialog.text = "И что... теперь д-делать? Ик!..";
			link.l1 = "Ну, теперь ничего уже не сделаешь. Он честно выиграл у тебя. Не пей так больше. Или хотя бы не играй в азартные игры в нетрезвом состоянии.";
		...
		case "NightAdventure_CardProf_Fight":
			dialog.text = "Везение. Просто вам повезло - вот и всё. Удача - дама капризная.";
			link.l1 = "Хм, и вправду. Ну, давай тогда сыграем с тобой, только чур я играю этими костями!";
			link.l1.go = "NightAdventure_CardProf_Fight_1";
		break;
		
		case "NightAdventure_CardProf_Fight_1":
			dialog.text = "Этими?.. То есть... Нет, я не хочу с вами играть! Вы меня оскорбили. Да и вообще, я на мели...";
			link.l1 = "Может быть ты хочешь, чтобы я всем в этой таверне рассказал"+GetSexPhrase("","а")+" про твои кости? Или может быть - показал"+GetSexPhrase("","а")+"? А ты им расскажешь потом про везение. Что думаешь?";
			link.l1.go = "NightAdventure_CardProf_Fight_2";
		break;
		
		case "NightAdventure_CardProf_Fight_2":
			dialog.text = "Что?! Да ты кем себя возомнил"+GetSexPhrase("","а")+"?! Шантажировать меня вздумал"+GetSexPhrase("","а")+"?! Я в тебе сейчас пару дырок проделаю, если не уберёшься!";
			link.l1 = "Пожалуй, предпочту остаться здесь и показать всем твои кости.";
			link.l1.go = "NightAdventure_CardProf_Fight_3";
		break;
		
		case "NightAdventure_CardProf_Fight_3":
			dialog.text = "А ну иди сюда, падаль!..";
			link.l1 = "Штаны не потеряй.";
		...
		case "NightAdventure_CardProf_Fight_5":
			dialog.text = "К-капитан?.. Вы под... ик!.. подрались?.. А этот... ик!.. ш-шулер?..";
			link.l1 = "Он сам нарвался. Больше он ни с кем не поиграет.";
			link.l1.go = "NightAdventure_CardProf_Fight_6";
		break;
		
		case "NightAdventure_CardProf_Fight_6":
			dialog.text = "А ведь... н-не хотел признаваться! Ик!..";
			link.l1 = "Да, пытался увиливать. Только денег у него твоих не оказалось. Наверное, успел потратить.";
			link.l1.go = "NightAdventure_CardProf_Fight_7NM";
			link.l2 = "Да, пытался увиливать. Вот, держи свои деньги.";
		...
		case "NightAdventure_CardProf_Fight_7NM": // скажем, что нет денег
			...
			dialog.text = "И что... теперь д-делать? Ик!..";
			link.l1 = "Ну, теперь ничего уже не сделаешь. Не пей так больше. Или хотя бы не играй в азартные игры в нетрезвом состоянии.";
		...
		case "NightAdventure_Sailor_3_1":
			dialog.text = "Капитан?.. ик!.. К-капитан... проводите до п-п-пирса... ик!.. а? Пожалуйста. А то мне... ик!.. н-на кор... на корабль н-надо... Боц... ман... р-ругаться будет... ик!.. если оп-поздаю...";
			link.l1 = "Отстань, не до тебя сейчас.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Ладно, пошли. Сам ты, похоже, не дойдёшь...";
		...
		// просится на корабль
		case "NA_SailorShip":
			dialog.text = "К-капитан!.. Корабль!";
			link.l1 = "Что 'корабль'?";
			link.l1.go = "NA_SailorShip_1";
		break;
		
		case "NA_SailorShip_1":
			dialog.text = "К-корабль!.. Где он?.. ик!..";
			link.l1 = "Очевидно, ушёл. Нужно меньше пить в тавернах. Всё судно одного пьяного матроса ждать не будет.";
			link.l1.go = "NA_SailorShip_2";
		break;
		
		case "NA_SailorShip_2":
			dialog.text = "Так я же... ик!.. не пьян... почти...";
			link.l1 = "А, ну да, действительно. Ладно, удачи тебе.";
			link.l1.go = "NA_SailorShip_3";
		break;
		
		case "NA_SailorShip_3":
			dialog.text = "Постойте, к... капитан! В-возьмите меня... ик!.. к себе на к-корабль! Я б-больше... ик!.. пить н-не буду...";
			if(GetFreeCrewQuantity(pchar) > 0)
			{
				link.l1 = "Ладно, что уж с тобой поделаешь. Но чтобы сейчас же отправился на судно. И не приведи Господь я тебя увижу где-нибудь пьяным - будешь палубу драить до следующего захода в порт.";
				link.l1.go = "NA_SailorShip_4";
			}
			link.l2 = "Чтобы ты и на моё жалованье напился, а я потом искал тебя по всем тавернам и подворотням колонии? Нет уж, мне такое счастье не нужно.";
		...
		case "NightAdventure_Bull":
			...
			{
				case 0:
				dialog.text = "Капитан?.. ик!.. Да ты... к-крыса ты с-сухопутная... ик!.. а не капитан!..";
				link.l1 = "Я таких, как ты, не одну сотню в абордажных схватках перерезал, так что пошёл вон, пьянь.";
				link.l1.go = "NightAdventure_End";
				break;
				
				case 1:
				dialog.text = "Капитан... ик!.. К-капитан того к-корыта на рейде?.. ик!..";
				link.l1 = "Если увидишь это корыто на горизонте, когда будешь в море - лучше сразу ныряй в воду: с акулами у тебя будет больше шансов выжить. А теперь пошёл прочь!";
				link.l1.go = "NightAdventure_End";
				break;
				
				case 2:
				dialog.text = "Капитан?.. ик!.. Да т-ты молокосос, а не к-капитан!.. М-мать твоя ещё в девках ходила... ик!.. к-когда я море б-бороздил...";
				link.l1 = "Зато твоя мать, наверное, рада была бы узнать, что такой молокосос, как я, стал капитаном, а ты так и остался пьяным матросом, пригодным только для того, чтобы палубы драить.";
				link.l1.go = "NightAdventure_End";
				break;
				
				case 3:
				dialog.text = "Капитан?.. Смотри м-мне... ик!.. капитан... Зубы в-выбью - моргнуть не... ик!.. не успеешь!..";
				link.l1 = "Отдачей убьёт. Пошёл вон! Зубы он мне выбивать собрался...";
				link.l1.go = "NightAdventure_End";
				break;
				
				case 4:
				dialog.text = "Капитан?.. А с виду... ик!.. оборванец к-какой-то...";
				link.l1 = "Это ты, судя по всему, ещё на себя в зеркало не смотрел...";
				link.l1.go = "NightAdventure_End";
				break;
				
				case 5:
				dialog.text = "К-капитан?.. Н-ну, я бы к тебе... ик!.. н-не согласился пойти служить... на к-корабль... ик!..";
				link.l1 = "А тебе никто и не предлагал идти ко мне на корабль, чтобы ты соглашался. Уйди с дороги.";
				link.l1.go = "NightAdventure_End";
				break;
			}
		break;
		...
		case "NightAdventure_SailorAgain":
			dialog.text = "Ик!.. Да... далеко ещё?..";
			link.l1 = "Сейчас дойдём, терпи.";
			link.l1.go = "exit";
		...
		case "NightAdventure_Citizen_1_1":
			dialog.text = "Капитан?.. ик!.. К-капитан... помогите мне! М-мне... то есть... ик!.. моему другу... очень нужна в-ваша п... помощь! Пожалуйста.";
			link.l1 = "Я не собираюсь среди ночи решать проблемы каждого встречного пьяницы.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Ладно, рассказывай, что там у твоего друга случилось.";
			link.l2.go = "NightAdventure_Citizen_1_2";
		break;
		
		case "NightAdventure_Citizen_1_2":
			dialog.text = "Мой д-друг... он, в общем... напился он. Прям в-вообще... ик!.. напился...";
			link.l1 = "Я, конечно, отрезвлял"+GetSexPhrase("","а")+" матросов, которые на вахте выпивали, но достаточно радикальными методами. Не думаю, что это нужно твоему другу.";
			link.l1.go = "NightAdventure_Citizen_1_3";
		break;
		
		case "NightAdventure_Citizen_1_3":
			dialog.text = "Да нет! Он... он со ст... ст... стражником, во! Поспорил он с ним. И... п-подрался. И его... ик!.. в тюрьму отвели... А он п-пьяный просто!";
			link.l1 = "И что ты от меня теперь хочешь? Он сам виноват: надо меньше пить. Или хотя бы не искать приключений потом на свою...";
			link.l1.go = "NightAdventure_Citizen_1_4";
		break;
		
		case "NightAdventure_Citizen_1_4":
			dialog.text = "Поговорите с нач... начальником т-тюрьмы, кап... ик!.. капитан! Он опустит... отпустит его! Хм... тут уж... ик!.. как п-повезёт... Мой друг ведь п-просто... поспорил... ну... и в морду д-дать хотел... но о-оружие... ик!.. н-не доставал...";
			link.l1 = "Вот мне больше делать нечего, кроме как по ночам вытаскивать из тюрьмы пьянчуг, подравшихся со стражниками. Нет уж, пусть посидит несколько дней. Может быть, в следующий раз головой думать будет.";
			link.l1.go = "NightAdventure_End";
		...
		case "NightAdventure_CitizenHomiePresent":
			dialog.text = "Бла... благодарю в-вас... капитан. И м-мой д... друг тоже. В-вот... это вам.";
			link.l1 = "В следующий раз следи за своим другом, когда он напьётся и решит пойти подраться со стражей. Не каждую ночь по улицам ходят капитаны, готовые договариваться с начальником тюрьмы за разных пьяниц. Удачи вам.";
		...
		case "NightAdventure_CitizenHomiePresent_1":
			...
			if(GetDataDay() < 7) 						{GiveItem2Character(pchar, "indian_2");}
			if(GetDataDay() >= 7  && GetDataDay() < 13) {GiveItem2Character(pchar, "obereg_5");}
			if(GetDataDay() >= 13 && GetDataDay() < 19) {GiveItem2Character(pchar, "obereg_6");}
			if(GetDataDay() >= 19 && GetDataDay() < 25) {GiveItem2Character(pchar, "obereg_8");}
			if(GetDataDay() == 25) 		    			{GiveItem2Character(pchar, "indian_9");}
			if(GetDataDay() == 26) 	    				{GiveItem2Character(pchar, "obereg_11");}
			if(GetDataDay() >= 27 && GetDataDay() < 31) {GiveItem2Character(pchar, "obereg_4");}
			if(GetDataDay() == 31) 	    				{GiveItem2Character(pchar, "amulet_11");}
			SetFunctionExitFromLocationCondition("NightAdventure_KillPhant", pchar.location, false);
		break;
		...
		case "NightAdventure_Citizen_RE":
			...
			{
				dialog.text = "О!.. Так я з-знаю, как... ик!.. освободить м-моего... друга... ик!.. Стража! Лазутчик в г-городе!.. ик!..";
				link.l1 = "Ты выбрал очень плохой вариант освобождения твоего друга.";
			...
			else
			{
				...
				dialog.text = "А-а, и... иностранец... ик!.. Ты это... будут п-проблемы... ик!.. Обращайся! У меня тут... ик!.. всё схвачено.";
				link.l1 = "Непременно. Если что случится - я сразу к тебе.";
				link.l1.go = "NightAdventure_End";
			}
		break;
		...
		case "NightAdventure_Citizen_NR":
			if (GetSummonSkillFromName(pchar, SKILL_SNEAK) + GetSummonSkillFromName(pchar, SKILL_FORTUNE) < (rand(100)+rand(100)))
			{
				dialog.text = "А... так вы... ик!.. Вы преступник! Стража!";
				link.l1 = "Ах вот, значит, как ты теперь заговорил!";
				link.l1.go = "NightAdventure_Fight";
			}
			else
			{
				AddCharacterExpToSkill(pchar, SKILL_SNEAK, 100);
				AddCharacterExpToSkill(pchar, SKILL_FORTUNE, 100);
				dialog.text = "Ох, как... ик!.. жаль. А д-давайте я... договорюсь за вас с... ик!.. начальником т-тюрьмы, а вы... ик!.. за м-моего друга?..";
				link.l1 = "Нет, за меня договариваться не нужно, спасибо. Удачи.";
				link.l1.go = "NightAdventure_End";
			}
		break;
		...
		case "NightAdventure_PirateCave":
			dialog.text = "Капитан?.. ик!.. К-капитан... слушайте, проводите до пещеры рядом с городом... ик!.. а? П-пожалуйста.";
			link.l1 = "До пещеры? Не понял. Зачем тебе к пещере? И зачем тебе я?";
			link.l1.go = "NightAdventure_PirateCave_1";
		break;
		
		case "NightAdventure_PirateCave_1":
			dialog.text = "Видите ли... к-капитан... ик!.. У меня деньги н-на выпивку з-закончились... А в... ик!.. в п-пещере у м... у меня там... заначка, в общем... Ик!.. Т-только я... это... б-боюсь один п-по дж... дж... джунглям... ик!.. ночью идти... Вот...";
			link.l1 = "Отстань, не до тебя сейчас.";
			link.l1.go = "NightAdventure_End";
			link.l2 = "Ладно, пошли. Сам ты, похоже, не дойдёшь...";
		...
		case "NightAdventure_PirateCave_3":
			...
			string NACave = "отсутсвует. Сообщите belamour, что вы находитесь";
			...
			log_testinfo("Пещера "+NACave+" на острове " +NAIsland);
		...
		case "NightAdventure_PirateCave_4": // диалог в пищере
			if(pchar.GenQuest.NightAdventureVar == "NightAdventure_PiratePeace")
			{
				...
				PlaySound("interface\important_item.wav");
				dialog.text = "Во, к-капитан... ик!.. спасибо б-большое... выручили! Я тут... ик!.. это самое... В общем, это в-вам.";
				link.l1 = "Да ладно, что уж... Но лучше больше так не напивайся.";
				link.l1.go = "NightAdventure_PirateCave_4_1";
				break;
			} 
			if(pchar.GenQuest.NightAdventureVar == "NightAdventure_PirateBad")
			{
				dialog.text = "Отлично, капитан! Рад, что вы согласились.";
				link.l1 = "Ну, и где твоя заначка? Погоди-ка, что-то по тебе не скажешь, что ты пьян.";
		...
		case "NightAdventure_PirateCave_5": // драка
			dialog.text = "Да я много не пил, не переживайте за меня. А по поводу заначки... Ребята!";
			link.l1 = "...";
		...
		case "NightAdventure_PirateCave_7": 
			dialog.text = "Ребята, я тут заначку привёл. Нужно её открыть. Если, конечно, эта заначка сама не откроется, по-хорошему. Капитан, что думаете?";
			link.l1 = LinkRandPhrase("Я думаю, что лучше бы ты напился до потери памяти. Хотя бы живой остался. А так теперь одна дорога...", "Я думаю, ты совсем с головой не дружишь, раз решился ограбить капитана боевого судна...", "Я думаю, что тебе, наверное, жить надоело от слова 'совсем', раз ты решился такое провернуть...");
			link.l1.go = "NightAdventure_PirateCave_8F";
			link.l2 = "Ладно, полагаю, что по-хорошему. И сколько вам нужно?";
			link.l2.go = "NightAdventure_PirateCave_8A";
		break;
		
		case "NightAdventure_PirateCave_8F": 
			dialog.text = "Значит, по-хорошему мы заначку не откроем?";
		...
		case "NightAdventure_PirateCave_8A": // испугался
			...
			link.l1 = "Боюсь, это была последняя заначка в твоей жизни.";
			link.l1.go = "NightAdventure_PirateCave_9F";
			link.l2 = "Хорошо, будь по-вашему. Чтоб я ещё когда-нибудь помог пьяному человеку...";
		...
		case "NightAdventure_PirateCaveAgain": 
			dialog.text = "Капитан, вы меня удивляете своей щедростью. Вы хотите ещё с нами поделиться?";
			link.l1 = "Я уже вам всё отдал. Ухожу...";
			link.l1.go = "exit";
		...
		case "NightAdventure_NobleGame": 
			dialog.text = ""+GetSexPhrase("Капитан","Девушка")+"?.. ик!.. ох, "+GetAddress_Form(NPChar)+" ... п-прошу прощения... ик!.. за м-моё... со... со... стояние. Вы не... не против сос... тавить мне к-компанию? Ик!.. П-пожалуйста."; 
			link.l1 = "Прошу прощения, сударь, но я тороплюсь."; 
			link.l1.go = "NightAdventure_NobleEnd";
			link.l2 = "Компанию? Прошу простить, но я сегодня не пью."; 
			link.l2.go = "NightAdventure_NobleGame_1";
		break;
		
		case "NightAdventure_NobleGame_1":
			dialog.text = "А пить и... ик!.. не н-надо! Тут т-такое дело... В общем... я... с другом... ну, это... ик!.. в-выпил... Вот и навеселе уже... по мне в-видно... ик!.. да?";
			link.l1 = "Честно сказать, да. Вам нужно помочь протрезветь?";
			link.l1.go = "NightAdventure_NobleGame_2";
		break;
		
		case "NightAdventure_NobleGame_2": 
			dialog.text = "Да нет! Мне и... и так... х-хорошо. Я теперь х-хочу... в карты, в общем с-сыграть хочу я... Ик!.. Т-только не на ж-жалкие... п-песо... с этими оборванцами из... ик!.. т-таверны... А с "+GetSexPhrase("н-настоящим дворянином","к-красивой девушкой")+"! Ик!.. На наст... настоящие ставки!.."; 
			link.l1 = "Прошу простить, сударь, но как бы вам сказать... Я не любитель"+GetSexPhrase("","ница")+" играть в азартные игры. Нужно матросам жалование платить, судно содержать - сами понимаете..."; 
			link.l1.go = "NightAdventure_NobleEnd";
			link.l2 = "Хм, ну ладно, давайте сыграем. Пройдёмте в таверну?"; 
			link.l2.go = "NightAdventure_NobleGame_3";
		break;
		
		case "NightAdventure_NobleGame_3": 
			dialog.text = "Ага! Только... ик!.. ме-е-едленно, по... пожалуйста. А то я... ик!.. в состоянии не... нестояния..."; 
			link.l1 = "Ладно, пойдём медленно."; 
		...
		case "NightAdventure_NobleGame_5": 
			...
			dialog.text = "В-вот за тот... ик!.. столик п-пошли!"; 
			link.l1 = "Хорошо, пошли."; 
		...
		case "NightAdventure_NobleGame_7": 
			dialog.text = "В-вы не п... не представляете, "+GetSexPhrase("к-капитан","дорогая")+", как... ик!.. как я рад встретить "+GetSexPhrase("н-настоящего дво... дворянина","к-красивую дев... девушку")+"! Ик!.. В этой г-глуши... обычно одно... ик!.. отребье..."; 
			link.l1 = "Да уж, его в местных колониях пруд пруди. Ну что, сыграем?"; 
		...
		case "NightAdventure_NobleGame_8": 
			...
			link.l3 = "Хм, прошу простить, кажется у меня на текущий момент карман пустоват..."; 
			link.l3.go = "NightAdventure_NobleGame_Голодранец";
		break;
		
		case "NightAdventure_NobleGame_Голодранец": 
			dialog.text = "Ч-что?.. У вас... ик!.. денег н-нет на игру со... со мной?.."; 
			link.l1 = "Ну, бывает и такое. Сыграем в следующий раз, не расстраивайтесь."; 
		...
		case "NightAdventure_NobleGame_Win": 
			...
			dialog.text = "Ик!.. Отлично провели в-время, к... "+GetSexPhrase("капитан","красавица")+"! Ик!.."; 
			link.l1 = "Соглашусь с вами. Хотя бы отдохнул"+GetSexPhrase("","а")+" немного от дел."; 
		...
		case "NightAdventure_NobleGame_Lose_1": 
			dialog.text = "И-иногда нужно в-вот так... ик!.. пров-водить время с п-приятн"+GetSexPhrase("ым","ой")+"... ик!.. "+GetSexPhrase("человеком","девушкой")+". Большое с-спасибо вам... ик!.. за компанию!"; 
			if(pchar.GenQuest.NightAdventureVar == "NightAdventure_NobleGameDuel")
			{		
				link.l1 = "Да уж, только проиграл"+GetSexPhrase("ся","ась")+" я в пух и прах. А как так вышло, что вы побеждали даже в тех случаях, когда у меня были все шансы выиграть?"; 
				link.l1.go = "NightAdventure_NobleGame_Duel";
				break;
			}	
			link.l1 = "Взаимно, благодарю вас за игру и за беседу."; 
		...
		case "NightAdventure_NobleGame_Duel": 
			dialog.text = "П-просто везение, к-"+GetSexPhrase("капитан","красавица")+"... ик!.."; 
			link.l1 = "Непохоже это на обычное везение..."; 
			link.l1.go = "NightAdventure_NobleGame_Duel_1";
		break;
		
		case "NightAdventure_NobleGame_Duel_1": 
			dialog.text = "На что вы намекаете? Уж не хотите ли вы сказать, что я играл нечестно? Вы сами всё видели, мы брали карты из одной колоды!"; 
			link.l1 = "Да, видел"+GetSexPhrase("","а")+". А ещё я видел"+GetSexPhrase("","а")+", что минуту назад у вас язык еле ворочался, а сейчас говорите так, будто у вас ни в одном глазу."; 
			link.l1.go = "NightAdventure_NobleGame_Duel_2";
		break;
		
		case "NightAdventure_NobleGame_Duel_2": 
			dialog.text = "Ах, чёрт! А ловко вы меня поймали! Ладно, признаю, скрывать не буду. Я не был пьян. Но знаете, как хорошо люди клюют на таких якобы богатеньких и пьяных, которые хотят сыграть в карты? Каждый, у кого есть что поставить, едва ли не первым бежит в таверну, чтобы поскорее получить несомненно крупный выигрыш у несомненно богатого, но пьяного дворянина. Позволю себе заметить, что вы перв"+GetSexPhrase("ый","ая")+", кто меня раскусил."; 
			link.l1 = "Так, и что теперь? Вернёте мне мои деньги по-хорошему?"; 
			link.l1.go = "NightAdventure_NobleGame_Duel_3";
		break;
		
		case "NightAdventure_NobleGame_Duel_3": 
			dialog.text = "Ха-ха-ха, не смешите меня, "+GetSexPhrase("капитан","дорогая")+". Конечно же, я вам ничего не верну! И не думайте, что вы сможете меня заставить. Я запросто могу изобразить уважаемого дворянина, на которого огрызается "+GetSexPhrase("какой-то залётный капитанишка","какая-то проститутка из борделя")+". Так что мой вам совет: смиритесь со своим поражением и просто уходите."; 
			link.l1 = "Что ж, искренне желаю вам не попасться мне при менее удачных обстоятельствах, потому что тогда вы уже не будете так смеяться. Прощайте."; 
			link.l1.go = "NightAdventure_NobleGame_HappyEnd_1";
			link.l2 = "А мой вам совет - постарайтесь успеть достать шпагу."; 
			link.l2.go = "NightAdventure_NobleGame_Duel_4";
		break;
		
		case "NightAdventure_NobleGame_Duel_4": 
			dialog.text = "Вы не посмеете... Это таверна, здесь полно народу..."; 
			link.l1 = "Так я же ведь просто "+GetSexPhrase("залётный капитанишка","проститутка из борделя")+", куда мне до правил приличия. А в портовой таверне ночью чего только может не случиться... Шпагой-то я владею всяко лучше вас."; 
			link.l1.go = "NightAdventure_NobleGame_Duel_5";
		break;
		
		case "NightAdventure_NobleGame_Duel_5": 
			dialog.text = "Ах так?! Ну, сейчас мы проверим, кто ею владеет лучше!"; 
		...
		case "NightAdventure_NobleWhore": 
			dialog.text = "Капитан?.. ик!.. ох, "+GetAddress_Form(NPChar)+"... п-прошу прощения... ик!.. за м-моё... со... со... стояние. Вы не... не могли бы п-помочь... мне? П-пожалуйста."; 
			link.l1 = "Прошу прощения, "+GetAddress_Form(NPChar)+", но я тороплюсь."; 
			link.l1.go = "NightAdventure_NobleEnd";
			link.l2 = "Ох... слушаю вас."; 
			link.l2.go = "NightAdventure_NobleWhore_1";
		break;
		
		case "NightAdventure_NobleWhore_1": 
			dialog.text = "Тут т-такое дело... В общем... я... навеселе уже... по мне в-видно... ик!.. да?"; 
			link.l1 = "Честно сказать, да. Вам нужно помочь протрезветь?"; 
			link.l1.go = "NightAdventure_NobleWhore_2";
		break;
		
		case "NightAdventure_NobleWhore_2": 
			dialog.text = "Да нет! Мне и... и так... х-хорошо. Я теперь х-хочу... даму, в общем х-хочу я... Т-только меня такого... п-пьяного... ик!.. х-хозяйка бор... деля... в-выгонит, в общем. В-вы не могли бы... ик!.. д-договориться за... за меня? Д-денег я... дам."; 
			link.l1 = "Прошу простить, "+GetAddress_Form(NPChar)+", но как бы вам сказать... Я по борделям не хожу. Честь, репутация - сами понимаете..."; 
			link.l1.go = "NightAdventure_NobleEnd";
			link.l2 = "Ох, ну ладно, я помогу вам."; 
		...
		case "NightAdventure_NobleWhore_5": 
			...
			dialog.text = "В-вот деньги... ик!.. Я в-вас тут... ждать б-буду."; 
			link.l1 = "Хорошо, скоро вернусь."; 
		...
		case "NightAdventure_NobleWhore_6": 
			...
			dialog.text = "Т-только... мне это... ик!.. с-самую... лучшую!.. Да..."; 
			link.l1 = "Ладно-ладно, я вас понял."; 
			link.l1.go = "exit";
		break;
		
		case "NightAdventure_NobleWhore_7": // заказ исполнен
			dialog.text = "Ик!.. Д-долго ещё?.."; 
			link.l1 = "Лучшая девушка этой колонии ждёт вас наверху."; 
			link.l1.go = "NightAdventure_NobleWhore_8";
		break;
		
		case "NightAdventure_NobleWhore_8": 
			dialog.text = "Да!.. Этот в-вечер... удался... Бла... благодарю в-вас, "+GetAddress_Form(NPChar)+". О-оставшиеся деньги... ик!... возьмите... себе. В-вот. А я п-пошёл... Меня там... ждёт... ПРРРЕКРАСНАЯ да... да... дама...";
			link.l1 = "Желаю вам приятно провести время."; 
		...
		case "NightAdventure_NobleBrothelAgain":
			...
			dialog.text = "Ик!.. Д-долго ещё?";
			link.l1 = "Сейчас договорюсь, потерпите.";
			link.l1.go = "exit";
		break;
		...
		case "NightAdventure_NobleEndAgain":
			...
			dialog.text = "Ик!..";
			link.l1 = "Вам нужно поспать.";
			link.l1.go = "exit";
		break;
		
		case "NightAdventure_NobleTownAgain":
			dialog.text = "Ик!.. Да... далеко ещё?..";
			link.l1 = "Сейчас дойдём, потерпите.";
			link.l1.go = "exit";
			NextDiag.TempNode = "NightAdventure_SailorAgain";
			LAi_SetActorType(npchar);
			LAi_ActorFollowEverywhere(npchar, "", -1);
		...
		case "CemeteryMan":
			dialog.text = "А? Кто вы? Что вы здесь делаете?";
			link.l1 = "Могу то же самое спросить у вас.";
			link.l1.go = "CemeteryMan_1";
		break;
		
		case "CemeteryMan_1":
			dialog.text = "Мы? А мы... это... Мы тут цветы... собираем...";
			link.l1 = "Могильные? Ну и как гербарий?";
			link.l1.go = "CemeteryMan_2";
		break;
		
		case "CemeteryMan_2":
			dialog.text = "Гер... Это с гербами связано?";
			link.l1 = "Так... Кажется, я понял"+GetSexPhrase("","а")+", в чём тут дело.";
			link.l1.go = "CemeteryMan_3";
		break;
		
		case "CemeteryMan_3":
			dialog.text = "Мы не мародёры! Не говорите про нас! Мы просто...";
			link.l1 = "Что 'просто'?";
			link.l1.go = "CemeteryMan_4";
		break;
		
		case "CemeteryMan_4":
			dialog.text = "Ну мы... Мой отец запрещает нам видеться! И её отец тоже! Вот мы и... искали место без свидетелей. Чтобы уединиться. Замок на двери всё равно сломан, так что попасть сюда несложно...";
			link.l1 = "Теперь понятно, что это за страшные стоны неупокоенной души!";
		...
		case "CemeteryMan_6":
			dialog.text = "Ну так... Если вы всё выяснили, может быть вы оставите нас?";
			link.l1 = "А вам не боязно устраивать ваши свидания в склепе? Не самое лучшее место.";
			link.l1.go = "CemeteryMan_7";
		break;
		
		case "CemeteryMan_7":
			dialog.text = "А мы не суеверные! А даже если бы и страшно было - какой у нас выбор? В городе об этом сразу прознали бы родители. А тут никто ничего не расскажет. Так что пока это лучший вариант.";
			link.l1 = "Я очень сожалею, но вы не могли бы поискать другое место? Не поймите неправильно: я-то всё понимаю - молодость и всё такое - но ваши свидания до смерти перепугали кладбищенского сторожа, который каждую ночь слышит из склепа шумы и стоны.";
			link.l1.go = "CemeteryMan_8";
		break;
		
		case "CemeteryMan_8":
			dialog.text = "Ну... Пугать смотрителя не входило в план. Но где нам тогда видеться? Или вы под открытым небом предлагаете нам свидания устраивать?";
			link.l1 = "Любовь не знает границ. Найдите другое место, где вы уж точно никому не будете мешать. Тогда и вам никто не помешает. Наверное. А лучше решить всё с вашими родителями - вы же не собираетесь до старости прятаться?";
			link.l1.go = "CemeteryMan_9";
		break;
		
		case "CemeteryMan_9":
			dialog.text = "Ладно, что-нибудь придумаем... Сторожа больше пугать не будем - вот вам моё слово.";
			link.l1 = "Вот и отлично. Удачи вам!";
