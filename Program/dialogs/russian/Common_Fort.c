			...
			else
			{
				...
				else
				{
					if (sti(NPChar.nation) == PIRATE)
					{
						if (sti(rColony.HeroOwn)) // наш горожанин
						{
			         		switch (rand(10))
							{
								...
								case 3:
									dialog.text = "Спешу пожаловаться, капитан: мы все стремимся снова в море. Эта служба на суше просто попёрек горла стоит!";
					...
					{// Addon 2016-1 Jason пиратская линейка
						...
						{
							dialog.text = "Эй, "+GetSexPhrase("дружище","красавица")+"! Не окажешь мне маленькую услугу?";
