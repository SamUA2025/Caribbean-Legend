...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("Preguntas, "+GetAddress_Form(NPChar)+"¿?","¿Cómo puedo ayudarte, "+GetAddress_Form(NPChar)+"¿?"),"Entendido. Proporciona текст, который нужно перевести."+GetSexPhrase("Mm, ¿cuál es la gran idea, "+GetAddress_Form(NPChar)+"¿ ","¿Otra vez con las preguntas extrañas? Cariño, ve a tomar un poco de ron o algo...")+"Entendido. Пожалуйста, предоставьте текст для перевода.","En todo este día, esta es la tercera vez que hablas de alguna pregunta..."+GetSexPhrase(" ¿Son estos algunos signos de atención?","")+"Hola, amigo.","Más preguntas, supongo, "+GetAddress_Form(NPChar)+"?","bloqueo",1,npchar,Dialog.CurrentNode);
