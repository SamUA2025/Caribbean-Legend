...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
		case "quests":
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("¿Qué preguntas tienes?","¿En qué puedo ayudarle, "+GetAddress_Form(NPChar)+"¿?"),"Intentaste hacerme alguna pregunta no hace mucho, "+GetAddress_Form(NPChar)+"... ","Esta es la tercera vez que me molestas ahora...","¿Más preguntas, supongo?","bloqueo",1,npchar,Dialog.CurrentNode);
		...
		case "Tonzag_check"://начинаем проверять нашего ГГ по всем статьям
			if(sti(pchar.reputation.nobility) > 48)//высокая репа
			{
				...
				dialog.text = "No necesito los servicios de un sinvergüenza. No tengo ninguna tarea que pueda ofrecerte.";
				link.l1 = "¡Eh, eh! ¡Cuidado con el lenguaje, compañero!";
				link.l1.go = "exit";
				break;
			}
			...
			dialog.text = "Hm... En realidad llegas a tiempo. Tengo un encargo que debe cumplirse de inmediato. Si te muestras bajo una buena luz, te presentaré a algunas personas influyentes. Ahora escucha, esto es lo que tienes que hacer.";
