	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First Time":
			...
			if (GetNationRelation2MainCharacter(sti(NPChar.nation)) == RELATION_ENEMY && sti(NPChar.nation) != PIRATE)
			{
				...
				{
					dialog.text = RandPhraseSimple("Vaya, parece que este granuja ha levantado la alarma en "+XI_ConvertString("Colonia"+npchar.city+"Eso")+"¡Capturad h"+GetSexPhrase("soy","eh")+"¡¡","¡Eh, mira esto! Varios "+NationNamePeople(sti(pchar.nation))+" lograr ser enemigos con "+NationNameAblative(sti(npchar.nation))+"¡Atrapad al malandrín!!!");
				}
				else
				{
					dialog.text = RandPhraseSimple("Agente enemigo cerca "+XI_ConvertString("Colonia"+npchar.city+"Gen")+"¡Captúralo!"+GetSexPhrase("soy","er")+"¡¡","Oye, mira, "+NationNamePeople(sti(pchar.nation))+" están caminando casi en "+XI_ConvertString("Colonia"+npchar.city+"Eso")+"¡Agárralo inmediatamente!!!");
				}
			...
			{				
				...
				dialog.text = LinkRandPhrase("¡Permíteme presentarme! Soy un oficial de patrulla de "+XI_ConvertString("Colonia"+npchar.city+"Gen")+", estamos buscando a un convicto fugado.","Hola, soy el jefe de esta patrulla. Estamos buscando a un esclavo fugado de "+XI_ConvertString("Colonia"+npchar.city+"Gen")+".","Saludos a ti, "+GetAddress_Form(NPChar)+". Mi destacamento está patrullando el territorio cerca de "+XI_ConvertString("Colonia"+npchar.city+"Gen")+".");
		...
		case "Reason_To_Fast_16":
			dialog.text = "Hemos saldado la suma con "+GetName(NAMETYPE_MAIN,pchar.questTemp.ReasonToFast.p3,NAME_ABL)+" por adelantado. No me gusta nada. Mata a h"+GetSexPhrase("soy","eh")+", ¡muchachos!";
		...
		case "Reason_To_Fast_AfterHunter3":
			dialog.text = "¡Maldito seas! ¡Casi creí en eso! ¡Guardias! Atrápenlo"+GetSexPhrase("yo","eh")+"...";
			link.l1 = "¡Inténtalo, bastardos!";
			link.l1.go = "Reason_To_Fast_ExitFight";
