...
void ProcessDialogEvent()
{
	...
	case "prepare_convoy_quest_3":
		...
		dialog.text = "Necesito ser escoltado a " + XI_ConvertString("Colony" + pchar.quest.destination + "Gen") + ", que está ubicado en " + XI_ConvertString(GetIslandByCityName(pchar.quest.destination) + "Dat") + ", en " + FindRussianDaysString(sti(pchar.ConvoyQuest.iDay)) + ", y por eso te pagaré " + FindRussianMoneyString(sti(pchar.ConvoyQuest.convoymoney)) + ". ¿Entonces, tu decisión?";
	...
	case "complete_convoy_quest":
		...
		AddSimpleRumour(LinkRandPhrase("El capitán de un barco mercante llamado " + GetFullName(npchar) + " dice que se puede confiar en el capitán " + GetMainCharacterNameDat() + " cuando se necesita escolta.",
									   "El comerciante " + GetFullName(npchar) + " dice que se puede confiar en el capitán " + GetMainCharacterNameDat() + ". " + GetSexPhrase("Él", "Ella") + " protego su barco de la mejor forma posible mientras lo escoltaban hasta " + XI_ConvertString("Colony" + pchar.quest.destination + "Gen") + ".",
									   "He oído que cumple la palabra dada a los capitanes de los mercantes que le piden escolta. Un comerciante llamado " + GetFullName(npchar) + " habla bien de ti."),
