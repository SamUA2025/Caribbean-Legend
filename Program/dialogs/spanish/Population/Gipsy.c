...
//-------------------------------------------------гадание-------------------------------------------------
		...
		case "guess_rate_2"://немного в везение или скрытность
			if (sti(pchar.money) >= 500)
			{
				...
				dialog.text = "¡Ah, gracias, mi apuesto joven halcón! Ahora escucha:"+sTemp+"Entendido, предоставьте текст для перевода.";
...
// <-- продажа мышьяка
	...
	// --> Мангароса
		...
		case "mangarosa_6":
			RemoveItems(pchar, "cannabis7", 1);
			PlaySound("interface\important_item.wav");
			dialog.text = "Su nombre es Amelia. Vive sola en una pequeña casa entre las dunas no lejos del mar, en algún lugar de la costa suroeste del Main español, como dicen los sabios.";
