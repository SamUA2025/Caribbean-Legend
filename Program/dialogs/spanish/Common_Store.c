	...
	switch(Dialog.CurrentNode)
	{
		...
		case "node_1":
			...
			if (CheckAttribute(PChar, "questTemp.WPU.Postcureer.LevelUp"))
			{
				...
				{
					link.l12 = "He traído una lista de precios de la ciudad de "+XI_ConvertString("Colonia"+pchar.questTemp.WPU.Postcureer.StartCity+"Gen")+".";
...
//navy <--
		...
		case "business":
			...
			if (CheckAttribute(pchar, "GenQuest.Sunplace.Trader") && pchar.GenQuest.Sunplace.Trader == "complete" && npchar.location == pchar.GenQuest.Sunplace.Trader.City + "_store") 
			{
				...
				link.l1 = "¡Je! Eso es seguro. "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.Sunplace.Trader.ShipType),"Nombre")))+" "+pchar.GenQuest.Sunplace.Trader.ShipName+" y la carga - "+GetGoodsNameAlt(sti(pchar.GenQuest.Sunplace.Trader.Goods))+", ya no pertenecen a tu rival."; 
...
//navy <--
            ...
            if (sti(Pchar.Ship.Type) != SHIP_NOTUSED && ok)
			{
				...
				// <-- homo
				link.l99 = "Pero no hay tiempo para charlas ahora.";
					...
					else
					{
                        ...
                        if (storeMan > 0)
                        {
    						...
    						else
    						{
                                ...
                                dialog.text = "¡Oh! Estaba a punto de pedirte un favor. Verás, necesito entregar la carga de "+GetGoodsNameAlt(iTradeGoods)+" en cantidad de "+FindRussianQtyString(iQuantityGoods)+"  a la ciudad de "+sNation+sTemp+", y cuanto antes mejor. Si logras hacerlo por "+FindRussianDaysString(makeint(pchar.CargoQuest.iDaysExpired))+", entonces al llegar recibirás "+FindRussianMoneyString(iMoney)+" como tu recompensa. ¿Qué dices?";
    							link.l1 = "Creo que estoy de acuerdo.";
    							link.l1.go = "exit_trade";
    							link.l2 = "Esto probablemente no es para mí.";
    							link.l2.go = "exit";
    						}
                        }
                        else
                        {
                            dialog.text = "Las realidades políticas en el archipiélago no me permiten comerciar. "+XI_ConvertString(NationShortName(sti(NPChar.nation))+"cazador")+" está en hostilidad con todos y me dejó solo en la tienda.";
...
//Jason --> мини-квест Дефицитный товар
		...
		case "Wine_Price":
			...
			link.l1 = "Entendido. Estoy listo para realizar la traducción. Por favor, proporciona текст, который necesita ser traducido.";
			Link.l1.edit = 4;
			link.l1.go = "Wine_Qty";
		...
		case "Wine_Qty":
			...
			dialog.text = "Hola, Amigo."+sti(iQty)+" ¿Botellas? Bien. Eso te costará "+FindRussianMoneyString(sti(pchar.questTemp.Wine.Summ))".";
...
// Jason --> -------------------------генератор Сомнительное предложение----------------------------------------
		...
		case "Contraoffer_pay":
			dialog.text = "Gracias por tu trabajo. Por favor, toma tu pago - "+FindRussianMoneyString(sti(pchar.GenQuest.Contraoffer.Trader.Summ))+".";
		link.l1 = "¡Gracias! Un placer hacer negocios contigo.";
		...
		case "Contraoffer_slaves"://единичный вариант за всю игру, ака 'пасхалка'
			...
			dialog.text = "Necesito urgentemente un lote de esclavos - "+sti(pchar.GenQuest.Contraoffer.Slaves.Qty)+" cabezas. Estoy listo para pagar "+sti(pchar.GenQuest.Contraoffer.Slaves.Price)+" monedas por alma, lo que ascenderá a "+sti(pchar.GenQuest.Contraoffer.Slaves.Money)+" pesos in total; term - "+FindRussianDaysString(sti(pchar.GenQuest.Contraoffer.Slaves.Days))+".";
		...
		case "Contraoffer_slaves_pay":
			dialog.text = "Gracias por tu trabajo. Por favor, toma tu pago - "+FindRussianMoneyString(sti(pchar.GenQuest.Contraoffer.Slaves.Money))+"Además, por favor, acepta esto. Estoy seguro de que lo encontrarás útil.";
			link.l1 = "¡Gracias! Un placer hacer negocios contigo.";
		...
		case "Sunplace_1":
			...
			dialog.text = "Muy bien. Entonces, el nombre de tu hombre es "+pchar.GenQuest.Sunplace.Trader.Enemyname+". Recientemente había enviado "+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.Sunplace.Trader.ShipType),"Nombre")+"Acc"))+" '"+pchar.GenQuest.Sunplace.Trader.ShipName+" con un cargamento de "+GetGoodsNameAlt(sti(pchar.GenQuest.Sunplace.Trader.Goods))+". Pagó una suma bastante decente por este cargamento, así que perderlo será un golpe bastante fuerte para su bolsa.";
		...
		case "FindFugitiveSt":
			...
			{
				dialog.text = NPCStringReactionRepeat("No sé qué decían en la taberna de Port Royal, pero nunca vi a ese hombre en mi tienda. De hecho, nunca lo he visto en ningún lado. Pregunta por el pueblo, quizás alguien te diga algo...","Ya me has preguntado por ese hombre, y te dije: ¡no lo sé!","¿Me estás tomando el pelo o eres un idiota?! ¡Ya estás haciendo las mismas preguntas por tercera vez!","Solo pensar, cómo semejante idiota se convirtió en capitán...","bloque",1,npchar,Dialog.CurrentNode);
