...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("¡Mi marido no me permite hablar con extraños!","No hay nada de que podríamos hablar.");
				Link.l1 = "No es gran cosa.";
				Link.l1.go = "exit";
                break;
			}
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Escucha, como ciudadano de esta ciudad, debo pedirte que te abstengas de caminar con una espada desenvainada.");
				link.l1 = LinkRandPhrase("Bien.","Bien.","Como usted diga...");
			}
			else
			{
				dialog.text = NPCharSexPhrase(NPChar,"Sostén tus caballos, "+GetSexPhrase("compañero","chica")+", cuando corretea con un arma en la mano. Eso puede ponerme nervioso...","No me gusta cuando "+GetSexPhrase("hombres","gente")+" camina con las armas preparadas. Me asusta...");
				link.l1 = RandPhraseSimple("Lo tengo.","Lo eliminaré.");
			}
			link.l1.go = "exit";
