void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
			...
			else
			{
				if(CheckAttribute(pchar,"GenQuest.CaptainComission.GetRumour") && GetQuestPastDayParam("GenQuest.CaptainComission.GetRumour") < 1)
				{
					if(!CheckAttribute(pchar,"GenQuest.CaptainComission.SpeakContra"))
					{
						...
						dialog.text = "No hay negocios hoy. Hay patrullas en cada bahía, nuestro gobernador está buscando una carga que fue confiscada por algún capitán de"+GetStrSmallRegister(XI_ConvertString(GetBaseShipParamFromType(sti(pchar.GenQuest.CaptainComission.ShipType),"Nombre")+"Acc"))+" "+pchar.GenQuest.CaptainComission.Name+" y fue escondido por él en algún lugar.";
				...
				{
					Link.l5 = "Un colega tuyo y mi buen amigo de  "+XI_ConvertString("Colonia"+PChar.GenQuest.ChurchQuest_2.QuestTown+"Gen")+" me contaron que estás al tanto de algunos negocios.";
		...
		case "Meeting":
			...
			if(CheckAttribute(PChar, "GenQuest.ChurchQuest_2.AskContra_2") && NPChar.location == PChar.GenQuest.ChurchQuest_2.Contra_Colony + "_tavern")
			{
				Link.l3 = "Un colega tuyo y mi buen amigo desde  "+XI_ConvertString("Colonia"+PChar.GenQuest.ChurchQuest_2.QuestTown+"Gen")+" dijo que estás al tanto de algunos negocios.";
