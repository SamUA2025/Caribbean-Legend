...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	...
	{
	case "quests":
		dialog.text = NPCStringReactionRepeat(RandPhraseSimple("¿Qué preguntas tienes para un soldado?", "El soldado tiene toda su vida planificada: vigilar, afanarse... ¿Cómo puedo ayudarle, " + GetAddress_Form(NPChar) + "?"), "No deberías molestar a un soldado con preguntas estúpidas, " + GetAddress_Form(NPChar) + "...", "Esta es la tercera vez hoy que intentas hacerme una pregunta...", "El destino de un soldado no es fácil, y ahora tú, escoria, me estás volviendo loco...", "block", 1, npchar, Dialog.CurrentNode);
