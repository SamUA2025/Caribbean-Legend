...
//			Dialog.Text = "Solo tenemos "+FindRussianDaysString(sti(PChar.GenQuest.PGG_Quest.Days))+" para interceptarlos.";
	...
	case "Quest_1_Ship_BadWay_1":
		Dialog.Text = RandSwear()+PCharRepPhrase(RandPhraseSimple("Captura "+GetSexPhrase("él","ella")+"! Amarra "+GetSexPhrase("él","ella")+"¡a la cañón! Veamos de qué color "+GetSexPhrase("su","ella")+"¡¡¡tripas son!!!","¡Eh! ¡Johnny! ¡Henderson! Atrapa eso "+GetSexPhrase("granuja","chica")+"¡No permitas que "+GetSexPhrase("él","ella")+"¡¡¡a llegar al bote salvavidas!!!"),RandPhraseSimple("¡Una lástima, capitán! Espero que te guste nuestra bodega, aunque. Y luego decidiremos qué hacer contigo.","Los esclavos blancos no suelen vivir mucho, capitán, así que elige: o Hends te dispara en la cabeza, o pasas el resto de tu vida en una cantera."));
