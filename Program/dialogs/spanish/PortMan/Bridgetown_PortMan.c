...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
    ...
    switch (Dialog.CurrentNode)
	{
		case "quests":
			...
			//регата
			dialog.text = NPCStringReactionRepeat(RandPhraseSimple("¿Qué tipo de preguntas?","¿Qué quieres, "+GetAddress_Form(NPChar)+"¿?"),"Ya has intentado hacerme una pregunta "+GetAddress_Form(NPChar)+"Entendido. Por favor, предоставьте текст для перевода, и я переведу его для вас.","Has estado hablando de alguna pregunta por tercera vez hoy...","Mira, si no tienes nada que decirme sobre los asuntos del puerto, entonces no me molestes con tus preguntas.","block",1,npchar,Dialog.CurrentNode);
		...
		case "caleuche":
			...
			link.l1 = "Entendido. Proporciona текст для перевода, и я отвечу только с переводом в кавычках.";
			Link.l1.edit = 2;
