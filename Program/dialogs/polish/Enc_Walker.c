...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			if (NPChar.sex != "man")
			{
                dialog.text = RandPhraseSimple("Mój mąż nie pozwala mi rozmawiać z obcymi!","Nie ma o czym możemy rozmawiać.");
				Link.l1 = "Nic wielkiego.";
				Link.l1.go = "exit";
                break;
			}
			link.l10 = LinkRandPhrase("Powiedz mi, jakie są popularne plotki w lokalnej karczmie?","Czy wolałbyś mi powiedzieć, co się tutaj dzieje?","Hej, kumplu, czy mógłbyś mi powiedzieć, jakie jest życie na suchym lądzie?");
		...
		case "sovet":
			...
			{
                dialog.text = RandPhraseSimple("Mój mąż nie pozwala mi rozmawiać z obcymi!","Nie ma o czym rozmawiać.");
				Link.l1 = "Nic wielkiego.";
				Link.l1.go = "exit";
                break;
			}
		...
		case "new question":
			...
			link.l5 = PCharRepPhrase(RandPhraseSimple("Nie, słyszysz rzeczy. Teraz pójdę.","Nie, nic. Po prostu wyszedłem na spacer."),RandPhraseSimple("Nie, nic. Powodzenia!","Po prostu przechadzam się. Do widzenia."));
			link.l5.go = "exit";
		break;
		...
		case "CitizenNotBlade":
			...
			{
				dialog.text = NPCharSexPhrase(NPChar,"Słuchaj, jako obywatel tego miasta, muszę cię poprosić, abyś powstrzymał się od chodzenia ze wyciągniętym mieczem.");
