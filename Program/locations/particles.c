...
*/
    ...
    if(CheckAttribute(Location, "locators.smoke") && !CheckAttribute(Location, "hidden_effects"))
	{
		...
		{
			locator = GetAttributeN(locator_group, n);
			if(frnd() < 0.3 ) CreateParticleSystem("smoke_medium",stf(locator.x),stf(locator.y),stf(locator.z),-1.57,0,0,0);
		}
	}
	...
	if(CheckAttribute(Location, "locators.torch_deck"))
	{
		...
		for(n = 0; n < num; n++)
		{
			...
			CreateParticleSystemX("torch",stf(locator.x),stf(locator.y),stf(locator.z),
				stf(locator.vz.x),stf(locator.vz.y),stf(locator.vz.z),0);
			CreateParticleSystem("smoke_light",stf(locator.x),stf(locator.y),stf(locator.z),0,0,0,0);
		...
		if(isHeatersEnable == true)
		{
			...
			for(int n = 0; n < num; n++)
			{
				...
				if(bLandfire)
				{
					...
					CreateParticleSystem("smoke_medium", x, y + 0.30, z, -1.57, 0,  0,  0); // Дым
				}	
				else
				{
...
bool CreateFireParticles(string sGroup, string sLocator)
{
	...
	CreateParticleSystemX("smoke_medium", x, y+1.15, z, x, y+1.15, z, 0); // Дым
	return true;
}
