...
// Jason --> функция занулит определённый товар в определённом магазине
...
void SetNull2Deposit(string _city) {
	...
	if (CheckAttribute(pchar, sQuest1) || CheckAttribute(pchar, sQuest2)) {
        Log_Info(XI_ConvertString("DepositsGone1") + GetCityName(_city) + XI_ConvertString("DepositsGone2"));
