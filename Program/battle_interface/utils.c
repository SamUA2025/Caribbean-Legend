...
//=======================================================================
//  Расчет общего процента повреждения всех парусов с учётом ломаных мачт
//=======================================================================
...
void InitShipHulls() 
{
	for (int shipIndex = 0; shipIndex < GetArraySize(&ShipsTypes); shipIndex++) {
		...
		string basePath = "RESOURCE\Textures\Ships\" + ship.name + "1\";
		if (!XI_CheckFolder(basePath)) {
			trace("InitShipHulls: " + ship.name + " has no hulls");
			continue;
		}
		
		int hullIndex = 1;
		while (true) {
			string path = basePath + "hull" + hullIndex + "\";
			if (!XI_CheckFolder(path)) {
				break;
			}
			hullIndex++;
		}
		
		ship.maxHulls = hullIndex - 1;
		trace("InitShipHulls: " + ship.name + " has " + ship.maxHulls + " hulls");
	}
}					  