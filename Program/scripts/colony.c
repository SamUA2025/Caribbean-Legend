...
void SetCrewBackAfterBattle()
{
	...
	if (iTemp > 0)
	{
	    if (GetCargoGoods(Pchar, GOOD_MEDICAMENT) < iTemp)
	    {
			...
			Log_Info(StringFromKey("colony_1", iTemp));
	    }
	    else
	    {
		    ...
		    {
		        Log_Info(StringFromKey("colony_2", Pchar.Ship.Name));
		    }
	    }
	}
	// расчёт медицины <--
...
{
	log_info(StringFromKey("colony_3"));
    ref    fortChr;
    ...
    {
        Log_Info(StringFromKey("colony_4"));
        return false;
    }
...
void TWN_FightInTown_OpenNext()
{
    ...
    Log_Info(StringFromKey("colony_5"));
			...
			switch(makeint(pchar.colonyprise.item))
			{
				...
				case 3:
					switch(drand(7))
					{
						case 0:
							...
							log_info(StringFromKey("colony_8"));
						break; 
						case 1:
							GiveItem2Character(pchar, GetGeneratedItem("blade_27"));
							log_info(StringFromKey("colony_9"));
						break;
						case 2:
							GiveItem2Character(pchar, GetGeneratedItem("blade_20"));
							log_info(StringFromKey("colony_10"));
						break;
						case 3:
				...
				case 4:
					if(CheckAttribute(pchar, "questTemp.AdmiralMap"))
					{
						...
						GiveAdmiralMapToCharacter(pchar, 0);
						GiveAdmiralMapToCharacter(pchar, 0);
						//log_info("Вы получили Отличные карты");
					}
					else
					{
...
void SetGovenoursToResidence() // Jason: усадка генгуберов и командоров - полностью ручной метод
{
	...
	rChar.name = StringFromKey("colony_11"); //шевалье Филипп
	rChar.lastname = StringFromKey("colony_12"); // де Пуанси
	...
	LAi_SetCharacterUseBullet(rChar, GUN_ITEM_TYPE, "bullet");
	...
	rChar.name = StringFromKey("colony_13"); // лорд Томас
	rChar.lastname = StringFromKey("colony_14"); // Виндзор
	...
	rChar.name = StringFromKey("colony_15"); // дон Хуан
	rChar.lastname = StringFromKey("colony_16"); // "де Кордова"
	...
	rChar.name = StringFromKey("colony_17"); // "полковник"
	rChar.lastname = StringFromKey("colony_18"); // "Джонатан Фокс"
	rChar.greeting = "Fox";
...
void TWN_Siege_Any(string city)
{
    ...
    else
    {
	    ...
	    Log_Info(StringFromKey("colony_19", GetCityName(city)));
...
void TWN_End_Siege_Any(string city)
{
    ...
    Log_Info(StringFromKey("colony_20"));
}
///////////////////////////////
// сухопутная осада
void TWN_SiegeLand_Any(string city)
{
    ...
    Log_Info(StringFromKey("colony_21", GetCityName(city)));
...
void TWN_Enemy_Any(string city)
{
	...
	Log_Info(StringFromKey("colony_22", GetCityName(city)));
	makeref(rColony, Colonies[FindColony(city)]);
    DeleteAttribute(rColony, "DontSetShipInPort"); //возвращаем жизнь
}
