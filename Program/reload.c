#define RELOAD_TIME_FADE_OUT	0.1
#define RELOAD_TIME_FADE_IN		0.1
...
float reload_cur_time = -1.0, reload_time = -1.0;
...
void Reload_Pre()
{
	DelEventHandler("evntSave","Reload_Pre"); // убрать прерывание
	SetEventHandler("frame","ReloadStart",1); // то, что было бы сразу
}

void ReloadStart()
{
	DelEventHandler("frame","ReloadStart"); // может не быть прерывания
...
{
	//ResetSoundScheme();
	//belamour legendary edition не сбрасывать звуки в одной схеме
    reload_time = GetTime();
	if(reload_cur_location_index >= 0 && reload_location_index >=0)
	{
...
//    DetectBlades();
    ...
    {
        Environment.oldtime = reload_time;
    }
	ReloadProgressUpdate();
	ReloadProgressEnd();
}
...
int ReloadToLocation(int location_index, aref reload_data)
{
		...
		{   // переход суша-суша
			for (i=0; i<iNumReload; i++)
			{
				...
				if (arLocator.Name == reload_data.emerge)
				{
	                ...
	                {
						if(locations[reload_cur_location_index].id != "Islamona_town")
	...
	if (CheckAttribute(&locations[location_index], "changeAnimation")) 
	{
		...
		{
			if(startHeroType != 4) mc.model.animation = mc.model;
			else mc.model.animation = "Rumba";
		...
		{
			/* if(startHeroType == 1 || startHeroType == 4) mc.model.animation = mc.model;
			else mc.model.animation = locations[location_index].changeAnimation; */
			mc.model.animation = mc.model;
			if(locations[location_index].type =="underwater") 
			{
				if(startHeroType == 4) mc.model.animation = "Rumba_swim";
				else mc.model.animation = "swim";
			}
		}	
	}
