...
int CreateCuracaoCharacters(int n)
{
	...
	ch.id		= "Villemstad_Mayor"; // belamour legendary edition
	ch.model	= "huber_25";
	ch.sex = "man";
	ch.name 	= GetCharacterName("Matthias");	// Матиас
    ch.lastname = GetCharacterName("Beck");	// Бек
	ch.City = "Villemstad";
...
//	LAi_SetLoginTime(ch, 6.0, 21.99);
	...
	// belamour legendary edition Хозяйка борделя
	makeref(ch,Characters[n]);
	ch.id		= "Villemstad_Hostess";
	ch.name		= GetCharacterName("Florentina");	// Флорентина
	ch.lastname = "";
	ch.model	= "maman_1";
