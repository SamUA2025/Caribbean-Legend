...
{// метод вызывается в момент новой игры, заполняет главного героя атрибутами, чтоб убрать лишние проверки в if
	...
	// ==> набор мелких квестов, даётся по одному до закрытия
	...
	pchar.quest.FishingBoat.win_condition.l1 = "Rank";
	pchar.quest.FishingBoat.win_condition.l1.value = 2; 
	pchar.quest.FishingBoat.win_condition.l1.operation = ">=";
...
void QuestActions()
{
	...
	if(CheckAttribute(pchar, "questTemp.ReasonToFast"))
	{
		...
		if(pchar.questTemp.ReasonToFast == "MeetPatrolFail") // в бухту пришёл не один или удачи не хватило
		{
			...
			AddQuestUserData("ReasonToFast", "sSex", GetSexPhrase(StringFromKey("LSC_Q2Utilite_1"),StringFromKey("LSC_Q2Utilite_2")));		
			CloseQuestHeader("ReasonToFast");
			pchar.questTemp.ReasonToFast.end = true;
		}
	...
	if (!CheckAttribute(pchar, "questTemp.ChickenGod") && GetDLCenabled(DLC_APPID_2) && sti(pchar.rank) >= 5) {
		DeleteQuestCondition("ChickenGod_Init");
		ChickenGod_Init("");
	}
}
...
void SetCapitainFromSeaToCity(string sChar)
{
	...
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_3", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name, GetFullName(sld)), 
		StringFromKey("LSC_Q2Utilite_4", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name), 
		StringFromKey("LSC_Q2Utilite_5", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
}
...
void PortmansBook_writeQuestBook(ref rid)
{
	...
	if (iTemp != -1)
	{
		...
		{
			AddQuestUserData(sTitle, "sAreal", StringFromKey("LSC_Q2Utilite_6") + XI_ConvertString(GetIslandByCityName(sld.quest.targetCity) + "Voc")); // belamour gen
		}
	}
}
...
void SetRobberFromMapToSea(string sChar)
{
	...
	AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_7", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)), 
		StringFromKey("LSC_Q2Utilite_8", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Gen")), sld.Ship.name), 
		StringFromKey("LSC_Q2Utilite_9", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
}
...
void PortmansSeekShip_writeQuestBook(ref rid)
{
	...
	if (iTemp != -1)
	{
		...
		if (GetIslandByCityName(sld.quest.targetCity) != sld.quest.targetCity)
		{
			AddQuestUserData(sTitle, "sAreal", StringFromKey("LSC_Q2Utilite_10") + XI_ConvertString(GetIslandByCityName(sld.quest.targetCity) + "Voc")); // belamour gen
		}
	}
}
...
void CitizCapFromMapToCity(string sChar)
{
	...
	{	//ходит в городе
		...
		//даем слух, что кэп на рейде
		AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_11", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)), 
			StringFromKey("LSC_Q2Utilite_12", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name), 
			StringFromKey("LSC_Q2Utilite_13", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
	}
	else
	{	//стоит на рейде в порту
		...
		Group_SetTaskNone(sGroup);
		//даем слух, что кэп на рейде
		AddSimpleRumourCity(LinkRandPhrase(StringFromKey("LSC_Q2Utilite_14", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld)), 
			StringFromKey("LSC_Q2Utilite_15", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name), 
			StringFromKey("LSC_Q2Utilite_16", GetStrSmallRegister(XI_ConvertString(RealShips[sti(sld.Ship.Type)].BaseName+"Acc")), sld.Ship.name, GetFullName(sld))), sld.City, Qty, 1, "none");
	}
	//таймер через сколько опять выйти на карту
...
void CitizSeekCap_writeQuestBook(ref rid)
{
	...
	if (iTemp != -1)
	{
		...
		if (GetIslandByCityName(sld.quest.targetCity) != sld.quest.targetCity)
		{
			AddQuestUserData(sTitle, "sAreal", StringFromKey("LSC_Q2Utilite_17") + XI_ConvertString(GetIslandByCityName(sld.quest.targetCity) + "Voc")); // belamour gen
		}
	}
}
