...
//belamour получить название лейбла у конечного локатора
string chrGetLabelName(ref loc, string reloadLocatorName)
{
	//Reload locator
	aref rl, at;
	makearef(rl, loc.reload);
	...
	for(int i = 0; i < num; i++)
	{
		...
		if(at.name == reloadLocatorName)
		{
			if(CheckAttribute(at, "label")) return at.label;
		}
	}
	return "";
}