		...
		if( !chrCheckReloadLocatorSkip(loc,locator) )
		{
			...
			if(chrCheckAutoReload(loc, locator) == true)
			{
				...
				if(chrIsNowEnableReload() != true)
				{
					...
					if(chrGetLabelName(loc,locator) == "sea") Log_SetActiveAction("ToSea");
...
ref funcGetWeaponID()
{
	...
	if (iTemp >= 0 && iTemp < MAX_CHARACTERS && CheckAttribute(&characters[iTemp], "equip.blade"))
	{
		...
		if (!CharUseMusket(&characters[iTemp]) && findsubstr(bladeId, "topor" , 0) != -1) g_strRetParam = "topor";
	}	
