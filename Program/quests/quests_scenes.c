...
void procQuestSceneCommand()
{
	...
	case "escape": // "ssas"
		...
		return;
	break;
	// умереть (без параметров), но персонаж не умирает в действительности и остаётся в данной локации
	case "dead": // "ssa"
	...
	case "ReloadToLocation": // "ssasss"
		...
		return;
	break;
	// подождать пока пройдёт заданное число секунд (задаем число секунд, использовать число типа float)
	case "TimeWait": // "ssaf"
		f1 = GetEventData();
...
void QuestSceneStartShot(ref shooter, ref shot, string consequenceFunc, string quest) {
...
void QuestScenePerformShot() {
	int iShooter = GetEventData();
	ref shooter = GetCharacter(iShooter);
	string quest = GetEventData();

	if (shooter.model.animation != "Rumba") {
		LAi_ActorAnimation(shooter, "Shot", quest, 1.5);
	} else {
		LAi_ActorAnimation(shooter, "Shot", quest, 1.25);
	}
	
	DelEventHandler("event_" + shooter.index + "_Shot", "QuestScenePerformShot");
}

void QuestSceneShotDeath() {
	int iShot = GetEventData();
	ref shot = GetCharacter(iShot);
	string consequenceFunc = GetEventData();

	if (consequenceFunc != "") {
		call consequenceFunc(shot);
	}
	
	DelEventHandler("event_" + shot.index + "_Dead", "QuestSceneShotDeath");
}

void QuestSceneStartMelee(ref attacker, ref defender, string consequenceFunc, string quest) {
...
void QuestScenePerformMelee() {
	int iAttacker = GetEventData();
	ref attacker = GetCharacter(iAttacker);
	string quest = GetEventData();

	SendMessage(attacker, "ll", MSG_CHARACTER_BLADEHAND, 1);
	LAi_ActorAnimation(attacker, "attack_force_3", quest, 0.9);
	
	DelEventHandler("event_" + attacker.index + "_Melee", "QuestScenePerformMelee");
}

void QuestSceneMeleeHit() {
	int iDefender = GetEventData();
	ref defender = GetCharacter(iDefender);
	string consequenceFunc = GetEventData();

	if (consequenceFunc != "") {
		call consequenceFunc(defender);
	}
	
	DelEventHandler("event_" + defender.index + "_Hit", "QuestSceneMeleeHit");
}
