...
{
	return GetDateStringEx(GetDataYear(), GetDataMonth(), GetDataDay());
}
...
string GetTimeStringEx(int hour, int minute)
{
	...
	sTime = sHour + ":" + sMinuts + sAdd;
	return sTime;
}

string GetTimeString()
{
	return GetTimeStringEx(sti(worldMap.date.hour), sti(worldMap.date.min));
}
// boal <--
...
void AddQuestRecordEx(string idQuest,string idReferenceQuest,string idText)
{
	...
	if( sTextLbl != "" )
	{
		...
		pchar.QuestInfo.(idQuest).Text.(sTextLbl) = "@"+idReferenceQuest+"@"+GetQuestBookData()+NewStr()+" "+"@"+idText;
		notification(XI_ConvertString("Logbook Update"), "LogBook");
...
// Прерывание на таймер с функцией с учётом часов <-- ugeen
...
void CalculateCheatsInfo() // Для статистики по читам. Используется в интерфейсе Debuger
{
	...
	SetFormatedText("INFO_TEXT2", XI_ConvertString("CM_CheatsUsed") + " ");
	...
	for(i=0; i<iAllCheats; i++)
	{
		...
		if(HasSubStr(sCheat, "F"))
			AddLineToFormatedText("INFO_TEXT2", XI_ConvertString("D_" + sCheat) + " - " + iCurCheatCount + " "+ XI_ConvertString("CM_times")+".");
		else
			AddLineToFormatedText("INFO_TEXT2", sCheat + " - " + iCurCheatCount + " "+ XI_ConvertString("CM_times")+".");
			
		iAllCheatsCount = iAllCheatsCount + iCurCheatCount;
	}
	
	PChar.Statistic.Cheats = iAllCheatsCount;
	AddLineToFormatedText("INFO_TEXT2", XI_ConvertString("Total") + " " + iAllCheatsCount + " "+ XI_ConvertString("CM_times")+".");
	
	if(iAllCheatsCount == 0) // Исли 0, значит читов не юзали
		SetFormatedText("INFO_TEXT2", XI_ConvertString("CM_notuse")+"!");
}
...
// belamour блок квестовых маркеров
void AddLandQuestMark(ref chr, string model)
{
	if(!CheckAttribute(&InterfaceStates,"EnabledQuestsMarks")) return;
	if(InterfaceStates.EnabledQuestsMarks == 1 || IgnoreQmCheckbox())
	{
		chr.quest.questflag.model = model;
		chr.quest.questflag.technique = "QuestMark"; 
		chr.quest.questflag.nochange = true;
	}
}

void DelLandQuestMark(ref chr)
{
	if(CheckAttribute(chr,"quest.questflag.model")) 
	{
		chr.quest.questflag.model = "";
		DeleteAttribute(chr,"quest.questflag.model");
	}
	if(CheckAttribute(chr,"quest.questflag.technique"))
	{
		chr.quest.questflag.technique = "";
		DeleteAttribute(chr,"quest.questflag.technique");
	}
	if(CheckAttribute(chr,"quest.questflag.nochange")) DeleteAttribute(chr,"quest.questflag.nochange");
}

bool IgnoreQmCheckbox()
{
	if(CheckAttribute(pchar,"questTemp.Sharlie.Lock")) return true;
	
	return false;
}

void AddLandQuestMarkToPhantom(string locID, string ChrID)
{
	SetFunctionLocationCondition("AddLandQuestMarkToPhantom_Next", locID, false);
	pchar.systeminfo.questmarkChrID = ChrID;
}

void AddLandQuestMarkToPhantom_Next(string qName)
{
	DoQuestFunctionDelay("AddLandQuestMarkToPhantom_delay", 1.0);
}

void AddLandQuestMarkToPhantom_delay(string qName)
{
	if(CheckAttribute(pchar,"systeminfo.questmarkChrID"))
	{
		if (GetCharacterIndex(pchar.systeminfo.questmarkChrID) != -1)
		{
			AddLandQuestMark(characterFromId(pchar.systeminfo.questmarkChrID), "questmarkmain");
		}
		DeleteAttribute(pchar,"systeminfo.questmarkChrID");
	}
}

void DelLandQuestMarkToPhantom()
{
	if(CheckAttribute(pchar,"systeminfo.questmarkChrID"))
	{
		if (GetCharacterIndex(pchar.systeminfo.questmarkChrID) != -1)
		{
			DelLandQuestMark(characterFromId(pchar.systeminfo.questmarkChrID));
		}
		if(CheckAttribute(pchar,"quest.AddLandQuestMarkToPhantom_Next"))
		{
			pchar.quest.AddLandQuestMarkToPhantom_Next.over = "yes";
		}
		DeleteAttribute(pchar,"systeminfo.questmarkChrID");
	}
}

void AddMapQuestMarkCity(string City, bool AutoDel)
{
	if(InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
	
	string sAttr = City+"_town";
	string dCity = "QM1"+City;
	if(!IsEntity(&worldMap))
	{
		worldMap.labels.(sAttr).icon1 = 7;
		worldMap.labels.(sAttr).majorQM = true;
	}
	else
	{
		pchar.quest.(dCity).win_condition.l1			= "EnterToSea";           	
        pchar.quest.(dCity).function    				= "DelayMapQMC";
	}
	if(AutoDel)
	{
		dCity = "QM2"+City;
		pchar.quest.(dCity).win_condition.l1           = "location";
	    pchar.quest.(dCity).win_condition.l1.location  = sAttr;
	    pchar.quest.(dCity).function                   = "DelayMapQMCDel";
	}
}

void DelayMapQMC(string City)
{
	string dCity = strcut(City, 3 , strlen(City)-1);
	string sAttr = dCity+"_town";
	pchar.quest.(dCity).over = "yes";
	worldMap.labels.(sAttr).icon1 = 7;
	worldMap.labels.(sAttr).majorQM = true;
}

void DelMapQuestMarkCity(string City)
{
	string sAttr = City+"_town";
	if(!IsEntity(&worldMap))
	{
...
void DelayMapQMCDel(string City)
{
	string dCity = strcut(City, 3 , strlen(City)-1);
	string sAttr = dCity+"_town";
	pchar.quest.(dCity).over = "yes";
...
void AddMapQuestMarkShore(string Shore, bool AutoDel)
{
	if(InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
	
	string sAttr = Shore;
	string dShore = "QM1"+Shore;
	if(!IsEntity(&worldMap))
	{
		worldMap.labels.(sAttr).icon1 = 7;
		worldMap.labels.(sAttr).heightView = 1000.0;
		worldMap.labels.(sAttr).majorQM = true;
	}
	else
	{
		pchar.quest.(dShore).win_condition.l1			= "EnterToSea";           	
        pchar.quest.(dShore).function    				= "DelayMapQMS";
	}
	if(AutoDel)
	{
		dShore = "QM2"+Shore;
		pchar.quest.(dShore).win_condition.l1           = "location";
	    pchar.quest.(dShore).win_condition.l1.location  = sAttr;
	    pchar.quest.(dShore).function                   = "DelayMapQMSDel";
	}
}

void DelayMapQMS(string Shore)
{
	string dShore = strcut(Shore, 3 , strlen(Shore)-1);
	string sAttr = dShore;
	pchar.quest.(dShore).over = "yes";
	worldMap.labels.(sAttr).icon1 = 7;
	worldMap.labels.(sAttr).heightView = 1000.0;
	worldMap.labels.(sAttr).majorQM = true;
}

void DelMapQuestMarkShore(string Shore)
{
	string sAttr = Shore;
	if(!IsEntity(&worldMap))
	{
		worldMap.labels.(sAttr).heightView = 200.0;
		if(CheckAttribute(worldMap,"labels."+sAttr+".icon1"))
			DeleteAttribute(&worldMap,"labels."+sAttr+".icon1");
		if(CheckAttribute(worldMap,"labels."+sAttr+".majorQM"))
			DeleteAttribute(&worldMap,"labels."+sAttr+".majorQM");
...
void DelayMapQMSDel(string Shore)
{
	string dShore = strcut(Shore, 3 , strlen(Shore)-1);
	string sAttr = dShore;
	pchar.quest.(dShore).over = "yes";
...
void AddMapQuestMarkIsland(string Island, bool AutoDel)
{
	if(InterfaceStates.EnabledQuestsMarks == 0 && !IgnoreQmCheckbox()) return;
	
	string sAttr = Island;
	string dIsland = "QM1"+Island;
	if(!IsEntity(&worldMap))
	{
		worldMap.labels.(sAttr).icon1 = 7;
		worldMap.labels.(sAttr).majorQM = true;
	...
	{
		pchar.quest.(dIsland).win_condition.l1			= "EnterToSea";           	
        pchar.quest.(dIsland).function    				= "DelayMapQMI";
	}
	if(AutoDel)
	{
		dIsland = "QM2"+Island;
		pchar.quest.(dIsland).win_condition.l1           = "location";
	    pchar.quest.(dIsland).win_condition.l1.location  = sAttr;
	    pchar.quest.(dIsland).function                   = "DelayMapQMIDel";
	}
}

void DelayMapQMI(string Island)
{
	string dIsland = strcut(Island, 3 , strlen(Island)-1);
	string sAttr = dIsland;
	pchar.quest.(dIsland).over = "yes";
	worldMap.labels.(sAttr).icon1 = 7;
	worldMap.labels.(sAttr).majorQM = true;
}

void DelMapQuestMarkIsland(string Island)
{
	string sAttr = Island;
	if(!IsEntity(&worldMap))
	{
...
void DelayMapQMIDel(string Island)
{
	string dIsland = strcut(Island, 3 , strlen(Island)-1);
	string sAttr = dIsland;
	pchar.quest.(dIsland).over = "yes";
...
void CheckMapQuestMarks()
{
	if(InterfaceStates.EnabledQuestsMarks == 0) return;
	bool bIslandCondition = false; // не удаляем знак от локаций без прерываний
	if(!IsEntity(&worldMap)) // пока обработка только в sea_ai
	{
		for(int i=0; i<MAX_LOCATIONS; i++)
		{
			if(!CheckAttribute(&Locations[i],"type")) continue;
			bool bLocFromSea = false;
			if(Locations[i].type == "seashore" || Locations[i].type == "town") bLocFromSea = true;
			if(!bLocFromSea)
			{
				if(!CheckAttribute(&Locations[i],"islandId")) continue;
				if(!CheckAttribute(&worldMap,"labels."+Locations[i].islandId+"")) continue;
				if(bQuestConditionInLoc(Locations[i].id))
				{
					
					log_testinfo("Id "+ Locations[i].Id);
					log_testinfo("islandId "+ Locations[i].islandId);
					AddMapMinorQuestMarkIsland(Locations[i].islandId);
					bIslandCondition = true;
				}
				else
				{
					if(!bIslandCondition) DelMapMinorQuestMarkIsland(Locations[i].islandId);
				}
				continue;
			}
			if(!CheckAttribute(&worldMap,"labels."+Locations[i].id+"")) continue;
			if(CheckAttribute(&worldMap,"labels."+Locations[i].id+".majorQM")) continue;
			if(bQuestConditionInLoc(Locations[i].id))
			{
				AddMapMinorQuestMark(Locations[i].id);
			}
			else
			{
				DelMapMinorQuestMark(Locations[i].id);
			}
		}
	}
	else
	{
		pchar.quest.CheckMapQuestMarksDelay.win_condition.l1			= "EnterToSea";           	
        pchar.quest.CheckMapQuestMarksDelay.function    				= "CheckMapQuestMarks_Delay";
	}
	
}

void CheckMapQuestMarks_Delay(string quest)
{
	CheckMapQuestMarks();
}

void AddMapMinorQuestMark(string sAttr)
{
	worldMap.labels.(sAttr).icon1 = 6;
	if(worldMap.labels.(sAttr).type == "Shore")
	{
		worldMap.labels.(sAttr).heightView = 1000.0;
	}
}

void DelMapMinorQuestMark(string sAttr)
{
	if(CheckAttribute(worldMap,"labels."+sAttr+".icon1"))
		DeleteAttribute(&worldMap,"labels."+sAttr+".icon1");
	if(worldMap.labels.(sAttr).type == "Shore")
...
void AddMapMinorQuestMarkIsland(string sAttr)
{
	worldMap.labels.(sAttr).icon = 6;
}

void DelMapMinorQuestMarkIsland(string sAttr)
{
	if(CheckAttribute(worldMap,"labels."+sAttr+".icon"))
		DeleteAttribute(&worldMap,"labels."+sAttr+".icon");
}

bool bQuestConditionInLoc(string loc_id)
{
	aref quests, quest;
	int  i, nQuestsNum;
	...
	for (i = 0; i < nQuestsNum; i++)
	{
		...
		if (CheckAttribute(quest, "win_condition.l1") && quest.win_condition.l1 == "Location" && quest.win_condition.l1.Location == loc_id)
		{   // уже висят квесты на эту локацию
			log_testinfo("getname "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
		if (CheckAttribute(quest, "win_condition.l1") && quest.win_condition.l1 == "locator" && quest.win_condition.l1.Location == loc_id)
		{   // уже висят квесты на эту локацию
			log_testinfo("getname locator "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
		if (CheckAttribute(quest, "win_condition.l2") && quest.win_condition.l2 == "Location" && quest.win_condition.l2.Location == loc_id)
		{   // уже висят квесты на эту локацию
			log_testinfo("getname l2 "+GetAttributeName(quest)+" locID "+loc_id);
			bConditions = true;
			break;
		}
	}
...
// belamour проверка рецептов для изучения
void CheckAlchemyRecipe()
{
	if(!bLandInterfaceStart || LAi_IsFightMode(pchar)) return;
	string sItem;
	ref rItem;
	aref arItems; 
	int i,iItemsNum;
	...
	for(i=0; i<iItemsNum; i++)
	{
		...
		if(rItem.groupID != SPECIAL_ITEM_TYPE) continue;
		
		if (findsubstr(rItem.id, "recipe_" , 0) != -1) 
		{
			if(!isMultiObjectKnown(rItem.result))
			{
				AddQuestRecordInfo("Recipe", rItem.result);
				SetAlchemyRecipeKnown(rItem.result);
				RemoveItems(pchar, rItem.id, 1);
				i = 0;
				iItemsNum = GetAttributesNum(arItems);
			}
		}
...
bool HasAlchemyRecipe()
{
	if(!bLandInterfaceStart || LAi_IsFightMode(pchar)) return false;
	string sItem;
	ref rItem;
	aref arItems; 
	int i,iItemsNum;
...
// belamour cle непрочитанная квестовая запись
bool CheckQuestInfo()
{
