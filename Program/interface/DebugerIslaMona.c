...
bool AddTestCube(string cubePrefix) {
	ref loc = &locations[FindLocation("Test")];
	aref cube;
	...
	if (cur < max) {
		cur++;
		string model = cubePrefix + "_" + cur;
		loc.models.always.(model) = model;
		cube.cur = cur;
		return true;
	} else {
		return false;
...
int CountTestPolygons() {
	ref loc = &locations[FindLocation("Test")];
	aref cube, cubes;
	int qty;
	int total = 0;
	
	makearef(cubes, loc.cubes);
	qty = GetAttributesNum(cubes);
...
string descF1 = "Телепорт в бухту Губернатора с установкой корабля в неё";
...
void CalculateInfoDataF1()
{
	...
	DoReloadCharacterToLocation("Shore75", "reload", "reload1");
...
string descF2 = "Первая стадия бухты";
...
string descF3 = "Финальная стадия бухты";
...
////////////////////////////////////////////////////////////////////////
string descF4 = "Открыть вход в факторию";
...
////////////////////////////////////////////////////////////////////////
string descF5 = "Поставить пушку в боевое положение";
...
////////////////////////////////////////////////////////////////////////
string descF6 = "Поставить пушку в мирное состояние";
...
////////////////////////////////////////////////////////////////////////
string descF7 = "ИслаМона стадия 2";
...
////////////////////////////////////////////////////////////////////////
string descF8 = "ИслаМона стадия 3";
...
////////////////////////////////////////////////////////////////////////
string descF9 = "Посадить пьяниц на третьей стадии";
...
void CalculateInfoDataF9()
{
	...
	int iCitizensQuantity = 10; // простая пьянь для антуражу
	int iColony = FindColony("IslaMona");
	int iNation = PIRATE;
...
void CalculateInfoDataF10()
{
    ...
    if (LAi_IsImmortal(mc))
    {
		...
		DeleteAttribute(mc, "ShipHideImmortal");// бессмертие корабля
        Log_SetStringToLog("God mode OFF");
    }
    else
    {
        LAi_SetImmortal(mc, true);
		mc.ShipHideImmortal = 1000;// бессмертие корабля
...
////////////////////////////////////////////////////////////////////////
string descF11 = "Подсчет числа полигонов в тестовой локации";

void CalculateInfoDataF11()
{
	totalInfo = descF11;
	
	totalInfo = totalInfo + NewStr() + NewStr() + "Итого " + CountTestPolygons() + " полигонов";
...
string descF12 = "Создать поселение на Исла-Моне";

void CalculateInfoDataF12() {
	totalInfo = descF12;
	
	int n = FindLocation("Location_reserve_02");
	ReinitLocation(n);
	...
	//Models
	//Always
	Locations[n].filespath.models = "locations\Town_IslaMona\City\";
	Locations[n].models.always.IslaMonaCity = "IslaMonaCity";	
	Locations[n].models.always.locators = "IslaMonaCity_locators";	
	//Day
	Locations[n].models.day.charactersPatch = "IslaMonaCity_patch";
	//Night
	Locations[n].models.night.charactersPatch = "IslaMonaCity_patch";
	//Environment
	locations[n].environment.weather = "true";
	...
	ref colony = GetColonyRefByID("IslaMona");
	colony.HeroOwn = true;
...
void CalculateInfoDataF13()
{
    ...
    SetBaseShipData(mc);
	mc.ship.name = "Фортуна";
	RealShips[sti(mc.Ship.Type)].ship.upgrades.hull  = 1;
    ...
    SetCharacterGoods(mc, GOOD_WEAPON, 700);
	SetCharacterGoods(mc, GOOD_MEDICAMENT, 1200); 

    string sEquipItem = GetCharacterEquipByGroup(mc, BLADE_ITEM_TYPE);
    ...
    sEquipItem = GetGeneratedItem("blade_33");
    ...
    AddItems(mc, "GunCap_colt", 50);
    AddItems(mc, "shotgun_cartridge", 50);

    AddItems(mc, "sextant2", 1);
	EquipCharacterbyItem(mc, "sextant2");
	AddItems(mc, "totem_06", 15);
...
//	ShipRepair(mc);
	...
	SetCharacterGoods(mc, GOOD_BALLS, 200);
	SetCharacterGoods(mc, GOOD_GRAPES, 200);
	SetCharacterGoods(mc, GOOD_KNIPPELS,200);
	SetCharacterGoods(mc, GOOD_BOMBS, 200);
	SetCharacterGoods(mc, GOOD_FOOD, 200);
	SetCharacterGoods(mc, GOOD_POWDER, 400);
	SetCharacterGoods(mc, GOOD_WEAPON, 200);

	SetCharacterGoods(mc, GOOD_PLANKS, 50);
	SetCharacterGoods(mc, GOOD_SAILCLOTH, 30);
...
string descF33 = "Прокачка ГГ и битва между нужными кораблями";
void CalculateInfoDataF33()
{
	...
	// указываем нужный корабль ГГ из ships.h
	mc.Ship.Type = GenerateShipExt(SHIP_LSHIP_ENG, true, mc);
	SetBaseShipData(mc);
...
// указываем вражеский корабль
	...
	FantomMakeCoolFighter(sld, 20, 50, 50, "slave_02", "pistol1", "bullet", 10);
	FantomMakeCoolSailor(sld, SHIP_CURSED_FDM, "Пират", CANNON_TYPE_CANNON_LBS36, 50, 50, 50);
	sld.name = "Мэр";
	sld.lastname = "Кривожополя";
	sld.FaceId = 29;
	sld.rank = 20;
	sld.reputation = 15;
	sld.dialog.filename = "PIRATE\CapitainWrag.c";
	...
	ref sld;
	sld = &RealShips[sti(sld.Ship.Type)];
	sld.MaxCaliber = 36;
	sld.MaxCrew = 777;
	sld.OptCrew = 622;
	sld.MinCrew = 94;	
	...
	Group_AddCharacter("CapitainWrag_Group", "CapitainWrag");
	Group_SetType("CapitainWrag_Group", "pirate");
	Group_SetGroupCommander("CapitainWrag_Group", "CapitainWrag");
	Group_SetAddress("CapitainWrag_Group", "Bermudes", "quest_ships", "quest_ship_8");
	Group_SetTaskAttack("CapitainWrag_Group", PLAYER_GROUP);
//
