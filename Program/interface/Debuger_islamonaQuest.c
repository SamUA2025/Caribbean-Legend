...
string descF1 = "Начать квест по постройке Исламоны";
...
void CalculateInfoDataF1()
{
	...
	sld.InOurHouse = true;
    sld.dialog.currentnode = "mirabelle_26";
	Mtraxx_MirabellaLife("");
	...
	AddItems(pchar, "mineral30", 15);
	AddItems(pchar, "mushket2", 1);
	AddItems(pchar, "gold_dublon", 3000);
	AddItems(pchar, "totem_06", 1);
	SetCharacterGoods(pchar, GOOD_OIL, 250);
	SetCharacterGoods(pchar, GOOD_CLOTHES, 500);
	SetCharacterGoods(pchar, GOOD_PLANKS, 4000);
	SetCharacterGoods(pchar, GOOD_SAILCLOTH, 2000);
	SetCharacterGoods(pchar, GOOD_BRICK, 2000);
	SetCharacterGoods(pchar, GOOD_LEATHER, 1500);
	SetCharacterGoods(pchar, GOOD_RUM, 1000);
	SetCharacterGoods(pchar, GOOD_WINE, 1000);
	SetCharacterGoods(pchar, GOOD_MEDICAMENT, 1000);
	SetCharacterGoods(pchar, GOOD_FOOD, 4000);
	SetCharacterGoods(pchar, GOOD_SLAVES, 15);
...
string descF2 = "Телепорт на остров";
...
////////////////////////////////////////////////////////////////////////
string descF5 = "Телепорт в дом исламоны";
...
void CalculateInfoDataF5()
{
   ...
   setCharacterShipLocation(pchar, "Shore75");
	setWDMPointXZ("Shore75");
	DoReloadCharacterToLocation("IslaMona_TwoFloorRoom", "reload", "reload1");
...
string descF13 = "Время на постройку таверны";
void CalculateInfoDataF13()
{
	idLngFile = LanguageOpenFile("ItemsDescribe.txt");
	// -->
...
string descF14 = "Время на постройку причала";
...
string descF15 = "Время на постройку Фактории";
...
string descF18 = "Добавить ресурсы для строительства церкви";
...
void CalculateInfoDataF18()
{
	...
	ref sld = &stores[29];
	AddStorageGoods(sld, GOOD_PLANKS, 300);
	AddStorageGoods(sld, GOOD_BRICK, 100);
	AddStorageGoods(sld, GOOD_OIL, 10);
	AddStorageGoods(sld, GOOD_SANDAL, 20);
	AddStorageGoods(sld, GOOD_MAHOGANY, 100);
	AddItems(pchar, "jewelry5", 50);
	AddItems(pchar, "mineral3", 100);
	AddItems(pchar, "jewelry8", 30);
	AddItems(pchar, "jewelry7", 1);
    // <
    totalInfo = totalInfo + LanguageConvertString(idLngFile,"new_string") + LanguageConvertString(idLngFile,"new_string") +
                "Команда отработала успешно!";
    LanguageCloseFile(idLngFile);
...
void CalculateInfoDataF24()
{
	...
	int i, n;
	string sTemp;
	GiveItem2Character(pchar, "topor_04");
	/* sld = &items[1015];
	log_info("Item " + sld.id); */
