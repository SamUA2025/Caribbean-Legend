...
void InitInterface_R(string iniName, ref rChar)
{
	...
	GameInterface.title = "titleSelfRepair";	
	StartAboveForm(true);
	...
	SetNewGroupPicture("REPAIR_SAIL_P_PIC",    "GOODS", "SailCloth");
...
//			trace("sChrId " + sChrId);
	...
	{
		GameInterface.strings.shipname = xi_refCharacter.ship.name;
	}
	else
	{
...
void RepairStatShow(ref chr, bool isVisible)
{
	...
	if(isVisible)
	{
		...
		string sText;
		sText = XI_ConvertString("RepairState") + ":\n" + hp + "%\n" + XI_ConvertString("RepairLimit") + "\n" + XI_ConvertString("RepairRecovery") + ":\n" + chr.repair.hull_limit + "%";
		...
		sText = XI_ConvertString("RepairTime") + ":\n" + GetMaxRepairTime() + " " + XI_ConvertString("hours");
		SetFormatedText("TIME_STR", sText);
		SendMessage( &GameInterface,"lsl",MSG_INTERFACE_MSG_TO_NODE,"TIME_STR", 5 );
		SendMessage( &GameInterface,"lsl",MSG_INTERFACE_MSG_TO_NODE,"REPAIR_QTY_H", 5 );
		SendMessage( &GameInterface,"lsl",MSG_INTERFACE_MSG_TO_NODE,"REPAIR_QTY_M", 5 );
		SendMessage( &GameInterface,"lsl",MSG_INTERFACE_MSG_TO_NODE,"REPAIR_QTY_S", 5 );
	}	
