...
void InitInterface(string iniName)
{
	...
	for(i = 0; i < GOODS_QUANTITY; i++)	// Картинки товаров
	{
		...
		Y += 130; // Интервал между картинками
	}
	...
	CreateString(true, "CurCompanion", XI_ConvertString(GetShipTypeName(PChar)) + " '" + PChar.Ship.Name + "'", FONT_CAPTION, COLOR_NORMAL, 960, 940, SCRIPT_ALIGN_CENTER, 1.0);
	...
	XI_RegistryExitKey("IExit_Escape"); // выход на ESC
}
...
void CreateGoodNamesStrings()
{
	...
	for(int i = 0; i < GOODS_QUANTITY; i++)	// Названия товаров
	{
		...
		if(CheckAttribute(GameInterface, "GOODS_ICONS.imagelist." + sGood))
		{
			...
			// Строки
			SendMessage(&GameInterface,"lslsssllllllfl",MSG_INTERFACE_MSG_TO_NODE, "GOODS_NAMES", 0, sGood, XI_ConvertString(sGood), FONT_NORMAL, X, Y, COLOR_NORMAL, COLOR_NORMAL, SCRIPT_ALIGN_CENTER, true, 1.3, 100);
			SendMessage(&GameInterface,"lslsssllllllfl",MSG_INTERFACE_MSG_TO_NODE, "GOODS_NAMES", 0, "Transfer" + sGood, XI_ConvertString("BuyGoods") + buyCount, FONT_NORMAL, x, y + 94, COLOR_NORMAL, COLOR_NORMAL, SCRIPT_ALIGN_CENTER, true, 1.2, 100);
		}
	}
}
...
void ShowTransferGoods(int iGood)
{
	...
	string sHeader = GetConvertStr(sGood, "GoodsDescribe.txt");
