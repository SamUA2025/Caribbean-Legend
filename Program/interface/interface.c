...
bool bGameMenuStart = false;// проверка на открытое esc menu для скрывания интерфейса
...
// Sith --->
void LaunchInventory()	// Интерфейс снаряжения
{
	if(procInterfacePrepare(INTERFACE_INVENTORY))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_INVENTORY;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void LaunchAbilities()	// Интерфейс навыков
{
	if(procInterfacePrepare(INTERFACE_ABILITIES))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_ABILITIES;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void LaunchDeath()	// Интерфейс смерти
{
	if(procInterfacePrepare(INTERFACE_DEATH))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_DEATH;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
...
void LaunchMessage(string messageName)	// Интерфейс уведомления
{
	if(procInterfacePrepare(INTERFACE_MESSAGE))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_MESSAGE;
		InitInterface_S(Interfaces[CurrentInterface].IniFile, messageName);
	}
}
// <--- Sith		   
// ugeen -->
...
void LaunchCheatsMenu()
{
	if(procInterfacePrepare(INTERFACE_BLACKMARK_CHEAT))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_BLACKMARK_CHEAT;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void LaunchCheatsCharcter()
{
	if(procInterfacePrepare(INTERFACE_CHEAT_CHARACTER))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CHEAT_CHARACTER;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}

void LaunchCheatsItems()
{
	if(procInterfacePrepare(INTERFACE_CHEAT_ITEMS))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_CHEAT_ITEMS;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
...
// ===> Sith
void LaunchOptionScreen(bool isMainMenu)
{
	if(procInterfacePrepare(INTERFACE_OPTIONSCREEN))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_OPTIONSCREEN;
		InitInterface_B(Interfaces[CurrentInterface].IniFile, isMainMenu);
	}
}
// <=== Sith
...
void LaunchDebugWhr()
{
	if(procInterfacePrepare(INTERFACE_DEBUGWHR))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_DEBUGWHR;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
// <-- ugeen
...
void ILaunchAfterFrame()
{
	...
	switch(nodName)
	{
		...
		case "I_INVENTORY":		LaunchInventory();  return; break;
		...
		case "I_ATLAS":			LaunchMapViewScreen();	return; break;
		case "I_ABILITIES":		LaunchAbilities();	return; break;
...
void LoadGameOptions()
{
	...
	// belamour legendary edition проверка на сответствие настроек управления
	/* if(CheckAttribute(pchar,"lastgamemod") && pchar.lastgamemod != MOD_BETTATESTMODE)
...
string GetVideoFileName(string baseName)
{
	...
	if( GetTargetPlatform()=="pc" ) return baseName+".webm";
	return baseName+".xmv";
}
...
void StartLanguageSetting(string lngID)
{
	...
	segmentName = LanguageGetLanguage() + "\talksound_alias.ini";
	SendMessage( &Sound, "ls", MSG_SOUND_ALIAS_ADD,  segmentName);
...
{
	if( bPlayVideoNow ) {return;}
	if( dialogRun ) {return;}  // лесник - если диалог ,то быстре сохарнение отменяется. лесник
	...
	{
        Log_Info(XI_ConvertString("SaveChurch"));
		return;
	}
...
{
	NullCharacter.ManualAutoSaveName = autoSaveName;
}
bool MakeAutoSave()
{
...
/*
Пример как вешать прерывание

Если вместо имени сейва с названием локации нужно свое слово "Первый бой" и т.п.
То перед вывозом сейва делаем

MakeAutoSaveNameParam(XI_ConvertString("свое слово метка"));

Вариант записи №1 через два прерывания, как уже было для абордажа
...
{
	DelEventHandler("evntSave","Continue_Sea_AbordageLoadPre"); // убрать прерывание
	SetEventHandler("frame","Continue_Sea_AbordageLoad",1); // то, что было бы сразу
}
...
Вызывать при записи СЖ не стоит, т.к. там обычно идет диалог или середина кода квест-реакшн
Вызывать можно в местах, где не порушит игру.
Переход суша-море. Норм
...
string GetPlayTime()
{
	...
	string sPlayTime = "";
	if( days>0 ) {
		sPlayTime = days + XI_ConvertString("days") + "- ";
	}
	sPlayTime += InterfaceStates.GameTime.hour + " " + XI_ConvertString("hours") + " " + InterfaceStates.GameTime.min + " "+ XI_ConvertString("min");
	return sPlayTime;
}
