	...
	if(!isBoarding)
	{
		...
		if (!CheckAttribute(loc, "DisableOfficers")) //eddy. офицерский флаг
		{
			for(i = 1; i < 4; i++)
			{
				...
				// Rebbebion, вношу правку от Hasert: если абордажные локаторы имеются, спавним абордажников 
				if (pchar.location.locator == "reload1_back")	
				{    
					log_testinfo("ОФФЫ ЕСТЬ1111");
                    Characters[idx].location.locator = "reload1" + "_" + locIndex;
				}
				
                if (pchar.location.locator == "reload2_back")	
				{    
					log_testinfo("ОФФЫ ЕСТЬ2222");
                    Characters[idx].location.locator = "reload2" + "_" + locIndex;
				}
				
                if (pchar.location.locator == "reload3_back")	
				{    
					log_testinfo("ОФФЫ ЕСТЬ3333");
                    Characters[idx].location.locator = "reload3" + "_" + locIndex;
				}
				
				if (pchar.location.locator == "gate_back") 					
                {  
					log_testinfo("ОФФЫ ЕСТЬ44444");
                    Characters[idx].location.locator = "gate" + "_" + locIndex;
                }   				
