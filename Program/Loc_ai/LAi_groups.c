...
void LAi_group_UpdateAlarm()
{
	...
	if(CheckAttribute(pchar, "systeminfo.tutorial.firstfight1_SandBox") && LAi_grp_playeralarm > 0)
	{
		DoQuestFunctionDelay("Tutorial_firstfight1_SandBox", 1.0);
		DeleteAttribute(pchar, "systeminfo.tutorial.firstfight1_SandBox");
	}
	if(CheckAttribute(pchar, "systeminfo.tutorial.secondfight_SandBox") && !bSeaActive && LAi_grp_playeralarm > 0)
	{
		DoQuestFunctionDelay("Tutorial_secondfight_SandBox", 1.0);
		DeleteAttribute(pchar, "systeminfo.tutorial.secondfight_SandBox");
	}
}