...
void CreatUnderwater(aref loc) // под водой
{
	if (loc.type == "underwater")
	{
		...
		if (CheckAttribute(loc, "type.LSC"))
		{
			...
			// belamour legendary edition оберег ловца -->
			if(IsEquipCharacterByArtefact(pchar, "talisman10")) SetShowTimer(750.0);
			else 
			{
		...
		{
			if (CheckAttribute(loc, "watertemple")) 
			// belamour legendary edition оберег ловца -->
			{
...
void CreatePlantation(aref loc)
{
	if(CheckAttribute(loc,"type") && loc.type == "Plantation")
	{
		...
		if(loc.id == "IslaMona_factoria" && !CheckAttribute(pchar,"questTemp.IslaMona.Factory")) return;
		int iColony = -1;
		int iNation = -1;
		...
		i = 0;
		num = rand(3) + 7; //кол-во
		if (loc.id == "IslaMona_factoria") num = 20;
		while(i < num)
		{
			iMassive = rand(9);
			if (model[iMassive] != "")
			{
				...
				if (loc.id == "IslaMona_factoria") 
				{
					chr.Dialog.Filename = "Quest\IslaMona_NPC.c";
					chr.dialog.currentnode = "slave";
				}
				...
				i++;
				if (loc.id != "IslaMona_factoria") model[iMassive] = "";
			}
		}
		...
		if (checkAttribute(loc, "soldiers") && CheckAttribute(loc, "locators.soldiers"))
		{
			for (i=1; i<=3; i++)
			{
				...
				if (i == 2 || i == 3) 
				{
					...
					{
						chr.Dialog.Filename = "Quest\IslaMona_NPC.c";
						chr.dialog.currentnode = "fort_pirate";
					}
				}
				else 
				{
